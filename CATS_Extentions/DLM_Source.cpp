...
double GaussSourceScaledTF1(double *x, double *Pars) {
    ...
    double &Size = Pars[0];
    double &Alpha = Pars[1];
    return Alpha * 4. * Pi * Radius * Radius * pow(4. * Pi * Size * Size, -1.5) *
           exp(-(Radius * Radius) / (4. * Size * Size));
}
...
// based on single particle emission, which leads to a rather complicated expression, that is approximated with the
// formula below
...
double CauchySourceTheta(double *Pars) { return 0.5 * CauchySource(Pars); }
...
double CauchySource_v2(double *Pars) {
    ...
    // double& CosTheta = Pars[2];
    double &Size = Pars[3];
    return 12. * Radius * Size * sqrt(2) /
           (Pi * Pi * sqrt(Radius * Radius + 4. * Size * Size) * (Radius * Radius + 6. * Size * Size)) *
...
double DoubleGaussSource(double *Pars) {
    ...
    // printf(" G1=%.1f x
    // %e\n",Weight1, 4.*Pi*Radius*Radius*pow(4.*Pi*Size1*Size1,-1.5)*exp(-(Radius*Radius)/(4.*Size1*Size1))); printf("
    // G2=%.1f x
    // %e\n",1.-Weight1, 4.*Pi*Radius*Radius*pow(4.*Pi*Size2*Size2,-1.5)*exp(-(Radius*Radius)/(4.*Size2*Size2)));

    return Weight1 * 4. * Pi * Radius * Radius * pow(4. * Pi * Size1 * Size1, -1.5) *
               exp(-(Radius * Radius) / (4. * Size1 * Size1)) +
           (1. - Weight1) * 4. * Pi * Radius * Radius * pow(4. * Pi * Size2 * Size2, -1.5) *
               exp(-(Radius * Radius) / (4. * Size2 * Size2));
}

double NormDoubleGaussSource(double *Pars) { return Pars[6] * DoubleGaussSource(Pars); }
...
//[7]_10: sigma3
//[8]_11: shift3
// weight3 = 1.-weight1-(1-weight1) * weight2 (i.e. limits between 0-1 and weighted such that it cannot go beyond
// allowed vals)
double NormTripleShiftedGauss(double *Pars) {
    ...
    if (W1 < 0 || W1 > 1 || W2 < 0 || W2 > 1 || W3 < 0 || W3 > 1) {
        ...
        if (!ErrorShown) {
            printf("\033[1;31mERROR:\033[0m (NormTripleShiftedGauss) The weights have unphysical values, possible "
                   "ERROR in the fit!!!\n");
...
double StupidGaussSum(double *Pars) {
    ...
    for (unsigned uG = 0; uG < NG; uG++) {
        ...
        if (!ErrorShown && (Weight < 0 || Weight > 1)) {
            printf("\033[1;31mERROR:\033[0m (StupidGaussSum) The weights have unphysical values, possible ERROR in the "
                   "fit!!!\n");
            ErrorShown = true;
        }
        WeightSum += Weight;
        double NormDist =
            pow(2. * Pi, -0.5) / Pars[6 + uG * 3] * exp(-0.5 * pow((Rad - Pars[5 + uG * 3]) / Pars[6 + uG * 3], 2.));
...
//"remaining" weight (i.e. 1 - weight of all previous poissons). As long as all weight pars are within 0-1 this will
//work
    ...
    for (unsigned uP = 0; uP < KdpPars::NumDistos; uP++) {
        ...
        if (RemainingNorm < 0 && RemainingNorm > -1e-6) {
            ...
            if (kdppars.wght[uP] >= 0 && kdppars.wght[uP] <= 1) {
                ...
                printf("\033[1;33mWARNING!\033[0m Could not properly correct the Norm! Check the weights!!! "
                       "RemainingNorm=%.3e\n",
                       RemainingNorm);
            }
        }
...
double LevyIntegral3D_2particle(double *Pars) {
    ...
    // double RetVal =
    // pow(Radius*IntVar,Dim*0.5)*DLM_Bessel1(Dim*0.5-1.,Radius*IntVar)*exp(-pow(Scale*2./Stability*IntVar,Stability));
    // printf(" RetVal=%f\n",RetVal);
    // return IntVar*IntVar/sqrt(IntVar*RadSzRatio)*DLM_Bessel1(0.5,IntVar*RadSzRatio)*exp(-pow(IntVar,Stability));
    return pow(Radius * IntVar, Dim * 0.5) * DLM_Bessel1(Dim * 0.5 - 1., Radius * IntVar) *
           exp(-pow(Scale * 2. / Stability * IntVar, Stability));
    // return RetVal;
}
...
// THIS IS ALL IMPLEMENTED IN HERE, SO THAT WE GET THE DISTRIBUTION CORRESPONDING TO
...
double LevySource3D_2particle(double *Pars) {
    ...
    if (Stability == 1) {
        return 2.97 * 2. * Scale * sqrt(2) * Radius * Radius / Pi *
               pow(Radius * Radius + 0.5 * 2.97 * 2.97 * Scale * Scale, -2.);
    } else if (Stability == 2) {
        return 4. * Pi * Radius * Radius * pow(4. * Pi * Scale * Scale, -1.5) *
               exp(-(Radius * Radius) / (4. * Scale * Scale));
    }
...
double LevyIntegral3D_single(double *Pars) {
    ...
    if (IntVar == 0)
        return 0;
    return pow(Radius * IntVar, Dim * 0.5) * DLM_Bessel1(Dim * 0.5 - 1., Radius * IntVar) *
           exp(-pow(Scale / sqrt(Stability) * IntVar, Stability));
}
...
double LevySource3D_single(double *Pars) {
    ...
    if (Stability == 1) {
        return 2.97 * Scale * sqrt(2) * Radius * Radius / Pi *
               pow(Radius * Radius + 0.125 * 2.97 * 2.97 * Scale * Scale, -2.);
    } else if (Stability == 2) {
        return 4. * Pi * Radius * Radius * pow(2. * Pi * Scale * Scale, -1.5) *
               exp(-(Radius * Radius) / (2. * Scale * Scale));
    }
...
double LevyIntegral3D(double *Pars) {
    ...
    if (IntVar == 0)
        return 0;
    return pow(Radius * IntVar, Dim * 0.5) * DLM_Bessel1(Dim * 0.5 - 1., Radius * IntVar) *
           exp(-pow(Scale * IntVar, Stability));
}
// using this function, we get the 3D Levy distribution (single particle)
// what is a bit worrying is that this guy give slightly different results compared to my DLM_Random::Stable, but
// whatever This implementation over here is written to be numerically unstable, so maybe that is the reason...
...
double LevySource_A(double* Pars){
    ...
    if(Stability<=0 || Stability>2) return 0;
        ...
        double k1 =
pow(2.,Stability)*sin(Pi*Stability*0.5)*exp(gammln((Stability+2.)*0.5))*exp(gammln((Stability+Dim)*0.5))/
...
PM1->GetIntegratedSource(0)->GetBinContent(PM1->GetIntegratedSource(0)->FindBin(Radius), GaussSource(Pars)));
...
// I shift it by some amount (t*k/m), this shift is dynamically modified to grow with r (starting from zero) by
// introducing some atan dependence, and finally the power law of r is changed depending on the radius and tau, we start
// off with r^2 and transition to ln^2(r+1) N.B. This function will most likely fail completely if t*k/m is comparable
...
double Gauss_Exp_Approx(double *Pars) {
    ...
    // double rMOM1 =
    // sqrt(pow(MMass,4.)-2.*pow(MMass*Mass[0],2.)+pow(Mass[0],4.)-2.*pow(MMass*Mass[1],2.)-2.*pow(Mass[0]*Mass[1],2.)+pow(Mass[1],4.))/(2.*Mass[0]);
    double OldRad = RAD;
    ...
    // this function is not normalized by default. This is done here. However we save information about the last time a
    // normalization was performed, if nothing has changed, this means NORM already has the correct value and we skip
    // this rather expensive step
    if ((oldMOM != MOM || oldSIG != SIG || oldTKM != TKM || oldMASS != MASS) && NORM != 1.23456789) {
        ...
        // we are interested in the true value of the integral, without normalization. Thus later on the Result is not
        // normalized
        NORM = 1.23456789;
...
// the exact solution of the same thing obtained with Mathematica. Stable only if Sigma and Tau are very similar (up to
// factor 2)
double Gauss_Exp_Exact(double *Pars) {
    ...
    if (TKM1 == 0 && TKM2 == 0) {
        ...
        return (pow(E, -pow(RAD, 2) / (4. * pow(SIG, 2)) - RAD / TKM2) *
                (4 * pow(E, RAD / TKM2) * SIG * TKM2 * (pow(SIG, 2) + pow(TKM2, 2)) -
                 2 * pow(E, pow(RAD, 2) / (4. * pow(SIG, 2))) * SIG * TKM2 *
                     (2 * pow(SIG, 2) + TKM2 * (-RAD + 2 * TKM2)) +
                 pow(E, pow(RAD, 2) / (4. * pow(SIG, 2)) + pow(SIG, 2) / pow(TKM2, 2)) * sqrt(Pi) *
                     (-4 * pow(SIG, 4) + 2 * pow(SIG, 2) * (RAD - 3 * TKM2) * TKM2 + RAD * pow(TKM2, 3)) *
                     (erf(RAD / (2. * SIG) - SIG / TKM2) + erf(SIG / TKM2)))) /
               (sqrt(Pi) * pow(TKM2, 5));
    } else if (TKM2 == 0) {
        return (pow(E, -pow(RAD, 2) / (4. * pow(SIG, 2)) - RAD / TKM1) *
                (4 * pow(E, RAD / TKM1) * SIG * TKM1 * (pow(SIG, 2) + pow(TKM1, 2)) -
                 2 * pow(E, pow(RAD, 2) / (4. * pow(SIG, 2))) * SIG * TKM1 *
                     (2 * pow(SIG, 2) + TKM1 * (-RAD + 2 * TKM1)) +
                 pow(E, pow(RAD, 2) / (4. * pow(SIG, 2)) + pow(SIG, 2) / pow(TKM1, 2)) * sqrt(Pi) *
                     (-4 * pow(SIG, 4) + 2 * pow(SIG, 2) * (RAD - 3 * TKM1) * TKM1 + RAD * pow(TKM1, 3)) *
    ...
    //! shouldn't I take TKM=TKM1+TKM2 for the computation, verify!
        ...
        WARNING = false;
        return (pow(E, (pow(SIG, 2) - RAD * TKM1) / pow(TKM1, 2)) *
                ((-2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM1, 2)) * TKM1) +
                 (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM1, 2))) * pow(SIG, 3)) / TKM1 -
                 (4 * (-1 + pow(E, -pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2)))) *
                  pow(SIG, 3)) /
                     TKM1 +
                 sqrt(Pi) * pow(SIG, 2) * erf(SIG / TKM1) +
                 (2 * sqrt(Pi) * pow(SIG, 4) * erf(SIG / TKM1)) / pow(TKM1, 2) -
                 (sqrt(Pi) * pow(SIG, 3) * (2 * pow(SIG, 2) - RAD * TKM1) *
                  erf(abs(-2 * pow(SIG, 2) + RAD * TKM1) / (2. * SIG * TKM1))) /
                     (pow(TKM1, 3) * abs(-RAD / (2. * SIG) + SIG / TKM1)) -
                 (pow(2 * pow(SIG, 2) - RAD * TKM1, 3) *
                  (-abs(-2 * pow(SIG, 2) + RAD * TKM1) +
                   pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2))) * sqrt(Pi) * SIG *
                       TKM1 * erf(abs(-2 * pow(SIG, 2) + RAD * TKM1) / (2. * SIG * TKM1)))) /
                     (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2))) *
                      pow(SIG, 2) * pow(TKM1, 4) * pow(abs(-RAD / (2. * SIG) + SIG / TKM1), 3)))) /
        ...
        return (pow(E, (pow(SIG, 2) - RAD * TKM1) / pow(TKM1, 2)) *
                    ((-2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM1, 2)) * TKM1) +
                     ...
                     (pow(2 * pow(SIG, 2) - RAD * TKM1, 3) *
                      (-abs(-2 * pow(SIG, 2) + RAD * TKM1) +
                       pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2))) * sqrt(Pi) *
                           SIG * TKM1 * erf(abs(-2 * pow(SIG, 2) + RAD * TKM1) / (2. * SIG * TKM1)))) /
                         (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2))) *
                          pow(SIG, 2) * pow(TKM1, 4) * pow(abs(RAD / (2. * SIG) - SIG / TKM1), 3))) +
                pow(E, (pow(SIG, 2) - RAD * TKM2) / pow(TKM2, 2)) *
                    ((2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM2, 2)) * TKM2) -
                     (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM2, 2))) * pow(SIG, 3)) / TKM2 +
                     (4 * (-1 + pow(E, -pow(-2 * pow(SIG, 2) + RAD * TKM2, 2) / (4. * pow(SIG, 2) * pow(TKM2, 2)))) *
                      pow(SIG, 3)) /
                         TKM2 -
                     sqrt(Pi) * pow(SIG, 2) * erf(SIG / TKM2) -
                     (2 * sqrt(Pi) * pow(SIG, 4) * erf(SIG / TKM2)) / pow(TKM2, 2) +
                     (sqrt(Pi) * pow(SIG, 3) * (2 * pow(SIG, 2) - RAD * TKM2) *
                      erf(abs(-2 * pow(SIG, 2) + RAD * TKM2) / (2. * SIG * TKM2))) /
                         (pow(TKM2, 3) * abs(RAD / (2. * SIG) - SIG / TKM2)) +
                     (pow(2 * pow(SIG, 2) - RAD * TKM2, 3) *
                      (-abs(-2 * pow(SIG, 2) + RAD * TKM2) +
                       pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM2, 2) / (4. * pow(SIG, 2) * pow(TKM2, 2))) * sqrt(Pi) *
                           SIG * TKM2 * erf(abs(-2 * pow(SIG, 2) + RAD * TKM2) / (2. * SIG * TKM2)))) /
                         (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM2, 2) / (4. * pow(SIG, 2) * pow(TKM2, 2))) *
                          pow(SIG, 2) * pow(TKM2, 4) * pow(abs(RAD / (2. * SIG) - SIG / TKM2), 3)))) /
...
double GaussExpSimple_Exact(double *Pars) {
    ...
    if (TKM == 0) {
        ...
        return (pow(E, (pow(SIG, 2) - RAD * TKM) / pow(TKM, 2)) *
                ((-2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM, 2)) * TKM) +
                 (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM, 2))) * pow(SIG, 3)) / TKM -
                 (4 * (-1 + pow(E, -pow(-2 * pow(SIG, 2) + RAD * TKM, 2) / (4. * pow(SIG, 2) * pow(TKM, 2)))) *
                  pow(SIG, 3)) /
                     TKM +
                 sqrt(Pi) * pow(SIG, 2) * erf(SIG / TKM) + (2 * sqrt(Pi) * pow(SIG, 4) * erf(SIG / TKM)) / pow(TKM, 2) -
                 (sqrt(Pi) * pow(SIG, 3) * (2 * pow(SIG, 2) - RAD * TKM) *
                  erf(abs(-2 * pow(SIG, 2) + RAD * TKM) / (2. * SIG * TKM))) /
                     (pow(TKM, 3) * abs(-RAD / (2. * SIG) + SIG / TKM)) -
                 (pow(2 * pow(SIG, 2) - RAD * TKM, 3) *
                  (-abs(-2 * pow(SIG, 2) + RAD * TKM) +
                   pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM, 2) / (4. * pow(SIG, 2) * pow(TKM, 2))) * sqrt(Pi) * SIG *
                       TKM * erf(abs(-2 * pow(SIG, 2) + RAD * TKM) / (2. * SIG * TKM)))) /
                     (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM, 2) / (4. * pow(SIG, 2) * pow(TKM, 2))) *
                      pow(SIG, 2) * pow(TKM, 4) * pow(abs(-RAD / (2. * SIG) + SIG / TKM), 3)))) /
...
double GaussExpTotIdenticalSimple_2body(double *Pars) {
    ...
    double oldrTAU = rTAU;
    rTAU = rTAU *
           sqrt(pow(rMASS, 4.) - 2. * pow(rMASS * p1MASS, 2.) + pow(p1MASS, 4.) - 2. * pow(rMASS * p2MASS, 2.) -
                2. * pow(p1MASS * p2MASS, 2.) + pow(p2MASS, 4.)) /
           (2. * p1MASS) / rMASS;
...
double GaussExpTotSimple_2body(double *Pars) {
    ...
    TKMA = rTAU *
           ...
           (2. * p1MASS) / rMASS;
    TKMB = rTAU_2 *
           sqrt(pow(rMASS_2, 4.) - 2. * pow(rMASS_2 * p1MASS_2, 2.) + pow(p1MASS_2, 4.) -
                2. * pow(rMASS_2 * p2MASS_2, 2.) - 2. * pow(p1MASS_2 * p2MASS_2, 2.) + pow(p2MASS_2, 4.)) /
           (2. * p1MASS_2) / rMASS_2;
    PARS[0] = Pars[0];
    PARS[1] = Pars[1];
    PARS[2] = Pars[2];
...
// Gauss including kT dependence, convoluted further with an exponential. The latter can be used to model resonances. In
// case we have two resonances, a nice approximation is just to add their tau/mass the parameters are the following: [3]
//= Number of kT bins NkT [4] = Number of resonances related to particle 1 [5] = Number of resonances related to
//particle 2
...
void MS_GaussExp_mT_Simple::SetLinear_mT(const double &lin) { Linear_mT = lin; }
void MS_GaussExp_mT_Simple::SetSlope_mT(const double &slope) { Slope_mT = slope; }
...
unsigned MS_GaussExp_mT_Simple::GetNumPars() { return 14; }
...
void DLM_StableDistribution::Generate(const double &stability, const double &location, const double &scale,
...
unsigned DLM_StableDistribution::GetNumPars() { return 4; }
...
unsigned DLM_CleverLevy::GetNumPars() { return 2; }
...
void DLM_CleverMcLevyReso::SetUpReso(const unsigned &whichparticle, const unsigned &whichreso, const double &weight,
                                     const double &mass, const double &tau, const double &mass0, const double &mass1,
                                     const double &momSmear, const bool &massSmear, const RESO_DEC_TOP &rdt) {
    if (whichparticle >= 2) {
    ...
    if (ResoWeight[whichparticle][whichreso] == weight && ResoMass[whichparticle][whichreso] == mass &&
        ResoTau[whichparticle][whichreso] == tau && ChildMass0[whichparticle][whichreso] == mass0 &&
        ChildMass1[whichparticle][whichreso] == mass1 && SmearResoMomentum[whichparticle][whichreso] == momSmear &&
        SmearResoMass[whichparticle][whichreso] == massSmear && ResoDecayTopology[whichparticle][whichreso] == rdt) {
        return;
    }
    if (Histo)
        Histo->SetBinContentAll(1e6);
...
void DLM_CleverMcLevyReso::SetUpResoEmission(const unsigned &whichparticle, const unsigned &whichreso,
                                             const DLM_Histo<double> *Distr) {
    if (whichparticle >= 2) {
                ...
                for (unsigned uIter = 0; uIter < NumMcIter; uIter++) {
                    ...
                    for (unsigned uParticle = 0; uParticle < 2; uParticle++) {
                        ...
                        for (unsigned uReso = 0; uReso < NumResonances[uParticle]; uReso++) {
                            // printf(" RanVal=%f; CW=%f;
                            // CW+RW=%f\n",RanVal,CummulativeWeight,CummulativeWeight+ResoWeight[uParticle][uReso]);
                            if (RanVal >= CummulativeWeight &&
                                RanVal < CummulativeWeight + ResoWeight[uParticle][uReso]) {
                                WhichReso = int(uReso);
                                // printf("WhichReso=%i from
                        ...
                        if (SmearResoMass[uParticle][WhichReso] && ResoTau[uParticle][WhichReso]) {
                            ...
                            } while (SmearedResoMass <
                                     ChildMass0[uParticle][WhichReso] + ChildMass1[uParticle][WhichReso]);
                        }

                        // we compute the effective momentum of the resonance
                        ResoMomentum =
                            sqrt(pow(SmearedResoMass, 4.) -
                                 2. * pow(SmearedResoMass * ChildMass0[uParticle][WhichReso], 2.) +
                                 pow(ChildMass0[uParticle][WhichReso], 4.) -
                                 2. * pow(SmearedResoMass * ChildMass1[uParticle][WhichReso], 2.) -
                        ...
                        if (SmearResoMomentum[uParticle][WhichReso] > 0) {
                            do {
                                SmearedResoMomentum =
                                    RanGen.Gauss(ResoMomentum, ResoMomentum * SmearResoMomentum[uParticle][WhichReso]);
                                // printf("Smear %.2f%% from %.3f to
                        ...
                        //! I think it should be M/(p*tau), which is the opposite as done before??????? Check for
                        //! consistency!!! also make sure that the ResoTau gets here with the correct units (natural,
                        //! i.e. 1/MeV)
                        ...
                        // random
                        case rdtRandom:
                            if (ResoEmissionAngle && ResoEmissionAngle[uParticle] &&
                                ResoEmissionAngle[uParticle][WhichReso]) {
                                // the theta angle we sample from the distribution we gave
                                RanVal = RanGen.Uniform(1e-6, 1 - 1e-6); // 0-->1
                                ResoPathSpherical[uParticle][1] =
                                    ResoEmissionAngle[uParticle][WhichReso]->Eval(&RanVal);
                                // make sure that the direction is inverted for the second particle
                                if (uParticle)
                                ...
                                // the upper thing was wrong, as actually for theta it is the cosine that is random, now
                                // corrected
                        ...
                        ResoPathCartesian[uParticle][0] = ResoPathSpherical[uParticle][0] *
                                                          sin(ResoPathSpherical[uParticle][1]) *
                                                          cos(ResoPathSpherical[uParticle][2]);
                        ResoPathCartesian[uParticle][1] = ResoPathSpherical[uParticle][0] *
                                                          sin(ResoPathSpherical[uParticle][1]) *
                                                          sin(ResoPathSpherical[uParticle][2]);
                        ResoPathCartesian[uParticle][2] =
                            ResoPathSpherical[uParticle][0] * cos(ResoPathSpherical[uParticle][1]);
                        // printf(" up%u: x=%f, y=%f,
                    ...
                    // printf(" RAD=%f --> ",RAD);
                    RAD = sqrt(pow(ResoPathCartesian[0][0] - ResoPathCartesian[1][0], 2.) +
                    ...
                    // if(BinSize==0){
                    // printf("We have problem with BinSize --> WhichBin[0]=%u (DiffVal=%f,
...
unsigned DLM_CleverMcLevyReso::GetNumPars() { return 2; }
...
void DLM_CleverMcLevyResoTM::AddBGT_PR(const float &bgt, const float &a_cp) {
    if (NumBGT_PR >= MaxBGT_PR) {
        ...
        //}
        // else{BGT_PR = new float* [MaxBGT_PR];for(unsigned uEntry=0; uEntry<MaxBGT_PR; uEntry++)BGT_PR[uEntry] = new
        // float [2];}
        for (unsigned uEntry = 0; uEntry < NumBGT_PR; uEntry++) {
...
void DLM_CleverMcLevyResoTM::AddBGT_RP(const float &bgt, const float &a_cp) {
    if (NumBGT_RP >= MaxBGT_RP) {
        ...
        //}
        // else{BGT_RP = new float* [MaxBGT_RP];for(unsigned uEntry=0; uEntry<MaxBGT_RP; uEntry++)BGT_RP[uEntry] = new
        // float [2];}
        for (unsigned uEntry = 0; uEntry < NumBGT_RP; uEntry++) {
            delete[] tempfloat[uEntry];
        }
...
void DLM_CleverMcLevyResoTM::AddBGT_RR(const float &bgt0, const float &a_cp0, const float &bgt1, const float &a_cp1,
                                       const float &a_p0p1) {
    if (NumBGT_RR >= MaxBGT_RR) {
        ...
        //}
        // else{BGT_RR = new float* [MaxBGT_RR];for(unsigned uEntry=0; uEntry<MaxBGT_RR; uEntry++)BGT_RR[uEntry] = new
        // float [5];}
        for (unsigned uEntry = 0; uEntry < NumBGT_RR; uEntry++) {
            delete[] tempfloat[uEntry];
        }
...
void DLM_CleverMcLevyResoTM::SetNormalization(const double &norm) { Normalization = norm; }
...
#pragma omp critical
                ...
                for (unsigned uIter = 0; uIter < NumMcIter; uIter++) {
                    ...
                    RAD = sqrt(RAD * RAD + BGT[0] * BGT[0] + BGT[1] * BGT[1] - 2. * RAD * BGT[0] * CosRcP0 +
                               2. * RAD * BGT[1] * CosRcP1 - 2. * BGT[0] * BGT[1] * CosP0P1);
                    // static double RAD_avg=0;
                    // RAD_avg+=RAD;
                    ...
                    // printf(" sqrt(%.1f^2 + %.3f^2 + %.3f^2 - 2*%.1f*%.3f*%.3f + 2*%.1f*%.3f*%.3f -
                    // 2*%.3f*%.3f*%.3f)\n",
...
unsigned DLM_CleverMcLevyResoTM::GetNumPars() { return 2; }
    ...
    do {
        ...
        if (InFile) {
            ...
            while (!feof(InFile)) {
                ...
                if (ReadMode == 0) {
                    ...
                    if (ControlMt == NumMtBins) {
                        // printf("\033[1;33mWARNING!\033[0m Possible bad input, as the num of Mt bins does not check
                        // out (max = %u)\n",NumMtBins);
                        continue;
                    } else {
                        sscanf(cline,
                               ...
                               "%f %f %f",
                               &FitMode, &MtBinCenter[ControlMt], &SrcPar[ControlMt].mean[0],
                               &SrcPar[ControlMt].stdv[0], &SrcPar[ControlMt].wght[0], &SrcPar[ControlMt].mean[1],
                               &SrcPar[ControlMt].stdv[1], &SrcPar[ControlMt].wght[1], &SrcPar[ControlMt].mean[2],
                               &SrcPar[ControlMt].stdv[2], &SrcPar[ControlMt].wght[2], &SrcPar[ControlMt].mean[3],
                               &SrcPar[ControlMt].stdv[3], &SrcPar[ControlMt].wght[3], &SrcPar[ControlMt].mean[4],
                               &SrcPar[ControlMt].stdv[4], &SrcPar[ControlMt].wght[4], &SrcPar[ControlMt].mean[5],
                               &SrcPar[ControlMt].stdv[5], &SrcPar[ControlMt].wght[5], &SrcPar[ControlMt].mean[6],
                               &SrcPar[ControlMt].stdv[6], &SrcPar[ControlMt].wght[6], &SrcPar[ControlMt].mean[7],
                               &SrcPar[ControlMt].stdv[7], &SrcPar[ControlMt].wght[7], &SrcPar[ControlMt].mean[8],
                               &SrcPar[ControlMt].stdv[8], &SrcPar[ControlMt].wght[8], &SrcPar[ControlMt].mean[9],
                               &SrcPar[ControlMt].stdv[9], &Chi2Ndf[ControlMt]);
                        ControlMt++;
                    }
                } // ReadMode
            } // while InFile
            ...
            for (unsigned uMt = 0; uMt < NumMtBins; uMt++) {
                ...
                // if(fabs(MtBinCenter[uMt]-1110)<1 && fabs(d_x-0.1867)<0.001 && fabs(h_x-4.067)<0.01 &&
                // fabs(h_z-10.167)<0.01 ){ if(WhichBin[1]>0){ printf("file = %s\n",InFileName.c_str());
...
bool DLM_CecaSource_v0::InitHisto() {
    ...
    if (AnaVersion.rfind("Cigar2_ds", 0) == 0 || AnaVersion.rfind("Jaime1_ds", 0) == 0 ||
        AnaVersion.rfind("JaimeDelay1_dLs", 0) == 0) { // pos=0 limits the search to the prefix
        ...
        while (getline(ssAnaVersion, str_tmp, '_')) {
            ...
            if (str_tmp.rfind("ds", 0) == 0) {
                ...
                // printf("ND=%u\n",NumDist);
                       ...
                       str_tmp.rfind("JaimeDelay1", 0) == 0) {
                // do nothing
            } else {
                printf("\033[1;31mERROR:\033[0m DLM_CecaSource_v0::InitHisto something wrong with the pars of "
                       "AnaVersion = %s!\n",
                       AnaVersion.c_str());
                return false;
            }
...
double DLM_CecaSource_v0::Eval(double *kxc) { return RootEval(&kxc[1], &kxc[3]); }
...
double DLM_CecaSource_v0::RootEval(double *x, double *pars) {
    ...
    for (unsigned uPar = 0; uPar < 4; uPar++) {
        if (pars[uPar] < dlmSource->GetLowEdge(uPar) || pars[uPar] > dlmSource->GetUpEdge(uPar)) {
            printf("\033[1;33mWARNING:\033[0m DLM_CecaSource_v0::RootEval pars[%u] = %.3e is outside the allowed range "
                   "[%.3e, %.3e]!\n",
...
unsigned DLM_CecaSource_v0::GetNbins(unsigned WhichPar) { return dlmSource->GetNbins(WhichPar); }
...
// creates a copy
DLM_KdpSource::DLM_KdpSource(KdpPars &input_kdp) { source_kdp = input_kdp; }
...
double DLM_KdpSource::RootEval(double *x, double *pars) { return PoissonSum(*x, source_kdp); }
...
DLM_MultiKdpSource::DLM_MultiKdpSource(DLM_Histo<KdpPars> &kdp_source, const int WhereIsKstar,
                                       const int WhereIsCosTheta)
    ...
    if (WhereIsKstar == WhereIsCosTheta && WhereIsKstar >= 0) {
        printf("\033[1;31mERROR:\033[0m Bad input in DLM_MultiKdpSource::DLM_MultiKdpSource(...): "
               "WhereIsKstar==WhereIsCosTheta\n");
        return;
    }
