...
DLM_CkDecomp::DLM_CkDecomp(const char *name, const unsigned &numchildren, DLM_Ck &ckfunction,
                           const DLM_Histo<float> *hSigmaMatrix, const bool &InvertAxis)
...
DLM_CkDecomp::~DLM_CkDecomp() {
    ...
    if (NumChildren) {
        ...
        delete[] Type;
        Type = NULL;
        delete[] CurrentStatusChild;
        CurrentStatusChild = NULL;

        for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
            if (RM_Child[uChild])
                delete RM_Child[uChild];
            if (SM_Child[uChild])
                delete SM_Child[uChild];
            if (CkChildMainFeed[uChild])
                delete CkChildMainFeed[uChild];
            if (CkSmearedChildMainFeed[uChild])
                delete CkSmearedChildMainFeed[uChild];
            if (SignalChild[uChild])
                delete SignalChild[uChild];
            if (SignalSmearedChild[uChild])
                delete SignalSmearedChild[uChild];
            if (PS_Child[uChild])
                delete PS_Child[uChild];
            if (LambdaPar[uChild])
                delete LambdaPar[uChild];
        }
        delete[] LambdaPar;
        LambdaPar = NULL;
        delete[] RM_Child;
        ...
        delete[] SM_Child;
        SM_Child = NULL;
        delete[] CkChildMainFeed;
        CkChildMainFeed = NULL;
        delete[] CkSmearedChildMainFeed;
        CkSmearedChildMainFeed = NULL;
        delete[] SignalChild;
        SignalChild = NULL;
        delete[] SignalSmearedChild;
        SignalSmearedChild = NULL;
        delete[] PS_Child;
        PS_Child = NULL;
    }
    ...
    if (RM_MomResolution) {
        delete RM_MomResolution;
        RM_MomResolution = NULL;
    }
    if (CkMainSmeared) {
        delete CkMainSmeared;
        CkMainSmeared = NULL;
    }
    if (CkMainFeed) {
        delete CkMainFeed;
    ...
    if (CkSmearedMainFeed) {
        delete CkSmearedMainFeed;
        CkSmearedMainFeed = NULL;
    }
    if (SignalMain) {
        delete SignalMain;
    ...
    if (SignalSmearedMain) {
        delete SignalSmearedMain;
        SignalSmearedMain = NULL;
    }
    if (PS_Main) {
        delete PS_Main;
...
void DLM_CkDecomp::AddPhaseSpace(const unsigned &WhichCk, const DLM_Histo<float> *hPhaseSpace) {
    ...
    if (PS_Child[WhichCk]) {
        delete PS_Child[WhichCk];
        PS_Child[WhichCk] = NULL;
    }
    if (!hPhaseSpace)
        return;
...
void DLM_CkDecomp::AddContribution(unsigned WhichCk, DLM_Histo<double> &fraction, int type, DLM_CkDecomp *child,
    ...
    if (child && !UniqueName(child->Name)) {
        printf("\033[1;33mWARNING:\033[0m Trying to duplicate the name '%s' of a contribution in "
               "DLM_CkDecomp::SetContribution.\n",
               child->Name);
        return;
    }
    ...
    if (RM_Child[WhichCk]) {
        delete RM_Child[WhichCk];
        RM_Child[WhichCk] = NULL;
    }
    if (SM_Child[WhichCk]) {
        delete SM_Child[WhichCk];
        SM_Child[WhichCk] = NULL;
    }
    ...
    if (child && RM_MomResolution) {
        SM_Child[WhichCk] = new DLM_ResponseMatrix(*child->CkMain, RM_MomResolution->hSigmaMatrix, NULL,
                                                   RM_MomResolution->InvertedAxis);
    }

    if (CkChildMainFeed[WhichCk]) {
        delete CkChildMainFeed[WhichCk];
        CkChildMainFeed[WhichCk] = NULL;
    }
    if (child) {
    ...
    for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
        if (LambdaPar[uChild]) {
            *LambdaMain -= *LambdaPar[uChild];
            if (Type[uChild] == cFake)
                *MuPar -= *LambdaPar[uChild];
        }
    }
}
...
double DLM_CkDecomp::EvalCk(const double &Momentum) {
    ...
    // this is needed in order to get a smooth transition for the range outside of the histogram
        ...
        // printf("CkMain->Eval(%.0f) = %.3f (l/mu =
        // %.3f)\n",CkMain->Eval(Momentum),Momentum,LambdaMain->Eval(Momentum)/MuPar->Eval(Momentum));
        for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
            if (Type[uChild] == cFeedDown) {
                if (Child[uChild]) {
                    VAL_CkSmearedMainFeed += GetChild(uChild)->GetCk()->Eval(Momentum) *
                                             LambdaPar[uChild]->Eval(Momentum) / MuPar->Eval(Momentum);
                } else if (LambdaPar[uChild]) {
                    VAL_CkSmearedMainFeed += (LambdaPar[uChild]->Eval(Momentum) / MuPar->Eval(Momentum));
                }
            } else {
                if (Child[uChild]) {
                    VAL_CkSmearedMainFake +=
                        GetChild(uChild)->GetCk()->Eval(Momentum) * LambdaPar[uChild]->Eval(Momentum);
                } else if (LambdaPar[uChild]) {
        ...
        for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
            ...
            if (Child[uChild]) {
                VAL_CkSmearedMainFake +=
                    LambdaPar[uChild]->Eval(Momentum) * Child[uChild]->CkSmearedMainFeed->Eval(Momentum);
            } else if (LambdaPar[uChild]) {
                VAL_CkSmearedMainFake += LambdaPar[uChild]->Eval(Momentum);
            }
        }
    }
    // printf("k*=%.0f; Mu=%.3f; CkSmMF=%.3f;
...
double DLM_CkDecomp::EvalMain(const double &Momentum) { return CkMain->Eval(Momentum); }
...
double DLM_CkDecomp::EvalMainFeed(const double &Momentum) { return CkMainFeed->Eval(Momentum); }
double DLM_CkDecomp::EvalSmearedMainFeed(const double &Momentum) { return CkSmearedMainFeed->Eval(Momentum); }
double DLM_CkDecomp::EvalSmearedFeed(const unsigned &WhichChild, const double &Momentum) {
    ...
    if (CkSmearedChildMainFeed[WhichChild])
        return CkSmearedChildMainFeed[WhichChild]->Eval(Momentum);
    else
        return 0;
}
...
double DLM_CkDecomp::EvalSignal(const double &Momentum) {
    ...
    for (unsigned uChild = 0; uChild < NumChildren; uChild++)
        Result += EvalSignalChild(uChild, Momentum);
    return Result;
}
...
void DLM_CkDecomp::QA_LambdaPar(const bool &yesno) { qa_lam = yesno; }

double DLM_CkDecomp::GetLambdaMain(const double Momentum) { return LambdaMain->Eval(Momentum); }
double DLM_CkDecomp::GetLambdaChild(const unsigned &WhichChild, const double Momentum) {
    if (WhichChild >= NumChildren)
        return 0;
    if (!LambdaPar[WhichChild])
        return 0;
    return LambdaPar[WhichChild]->Eval(Momentum);
}
unsigned DLM_CkDecomp::GetNumChildren() { return NumChildren; }

DLM_CkDecomp *DLM_CkDecomp::GetChild(const unsigned &WhichChild) {
    if (WhichChild >= NumChildren)
        return NULL;
    else
        return Child[WhichChild];
}
DLM_CkDecomp *DLM_CkDecomp::GetContribution(const char *name) {
    ...
    for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
        if (Child[uChild])
            RESULT = Child[uChild]->GetContribution(name);
        if (RESULT)
            break;
    }
    return RESULT;
...
DLM_Ck *DLM_CkDecomp::GetCk() { return CkMain; }

const DLM_Histo<float> *DLM_CkDecomp::GetResolutionMatrix() { return RM_MomResolution->hSigmaMatrix; }
    ...
    if (!DecompositionStatus && qa_lam) {
        ...
        for (unsigned uMom = 0; uMom < TotLambda.GetNbins(); uMom++) {
            if (fabs(TotLambda.GetBinContent(uMom) - 1.) > 1e-6) {
                printf("\033[1;33mWARNING:\033[0m The lambda parameters sum to %f. Rescaling them such as to add to "
                       "unity!\n",
                       TotLambda.GetBinContent(uMom));
                LambdaMain->SetBinContent(uMom, LambdaMain->GetBinContent(uMom) / TotLambda.GetBinContent(uMom));
                for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
                    if (LambdaPar[uChild])
                        LambdaPar[uChild]->SetBinContent(uMom, LambdaPar[uChild]->GetBinContent(uMom) /
                                                                   TotLambda.GetBinContent(uMom));
                }
            }
        }
            ...
            if (Type[uChild] != cFeedDown) {
                if (Child[uChild]) {
                    for (unsigned uBin = 0; uBin < SignalSmearedChild[uChild]->GetNbins(); uBin++) {
                        ...
                        if (LambdaPar[uChild])
                            SignalSmearedChild[uChild]->SetBinContent(
                                uBin,
                                LambdaPar[uChild]->Eval(Momentum) * Child[uChild]->CkSmearedMainFeed->Eval(&Momentum));
                    }
                    if (LambdaPar[uChild]) {
                ...
                for (unsigned uBin = 0; uBin < CkMainFeed->GetNbins(); uBin++) {
                    ...
                    if (LambdaPar[uChild]) {
                        CkMainFeed->Add(uBin, CkChildMainFeed[uChild]->Eval(&Momentum) *
                                                  LambdaPar[uChild]->Eval(Momentum) / MuPar->Eval(Momentum));
                        SignalChild[uChild]->SetBinContent(uBin, CkChildMainFeed[uChild]->Eval(&Momentum) *
                                                                     LambdaPar[uChild]->Eval(Momentum));
                    }
                }
                if (UpdateDecomp) {
...
void DLM_CkDecomp::Smear(const DLM_Histo<double> *CkToSmear, const DLM_ResponseMatrix *SmearMatrix,
                         DLM_Histo<double> *CkSmeared, DLM_Histo<float> *PhaseSpace) {
    if (!SmearMatrix) {
        ...
        for (unsigned uBinSmear = 0; uBinSmear < CkToSmear->GetNbins(); uBinSmear++) {
            ...
            if (LastBin >= CkToSmear->GetNbins())
                LastBin = CkToSmear->GetNbins() - 1;
            double PSPACE_NORM = PhaseSpace ? 0 : 1;
            for (unsigned uBinTrue = FirstBin; uBinTrue <= LastBin; uBinTrue++) {
                // as the response matrix is normalized to the size of the bin, during the integration we multiply for
                // it
                ...
                // CkSmeared->Add(uBinSmear,
                // SmearMatrix->ResponseMatrix[uBinSmear][uBinTrue]*CkToSmear->GetBinContent(uBinTrue)*
...
bool DLM_CkDecomp::UniqueName(const char *name) {
    ...
    for (unsigned uChild = 0; uChild < NumChildren; uChild++) {
        if (Child[uChild]) {
            if (Child[uChild]->UniqueName(name) == false)
                return false;
        }
    }
    return true;
