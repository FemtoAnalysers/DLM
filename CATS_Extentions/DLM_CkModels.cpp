...
double LednickyAsInStar(const double &Momentum, const double &GaussR, const double &ScattLenSin,
                        const double &EffRangeSin, const double &Norm, const double &lambda, const double &ares,
                        const double &RadRes) {
    ...
    if (GaussR != GaussR) {
        printf("\033[1;33mWARNING:\033[0m LednickyAsInStar got a bad value for the Radius (nan). Returning default "
               "value of 1.\n");
        return 1;
    }
    ...
    return Norm * (1. +
                   lambda * (-0.5 * exp(-Radius * Radius * QMOM * QMOM) +
                             0.25 * pow(abs(ScattAmplSin) / Radius, 2) * (1 - (eRan1) / (2 * sqrt(Pi) * Radius)) +
                             real(ScattAmplSin) * F1 / (sqrt(Pi) * Radius) - imag(ScattAmplSin) * F2 / Radius / 2.) +
                   ares * exp(-RadRes * RadRes * QMOM * QMOM));
}

double GeneralLednicky(const double &Momentum, const double &GaussR, const double &ScattLenSin,
    ...
    // }
    if (GaussR != GaussR) {
        printf("\033[1;33mWARNING:\033[0m GeneralLednicky got a bad value for the Radius (nan). Returning default "
               "value of 1.\n");
        return 1;
    }
...
double GeneralLednicky2channel(const double &Momentum, const double &GaussR, const double &ScattLenSin,
                               const double &EffRangeSin, const double &ScattLenTri, const double &EffRangeTri,
                               const bool &QS, const bool &InverseScatLen, const double &Weight1,
...
double GeneralLednicky(const double &Momentum, const double &GaussR, const complex<double> &ScattLenSin,
                       const double &EffRangeSin, const complex<double> &ScattLenTri, const double &EffRangeTri,
                       const bool &SinOnly, const bool &QS, const bool &InverseScatLen) {
...
double GeneralLednickyIntegral(const double &Momentum, const double &GaussR, const double &cutoff,
                               ...
                               const complex<double> &ScattLenTri, const double &EffRangeTri, const bool &SinOnly,
...
double GeneralCoulombLednicky(const double &Momentum, const double &GaussR, const double &ScattLenSin,
    ...
    if (GaussR != GaussR) {
        printf("\033[1;33mWARNING:\033[0m GeneralCoulombLednicky got a bad value for the Radius (nan). Returning "
               "default value of 1.\n");
        return 1;
    }
...
double GeneralCoulombLednickyAvg(const double &Momentum, const double &GaussR, const std::complex<double> &ScattLenSin,
                                 const double &EffRangeSin, const bool &QS, const double &RedMass, const double &Q1Q2) {
    // const double FmToNu=5.067731237e-3;
    // const std::complex<double> i(0,1);
    // const double Pi(3.141592653589793);
    ...
    double CkValue =
        0.5 * pow(abs(ScattAmplSin) / Radius, 2) *
            (1 - (eRan1) / (2 * sqrt(Pi) * Radius) + 0.5 * pow(A_c - 1., 2) * (1. - exp(-4. * Rho2))) +
        Momentum * real(ScattAmplSin) * gsl_sf_dawson(2. * Rho) / (2. * sqrt(Pi) * Rho2) -
        Momentum * imag(ScattAmplSin) * (0.25 * (1. - exp(-4. * Rho2)) / Rho2 + (A_c - 1.) * cos(Rho) * exp(-Rho2));
    // double CkValue =    0.25*pow(abs(ScattAmplSin)/Radius,2)*(1-(eRan1)/(2*sqrt(Pi)*Radius))
...
double ComplexLednicky_Identical_Singlet_InvScatLen(const double &Momentum, const double *SourcePar,
                                                    const double *PotPar) {
    complex<double> ScatLen(PotPar[0], PotPar[1]);
...
double ComplexLednicky_Singlet_doublegaussian_lambda(const double &Momentum, const double *SourcePar,
                                                     const double *PotPar) {
    complex<double> ScatLen(PotPar[0], PotPar[1]);
    return SourcePar[3] *
               (SourcePar[2] * GeneralLednicky(Momentum, SourcePar[0], ScatLen, PotPar[2], 0, 0, true, false, false) +
                (1 - SourcePar[2]) *
                    GeneralLednicky(Momentum, SourcePar[1], ScatLen, PotPar[2], 0, 0, true, false, false)) +
           1. - SourcePar[3];
}
...
double Lednicky_2channel(const double &Momentum, const double *SourcePar, const double *PotPar) {
    return GeneralLednicky2channel(Momentum, SourcePar[0], PotPar[0], PotPar[1], PotPar[2], PotPar[3], false, false,
                                   PotPar[4], PotPar[5]);
}
...
double LednickyCoulomb_Identical_Singlet(const double &Momentum, const double *SourcePar, const double *PotPar) {
    // return
    // CoulombPenetrationFactor(Momentum,PotPar[2],PotPar[3])*Lednicky_Identical_Singlet(Momentum,SourcePar,PotPar);
...
double Lednicky_gauss_pAp_test(const double &Momentum, const double *SourcePar, const double *PotPar) {
    ...
    std::complex<double> ScattAmplCoul =
        pow(1. / sLen + 0.5 * eRan * Momentum2 - i * Momentum * A_c - 2. * Momentum * eta * CoulombEuler(eta), -1.);

    double F1 = gsl_sf_dawson(2. * Momentum * radius) / (2. * Momentum * radius);
    double F2 = (1. - std::exp(-4. * Momentum * Momentum * radius * radius)) / (2. * Momentum * radius);

    double CkValue = 0.;
    CkValue += 0.5 * std::pow(std::abs(ScattAmplCoul) / radius, 2) * (1. - (eRan) / (2 * std::sqrt(Pi) * radius)) +
               2 * std::real(ScattAmplCoul) * F1 / (std::sqrt(Pi) * radius) - std::imag(ScattAmplCoul) * F2 / radius;
...
double Lednicky_gauss_pAL(const double &Momentum, const double *SourcePar, const double *PotPar) {
    //  double Lednicky_gauss_pAL(const double &Momentum,  const double* SourcePar, const std::complex<double>&
    //  ScattLenSin, const double& EffRangeSin){
...
double Lednicky_gauss_LAL(const double &Momentum, const double *SourcePar, const double *PotPar) {
    // double Lednicky_gauss_LAL(const double &Momentum, const double* SourcePar, const std::complex<double>&
    // ScattLenSin, const double& EffRangeSin){
...
double LednickyCoulomb_Triplet(const double &Momentum, const double *SourcePar, const double *PotPar) {
    ...
    return GeneralCoulombLednicky(Momentum, SourcePar[0], PotPar[0], PotPar[1], PotPar[2], PotPar[3], false, PotPar[4],
                                  PotPar[5]);
}
...
double LednickyCoulomb_Identical_Triplet(const double &Momentum, const double *SourcePar, const double *PotPar) {
    ...
    // return
    // CoulombPenetrationFactor(Momentum,PotPar[4],PotPar[5])*Lednicky_Identical_Triplet(Momentum,SourcePar,PotPar);
    return GeneralCoulombLednicky(Momentum, SourcePar[0], PotPar[0], PotPar[1], PotPar[2], PotPar[3], true, PotPar[4],
                                  PotPar[5]);
}
...
double Lednicky_gauss_Sigma0(const double &Momentum, const double *SourcePar, const double *PotPar) {
    // This model tries to calculate the Sigma0 correlation function, one can simply say trying because nothing is known
    ...
    double k2 = sqrt(
        2 * mu2 * (MomentumGeV * MomentumGeV / (2 * mu1) + mass_proton + mass_sigma0 - mass_neutron - mass_sigmaplus));
    ...
    complex<double> D_1s0 =
        (Kmin_11_1s0 + complex<double>(0., -k1)) * (Kmin_22_1s0 + complex<double>(0., -k2)) - Kmin_12_1s0 * Kmin_12_1s0;
    complex<double> D_3s1 =
        (Kmin_11_3s1 + complex<double>(0., -k1)) * (Kmin_22_3s1 + complex<double>(0., -k2)) - Kmin_12_3s1 * Kmin_12_3s1;
    ...
    //---------------------------------------------
    double corr_1s0 =
        0.5 * pow(abs(f11_1s0), 2.) / (RG * RG) + 2. * real(f11_1s0) / (sqrt(Pi) * RG) * fF1 - imag(f11_1s0) / RG * fF2;
    corr_1s0 *= 0.25;

    double corr_3s1 =
        0.5 * pow(abs(f11_3s1), 2.) / (RG * RG) + 2. * real(f11_3s1) / (sqrt(Pi) * RG) * fF1 - imag(f11_3s1) / RG * fF2;
    corr_3s1 *= 0.75;
    //---------------------------------------------
    ...
    double corr_1s0_distort = pow(abs(f11_1s0), 2.) * 2. * real(dKmin_11_1s0_dk) +
                              pow(abs(f12_1s0), 2.) * 2. * real(dKmin_22_1s0_dk) +
                              2. * real(factor1_1s0) * 2. * real(dKmin_12_1s0_dk);
    corr_1s0_distort *= -0.25 / (4 * sqrt(Pi) * pow(RG, 3.));

    double corr_3s1_distort = pow(abs(f11_3s1), 2.) * 2. * real(dKmin_11_3s1_dk) +
                              pow(abs(f12_3s1), 2.) * 2. * real(dKmin_22_3s1_dk) +
                              2. * real(factor1_3s1) * 2. * real(dKmin_12_3s1_dk);
...
double LednickySingletScatAmplitude(const double &kStar, const double *SourcePar, const double *PotPar) {
    ...
    auto eta = [](const double kStar, const double *PotPar) {
        return std::exp(PotPar[3] * kStar) + PotPar[4] * kStar + PotPar[5] * kStar * kStar;
    };
    ...
    auto ScatteringAmplitude = [&delta, &eta](const double &kStar, const double *PotPar) {
    ...
    auto OneOverK = [&delta, &eta](const double &kStar, const double *PotPar) {
        ...
        std::complex<double> nominator(2 * etaVal * std::sin(2. * deltaVal), -(1. - etaValSq));
        return nominator * kStar / (1 + etaVal * etaVal - 2. * etaValSq * std::cos(2. * deltaVal));
    };
    ...
    const double oneOverKSecondDerivative =
        std::real(((OneOverK(kStar + precision, PotPar) - OneOverK(kStar, PotPar)) -
                   (OneOverK(kStar, PotPar) - OneOverK(kStar - precision, PotPar))) /
                  (precision * precision));

    double F1 = gsl_sf_dawson(2. * kStar * radius) / (2. * kStar * radius);
    double F2 = (1. - std::exp(-4. * kStar * kStar * radius * radius)) / (2. * kStar * radius);

    double CkValue = 0.5 * std::pow(std::abs(ScatAmpl) / radius, 2) *
                         (1. - (oneOverKSecondDerivative) / (2 * std::sqrt(Pi) * radius)) +
                     2 * std::real(ScatAmpl) * F1 / (std::sqrt(Pi) * radius) - std::imag(ScatAmpl) * F2 / radius;
...
double Lednicky_CC_pAL(const double &Momentum, const double *SourcePar, const double *PotPar) {
    ...
    double k1 = MomentumGeV / hbarc; // momentum in the elastic channel
    double k2 =
        sqrt(2 * mu2 * (MomentumGeV * MomentumGeV / (2 * mu1) + mass_proton + mass_Lambda - mass_cc1 - mass_cc2));
    k2 /= hbarc; // momentum in the inelastic channel
    //(k2>k1)
    ...
    // Determinant of scattering amplitude matrix:
    complex<double> Det =
        (Kinv_11 + complex<double>(0., -k1)) * (Kinv_22 + complex<double>(0., -k2)) - Kinv_21 * Kinv_21;
    ...
    //---------------------------------------------
    double corr_el =
        0.5 * pow(abs(f11), 2.) / (RG * RG) + 2. * real(f11) / (sqrt(Pi) * RG) * fF1 - imag(f11) / RG * fF2;
    //---------------------------------------------
    ...
    double corr_distort = pow(abs(f11), 2.) * 2. * real(d11_dk) + pow(abs(f21), 2.) * 2. * real(d22_dk) +
                          2. * real(factor1) * 2. * real(d21_dk);
...
double Lednicky_CC_LAL(const double &Momentum, const double *SourcePar, const double *PotPar) {
    ...
    double k1 = MomentumGeV / hbarc; // momentum in the elastic channel
    double k2 =
        sqrt(2 * mu2 * (MomentumGeV * MomentumGeV / (2 * mu1) + mass_Lambda + mass_Lambda - mass_cc1 - mass_cc2));
    k2 /= hbarc; // momentum in the inelastic channel
    //(k2>k1)
...
/// @brief  Lednicky-Lyuboshits formula with Flatte-like scattering amplitude for the decay into the pair we are
/// measuring (assumed channel 2) based on the Sill distribution (F. Giacosa, Eur. Phys. J. A (2021) 57:336)
...
/// @param Gamma2: width of the decay channel 2, highes thresholds (e.g. ΛK- for Ξ(1620)), typically the pair you are
/// measuring
...
double GeneralLednickySill_twochannels(const double &Momentum, const double &GaussR, const double &MassR,
                                       const double &Gamma1, const double &Gamma2, const double &m11, const double &m12,
                                       const double &m21, const double &m22) {

    if (GaussR != GaussR) {
        printf("\033[1;33mWARNING:\033[0m GeneralLednickySill got a bad value for the Radius (nan). Returning default "
               "value of 1.\n");
        return 1;
    }
    ...
    complex<double> ScattAmpl =
        (-2 * Gamma2) *
        (pow(En * En - MassR * MassR + i * Gamma1 * sqrt(En * En - En1 * En1) + i * Gamma2 * sqrt(En * En - En2 * En2),
             -1.));

    /// correction for small sources, involving
    double DeltaC =
        pow(abs(ScattAmpl), 2) * (2. + m22 / m12 + m12 / m22) / (2. * sqrt(Pi) * Radius * Radius * Radius * 2 * Gamma2);

    double CkValue = 0.;
    CkValue += 0.5 * pow(abs(ScattAmpl) / Radius, 2) + 2 * real(ScattAmpl) * F1 / (sqrt(Pi) * Radius) -
               imag(ScattAmpl) * F2 / Radius + DeltaC;
...
double LednickySilltwochannels_doublegaussian_lambda(const double &Momentum, const double *SourcePar,
                                                     const double *PotPar) {
    ...
    double m21 = PotPar[5];
    double m22 = PotPar[6];
    return (SourcePar[3] * (SourcePar[2] * GeneralLednickySill_twochannels(Momentum, SourcePar[0], MassR, Gamma1,
                                                                           Gamma2, m11, m12, m21, m22) +
                            (1 - SourcePar[2]) * GeneralLednickySill_twochannels(Momentum, SourcePar[1], MassR, Gamma1,
                                                                                 Gamma2, m11, m12, m21, m22)) +
            1. - SourcePar[3]);
}
...
double GeneralLednickySillConstraints_twochannels(const double &Momentum, const double &GaussR, const double &MassR,
                                                  const double &GammaTilde1, const double &GammaTot, const double &m11,
                                                  const double &m12, const double &m21, const double &m22) {
    ...
    double GammaTilde2 = GammaTot / (sqrt(1. - alpha2 * alpha2)) -
                         GammaTilde1 * sqrt(1. - alpha1 * alpha1) / (sqrt(1. - alpha2 * alpha2));

    complex<double> ScattAmpl =
        (-2 * GammaTilde2) * (pow(En * En - MassR * MassR + i * GammaTilde1 * sqrt(En * En - En1 * En1) +
                                      i * GammaTilde2 * sqrt(En * En - En2 * En2),
                                  -1.));

    /// correction for small sources, involving
    double DeltaC = pow(abs(ScattAmpl), 2) * (2. + m22 / m12 + m12 / m22) /
                    (2. * sqrt(Pi) * Radius * Radius * Radius * 2 * GammaTilde2);
...
double LednickySilltwochannelsConstraints_doublegaussian_lambda(const double &Momentum, const double *SourcePar,
                                                                const double *PotPar) {
    double MassR = PotPar[0];
    ...
    double m22 = PotPar[6];
    return (SourcePar[3] *
                (SourcePar[2] * GeneralLednickySillConstraints_twochannels(Momentum, SourcePar[0], MassR, GammaTilde1,
                                                                           GammaTot, m11, m12, m21, m22) +
                 (1 - SourcePar[2]) * GeneralLednickySillConstraints_twochannels(
                                          Momentum, SourcePar[1], MassR, GammaTilde1, GammaTot, m11, m12, m21, m22)) +
            1. - SourcePar[3]);
}
// ###############################################
/// @brief  Lednicky-Lyuboshits formula with Flatte-like scattering amplitude for the decay into the pair we are
/// measuring (assumed channel 2) based on the Sill distribution (F. Giacosa, Eur. Phys. J. A (2021) 57:336), no
/// constraints + LL formula with ere
double LednickySilltwochannelsERE_doublegaussian_lambda(const double &Momentum, const double *SourcePar,
                                                        const double *PotPar) {
    ...
    /// d0   [2]
    complex<double> ScatLen(PotPar[0], PotPar[1]);
    double cfemto = SourcePar[3] * (SourcePar[2] * GeneralLednicky(Momentum, SourcePar[0], ScatLen, PotPar[2], 0, 0,
                                                                   true, false, false) +
                                    (1 - SourcePar[2]) * GeneralLednicky(Momentum, SourcePar[1], ScatLen, PotPar[2], 0,
                                                                         0, true, false, false)) +
                    1. - SourcePar[3];
    ...
    double csill =
        (SourcePar[3] *
             (SourcePar[2] * GeneralLednickySill_twochannels(Momentum, SourcePar[0], MassR, GammaTilde1, GammaTilde2,
                                                             m11, m12, m21, m22) +
              (1 - SourcePar[2]) * GeneralLednickySill_twochannels(Momentum, SourcePar[1], MassR, GammaTilde1,
                                                                   GammaTilde2, m11, m12, m21, m22)) +
         1. - SourcePar[3]);
