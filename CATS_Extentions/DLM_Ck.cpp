...
DLM_Ck::DLM_Ck(const DLM_Ck &other, bool FULL)
    : DLM_Histo(other), NumSourcePar(other.NumSourcePar), NumPotPar(other.NumPotPar) {
    if (FULL) {
        Kitty = other.Kitty;
...
DLM_Ck::DLM_Ck(const unsigned numbins, const double &kmin, const double &kmax)
    : DLM_Histo(), NumSourcePar(0), NumPotPar(0) {
    Kitty = NULL;
    CkFunction = NULL;
    SetUp(1);
...
DLM_Ck::DLM_Ck(const unsigned &nSourcePar, const unsigned &nPotPar, CATS &cat)
    : DLM_Histo(), NumSourcePar(nSourcePar), NumPotPar(nPotPar) {
    SetUp(1);
    for (unsigned uBin = 1; uBin <= cat.GetNumMomBins(); uBin++) {
        if (cat.GetMomBinLowEdge(uBin) <= cat.GetMomBinLowEdge(uBin - 1)) {
            printf("\033[1;31mERROR:\033[0m DLM_Ck: The bin ranges should be in ascending order and a bin-width of 0 "
                   "is not allowed!\n");
            return;
        }
    }
    if (!BinRange[0])
        BinRange[0] = new double[cat.GetNumMomBins() + 1];
    if (!BinCenter[0])
        BinCenter[0] = new double[cat.GetNumMomBins()];
...
DLM_Ck::DLM_Ck(const unsigned &nSourcePar, const unsigned &nPotPar, CATS &cat, const unsigned &numbin,
               const double *bins)
    : DLM_Histo(), NumSourcePar(nSourcePar), NumPotPar(nPotPar) {
    SetUp(1);
    for (unsigned uBin = 1; uBin <= cat.GetNumMomBins(); uBin++) {
...
DLM_Ck::DLM_Ck(const unsigned &nSourcePar, const unsigned &nPotPar, CATS &cat, const unsigned &numbin,
               const double &minMom, const double &maxMom)
    : DLM_Histo(), NumSourcePar(nSourcePar), NumPotPar(nPotPar) {
    SetUp(1);
    for (unsigned uBin = 1; uBin <= cat.GetNumMomBins(); uBin++) {
        if (cat.GetMomBinLowEdge(uBin) <= cat.GetMomBinLowEdge(uBin - 1)) {
...
DLM_Ck::DLM_Ck(const unsigned &nSourcePar, const unsigned &nPotPar, const unsigned &numbin, const double *bins,
               double (*CorrFun)(const double &, const double *, const double *))
    : DLM_Histo(), NumSourcePar(nSourcePar), NumPotPar(nPotPar), CkFunction(CorrFun) {
    Kitty = NULL;
    SetUp(1);
    SetUp(0, numbin, bins);
    Initialize();
...
DLM_Ck::DLM_Ck(const unsigned &nSourcePar, const unsigned &nPotPar, const unsigned &numbin, const double &minMom,
               const double &maxMom, double (*CorrFun)(const double &, const double *, const double *))
    : DLM_Histo(), NumSourcePar(nSourcePar), NumPotPar(nPotPar), CkFunction(CorrFun) {
    Kitty = NULL;
    SetUp(1);
    SetUp(0, numbin, minMom, maxMom);
...
void DLM_Ck::DefaultConstructor() {
    ...
    // if(CkFunction){
    if (NumSourcePar)
        SourcePar = new double[NumSourcePar];
    if (NumPotPar)
        PotPar = new double[NumPotPar];
    //}
...
void DLM_Ck::SetSourcePar(const unsigned &WhichPar, const double &Value) {
    if (WhichPar >= NumSourcePar)
        return;
    if (CkFunction) {
        if (SourcePar[WhichPar] == Value)
            return;
        SourcePar[WhichPar] = Value;
        SourceUpToDate = false;
    } else if (Kitty && Kitty->GetUseAnalyticSource()) {
        if (Kitty->GetAnaSourcePar(WhichPar) == Value)
            return;
        Kitty->SetAnaSource(WhichPar, Value, true);
        SourceUpToDate = false;
    } else if (Kitty && !Kitty->GetUseAnalyticSource()) {
        if (Kitty->GetTransportRenorm() == Value)
            return;
...
unsigned DLM_Ck::GetNumSourcePar() { return NumSourcePar; }

double DLM_Ck::GetPotPar(const unsigned &WhichPar) {
    if (WhichPar >= NumPotPar)
        return 0;
    return PotPar[WhichPar];
}
unsigned DLM_Ck::GetNumPotPar() { return NumPotPar; }
void DLM_Ck::SetCutOff(const double &Momentum, const double &kc) {
    if (Momentum < GetBinUpEdge(0, NumBins[0] - 1))
        CutOff = Momentum;
    else
        CutOff = GetBinUpEdge(0, NumBins[0] - 1);
    CutOff_kc = kc;
}
double DLM_Ck::GetCutOff() const { return CutOff; }
double DLM_Ck::GetCutOff_kc() const { return CutOff_kc; }
CATS *DLM_Ck::GetTheCat() const { return Kitty; }

void DLM_Ck::SetPotPar(const unsigned &WhichPar, const double &Value) {
    if (WhichPar >= NumPotPar)
        return;
    if (CkFunction) {
        if (PotPar[WhichPar] == Value)
            return;
...
void DLM_Ck::Update(const bool &FORCE) {
    ...
    if (CkFunction) {
        ...
        for (unsigned uBin = 0; uBin < NumBins[0]; uBin++) {
            if (Kitty->GetMomBinUpEdge(Kitty->GetNumMomBins() - 1) > GetBinCenter(0, uBin) &&
                Kitty->GetMomBinLowEdge(0) < GetBinCenter(0, uBin) && CutOff > GetBinCenter(0, uBin)) {
                ...
                // printf("Kitty->GetMomBinLowEdge(0)>=GetBinCenter(0,uBin) :
                // %f>=%f",Kitty->GetMomBinLowEdge(0),GetBinCenter(0,uBin));
                BinValue[uBin] = 0;
            }
    ...
    for (unsigned uBin = 0; uBin < NumBins[0]; uBin++) {
        ...
        if (Cf < 1 && BinVal > 1) {
            BinVal = 1;
            printf("\033[1;33mWARNING:\033[0m DLM_Ck::Update got an error called Cf<1. Please share with the "
                   "developers!\n");
        }
        if (Cf > 1 && BinVal < 1) {
            BinVal = 1;
...
double DLM_Ck::Eval(const double &Momentum) {
    ...
    else if (Momentum < CutOff && Momentum < BinRange[0][NumBins[0]])
