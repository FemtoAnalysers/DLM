...
class DLM_CkDecomp {
  ...
  public:
    ...
    DLM_CkDecomp(const char *name, const unsigned &numchildren, DLM_Ck &ckfunction,
                 const DLM_Histo<float> *hSigmaMatrix, const bool &InvertAxis = false);
  ...
  protected:
    ...
    // N.B. I assume that the MomResolution is only taken into account if you look into that particular C(k) or the
    // feed-down contributions. For the children which are of type fake, the Sigma matrix is taken from their
    // definition.
    ...
    // void SmearOLD(const DLM_Histo<double>* CkToSmear, const DLM_ResponseMatrix* SmearMatrix, DLM_Histo<double>*
    // CkSmeared);
    void Smear(const DLM_Histo<double> *CkToSmear, const DLM_ResponseMatrix *SmearMatrix, DLM_Histo<double> *CkSmeared,
               DLM_Histo<float> *PhaseSpace = NULL);
};
