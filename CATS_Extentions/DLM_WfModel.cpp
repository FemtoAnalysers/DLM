...
// The two strings should have a dsscription with separation "_", where we start with version (NLO, N2LO), second number
// is cut off, third value is scattering parameter is s wave, 4th par is which waves we take we may even add more stuff
// at the back if we want to in the future e.g. NLO19_600_291_sd would be the NLO19 at cutoff 600 MeV, with f0 = 2.91 fm
...
DLM_Histo<complex<double>> ***Init_pp_Haidenbauer(const char *CatsFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
            ...
            if ((CurrentLine) % int(NumRadBins) == 0) {
                ...
                if (NumMomBins[uFile]) {
                    ...
                    MomentumBins[uFile][NumMomBins[uFile]] =
                        0.5 * (Momentum[uFile][NumMomBins[uFile]] + Momentum[uFile][NumMomBins[uFile] - 1]);
                }
                NumMomBins[uFile]++;
            }
            CurrentLine++;
        ...
        MomentumBins[uFile][NumMomBins[uFile]] =
            2. * Momentum[uFile][NumMomBins[uFile] - 1] - MomentumBins[uFile][NumMomBins[uFile] - 1];
    }
...
void Init_pp_Epelbaum(const char *CatsFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (short unsigned usCh = 0; usCh < NumChannels; usCh++) {
        for (short unsigned usPw = 0; usPw < Kitty.GetNumPW(usCh); usPw++) {
            if (usCh == 0 && usPw % 2 == 1)
                continue;
            if ((usCh >= 1 && usCh <= 9) && usPw % 2 == 0)
                continue;
            char *wf_file_name = new char[256];
            sprintf(wf_file_name, "%s/Interaction/pp/Epelbaum_2025-06/wf_epel_usCh%u_usPw%u.dlm", CatsFolder, usCh,
                    usPw);
            DLM_Histo<complex<double>> wf_histo;
            wf_histo.QuickLoad(wf_file_name);
            char *ps_file_name = new char[256];
            sprintf(ps_file_name, "%s/Interaction/pp/Epelbaum_2025-06/ps_epel_usCh%u_usPw%u.dlm", CatsFolder, usCh,
                    usPw);
...
void Init_pp_Norfolk(const char *CatsFolder, CATS &Kitty, const int &TYPE) {
    ...
    if (TYPE == 112) {
        ...
        // printf("\033[1;33mWARNING:\033[0m The Norfolk potential TYPE\033[0m %s does not exist! Reverting to default
        // type NV2-IIA_EM2\n", TYPE);
        strcpy(norfolk_type, "NV2-IIA_EM2");
    }
    ...
    for (short unsigned usCh = 0; usCh < NumChannels; usCh++) {
        for (short unsigned usPw = 0; usPw < Kitty.GetNumPW(usCh); usPw++) {
            ...
            if ((usCh >= 1 && usCh <= 9) && usPw % 2 == 0)
                continue;
            char *wf_file_name = new char[256];
            sprintf(wf_file_name, "%s/Interaction/pp/Norfolk_2025-06/%s/wf_norfolk_usCh%u_usPw%u.dlm", CatsFolder,
                    norfolk_type, usCh, usPw);
            DLM_Histo<complex<double>> wf_histo;
            wf_histo.QuickLoad(wf_file_name);
            char *ps_file_name = new char[256];
            sprintf(ps_file_name, "%s/Interaction/pp/Norfolk_2025-06/%s/ps_norfolk_usCh%u_usPw%u.dlm", CatsFolder,
                    norfolk_type, usCh, usPw);
            DLM_Histo<complex<double>> ps_histo;
            ps_histo.QuickLoad(ps_file_name);
    ...
    for (short unsigned usCh = 0; usCh < NumChannels; usCh++) {
        for (short unsigned usPw = 0; usPw < Kitty.GetNumPW(usCh); usPw++) {
            ...
            if ((usCh >= 1 && usCh <= 9) && usPw % 2 == 0)
                continue;
            char *wf_file_name = new char[256];
            sprintf(wf_file_name, "%s/Interaction/pp/AV18_2025-06/%s/wf_av18_usCh%u_usPw%u.dlm", CatsFolder, av18_type,
                    usCh, usPw);
            DLM_Histo<complex<double>> wf_histo;
            wf_histo.QuickLoad(wf_file_name);
            char *ps_file_name = new char[256];
            sprintf(ps_file_name, "%s/Interaction/pp/AV18_2025-06/%s/ps_av18_usCh%u_usPw%u.dlm", CatsFolder, av18_type,
                    usCh, usPw);
            DLM_Histo<complex<double>> ps_histo;
            ps_histo.QuickLoad(ps_file_name);
...
DLM_Histo<complex<double>> ***Init_pp_Epelbaum_OLD(const char *CatsFolder, CATS &Kitty, const int &TYPE) {
    ...
    // setup file reading
    enum EpelFiles { f1S0, f3P0, f3P1, f3P2, f1D2, f3F2, f3F3, f3P2_3F2, f3F2_3P2, fNFiles };
    // hard code name of the input files in the directory passed to this function
    std::string path(CatsFolder);
    std::array<std::string, fNFiles> Dirs{"1S0", "3P0", "3P1", "3P2", "1D2", "3F2", "3F3", "3P2-3F2", "3F2-3P2"};
    ...
    TH1F *HistMomentum = static_cast<TH1F *>(Input->Get("1S0/wave_vs_distance/hist_wf_real_vs_r_p5"));
    TH1F *HistRadius = static_cast<TH1F *>(Input->Get("1S0/wave_vs_momentum/hist_wf_real_vs_p_r0.000"));
    ...
    for (int i = 0; i < fNFiles; i++) {
        ...
        for (int j = 0; j < MomentumBinEdges.size(); j++) {
            ...
            TString RName = Form("%s/wave_vs_distance/hist_wf_real_vs_r_p%d", Dirs[i].c_str(), bin);
            TH1F *Real = static_cast<TH1F *>(Input->Get(RName));
            TString IName = Form("%s/wave_vs_distance/hist_wf_imag_vs_r_p%d", Dirs[i].c_str(), bin);
    ...
    for (unsigned uHist = 0; uHist < NumDLM_Histos; uHist++) {
        ...
        for (unsigned uPw = 0; uPw < Kitty.GetNumPW(uHist); uPw++) {
            ...
            HistoWF[uHist][uPw].SetUp(2);
            HistoWF[uHist][uPw].SetUp(0, MomentumBinEdges.size() - 1, MomentumBinEdges.data());
            HistoWF[uHist][uPw].SetUp(1, RadiusBinEdges.size() - 1, RadiusBinEdges.data());
            HistoWF[uHist][uPw].Initialize();
        }
    }
    HistoPS = new DLM_Histo<complex<double>> *[NumDLM_Histos];
    for (unsigned uHist = 0; uHist < NumDLM_Histos; uHist++) {
        ...
        for (unsigned uPw = 0; uPw < Kitty.GetNumPW(uHist); uPw++) {
            ...
            HistoPS[uHist][uPw].SetUp(1);
            HistoPS[uHist][uPw].SetUp(0, MomentumBinEdges.size() - 1, MomentumBinEdges.data());
            HistoPS[uHist][uPw].Initialize();
        }
    }
    ...
    for (unsigned uHist = 0; uHist < NumDLM_Histos; uHist++) {
        for (unsigned uPw = 0; uPw < Kitty.GetNumPW(uHist); uPw++) {
            ...
            for (size_t momBin = 0; momBin < MomentumBinEdges.size() - 1; momBin++) {
                for (size_t radiusBin = 0; radiusBin < RadiusBinEdges.size() - 1; radiusBin++) {
                    WhichBin[0] = momBin;
                    WhichBin[1] = radiusBin;
                    HistoWF[uHist][uPw].SetBinContent(
                        WhichBin, WaveFunctionAll.at(WhichFile[uHist][uPw]).at(momBin).at(radiusBin));
                }
                HistoPS[uHist][uPw].SetBinContent(momBin, PhaseShiftAll.at(WhichFile[uHist][uPw]).at(momBin));
            }
        }
    }
...
// ch2 and ch3 are the SN->LN in the S-wave (1S0 and 3S1), however we ONLY add to the total correlation function the
// s-wave to obtain the total correlation we need to add 1/4*ch0+3/4*ch1+1/4*ch2+3/4*ch3. As the ch2 and ch3 are just
// "corrections" to the primary channels (0 and 1) it is completely fine that our weights do not sum up to 1
DLM_Histo<complex<double>> ***Init_pL_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE,
                                                  const int &CUTOFF) {
    double RadiusStepSize;
    double RadiusMinimum;
    double RadiusMaximum;
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (TYPE % 10 == 0 || TYPE == 12) {
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f", &fRadius, &fMomentum, &fReWf, &fImWf, &fReAsWf, &fImAsWf,
                       &fCatsWf, &fDummy, &fPhaseShift);
            } else if (TYPE % 10 == 1) {
                if (uFile == 0) {
                    sscanf(cdummy, " %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_LNLN_SS, &fImWf_LNLN_SS,
                           &fReWf_SNLN_SS, &fImWf_SNLN_SS);
                    fReWf_LNLN_DS = 0;
                    fImWf_LNLN_DS = 0;
                    fReWf_SNLN_DS = 0;
                    fImWf_SNLN_DS = 0;
                } else if (uFile == 1) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_LNLN_SS, &fImWf_LNLN_SS,
                           &fReWf_SNLN_SS, &fImWf_SNLN_SS, &fReWf_LNLN_DS, &fImWf_LNLN_DS, &fReWf_SNLN_DS,
                           &fImWf_SNLN_DS);
                } else {
...
DLM_Histo<complex<double>> ***Init_pL_Haidenbauer2019(const char *InputFolder, CATS &Kitty, const int &TYPE,
                                                      const int &CUTOFF) {
    double RadiusStepSize;
    double RadiusMinimum;
    double RadiusMaximum;
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (uFile == f1S0) {
                ...
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf1, &fImWf1, &fReWf2, &fImWf2,
                       &fReWf3, &fImWf3, &fReWf4, &fImWf4);
                // 1: 3S1+3P0+3D1
...
// at the back if we want to in the future e.g. NLO19_600_291_sd would be the NLO19 at cutoff 600 MeV, with f0 = 2.91 fm
// and we consider only s and d waves
DLM_Histo<complex<double>> ***Init_pL_Haidenbauer2023(const char *CatsInputFolder, CATS &Kitty, const char *Singlet,
                                                      const char *Triplet) {
    double RadiusStepSize;
    double RadiusMinimum;
    double RadiusMaximum;
        ...
        if (CutOff_1S0 == 600) {
            ...
            if (ApproxEqualF(f1S0_input, 2.91)) {
                ...
                if (CalcVersion1S0 == su3b) {
                    ...
                    strcat(InputFileName[f1S0], "NLO19_1s0_2p10.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p10 does not have a default version, using su3b "
                           "instead!\n");
                }
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p20 does not have su3b version, using the default "
                           "instead!\n");
                }
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p30 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 2.45)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p45 does not have su3b version, using the default "
                           "instead!\n");
                }
                ...
                if (CalcVersion1S0 == su3b) {
                    ...
                    strcat(InputFileName[f1S0], "NLO19_1s0_2p50.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p50 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 2.53)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p53 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 2.55)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p55 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 2.65)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p65 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 2.80)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_2p80 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 3.00)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_3p00 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 3.10)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_3p10 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 3.20)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_3p20 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 3.30)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion1S0 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_3p30 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f1S0_input, 3.34)) {
                strcat(InputFileName[f1S0], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                if (CalcVersion1S0 == su3b) {
                    ...
                    strcat(InputFileName[f1S0], "NLO19_1s0_3p34.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_1s0_3p34 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else {
        ...
        if (CutOff_3S1 == 600) {
            ...
            if (ApproxEqualF(f3S1_input, 1.41)) {
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_0p90 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 0.95)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_0p95 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.00)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p00 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.05)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p05 does not have su3b version, using the default "
                           "instead!\n");
                }
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p10 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.15)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p15 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.18)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                if (CalcVersion3S1 == su3b) {
                    ...
                    strcat(InputFileName[f3S1], "NLO19_3s1_1p18.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p18 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.20)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p20 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.25)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p25 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.30)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p30 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.32)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p35 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.44)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                if (CalcVersion3S1 == su3b) {
                    ...
                    strcat(InputFileName[f3S1], "NLO19_3s1_1p44.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p44 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.46)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                if (CalcVersion3S1 == su3b) {
                    ...
                    strcat(InputFileName[f3S1], "NLO19_3s1_1p46.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p46 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.56)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                if (CalcVersion3S1 == su3b) {
                    ...
                    strcat(InputFileName[f3S1], "NLO19_3s1_1p56.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p56 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.60)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                ...
                if (CalcVersion3S1 == su3b) {
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p60 does not have su3b version, using the default "
                           "instead!\n");
                }
            } else if (ApproxEqualF(f3S1_input, 1.66)) {
                strcat(InputFileName[f3S1], "pLambda_Coupled_SD/Chiral19_FineTunes/");
                if (CalcVersion3S1 == su3b) {
                    ...
                    strcat(InputFileName[f3S1], "NLO19_3s1_1p66.su3b");
                    printf("\033[1;33mWARNING:\033[0m NLO19_3s1_1p66 does not have a default version, using su3b "
                           "instead!\n");
                }
            } else {
                printf("Something very bad has happend in Init_pL_Haidenbauer2023\n");
...
DLM_Histo<complex<double>> ***Init_pL_Haidenbauer2023(const char *InputFolder, CATS *Kitty, const char *Singlet,
                                                      const char *Triplet) {
    return Init_pL_Haidenbauer2023(InputFolder, *Kitty, Singlet, Triplet);
}

// WFs obtained through private communication with the authors of
// https://arxiv.org/pdf/2201.08238 (https://pos.sissa.it/413/085)
// J. Haidenbauer and U.G. Meißner
// TYPE and CUTOFF are for the moment dummies
DLM_Histo<complex<double>> ***Init_pXi_Haidenbauer2025(const char *InputFolder, CATS &Kitty, const int &TYPE,
                                                       const int &CUTOFF) {
    double RadiusStepSize;
    double RadiusMinimum;
    double RadiusMaximum;
    ...
    Kitty.SetRedMass((Mass_Xim * Mass_p) / (Mass_Xim + Mass_p));
    ...
    for (unsigned short usCh = 0; usCh < NumChannels; usCh++) {
        if (usCh == 1)
            Kitty.SetNumPW(usCh, 3);
        else
            Kitty.SetNumPW(usCh, 1);

        if (usCh == 1 || usCh >= 5) {
            Kitty.SetSpin(usCh, 1);
            Kitty.SetChannelWeight(usCh, 0.75);
        } else {
            Kitty.SetSpin(usCh, 0);
            Kitty.SetChannelWeight(usCh, 0.25);
        }
        if (usCh >= 2) {
            Kitty.SetOnlyNumericalPw(usCh, true);
        }
    ...
    strcat(InputFileName[f1S0], "M060CS.data");
    strcat(InputFileName[f3S1], "M160CS.data");
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (uFile == 0) // 1S0
            {
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_LL, &fImWf_LL, &fReWf_X0n,
                       &fImWf_X0n, &fReWf_Xp, &fImWf_Xp, &fReWf_LS, &fImWf_LS);

            } else if (uFile == 1) // 3S1
            {
                sscanf(cdummy, "  %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_LL,
                       &fImWf_LL, &fReWf_X0n, &fImWf_X0n, &fReWf_Xp, &fImWf_Xp, &fReWf_LS, &fImWf_LS, &fReWf_d_X0n,
                       &fImWf_d_X0n, &fReWf_d_Xp, &fImWf_d_Xp, &fReWf_d_LS, &fImWf_d_LS);
            } else {
                printf("Oh man... something wrong in scanning files in the setup of Haidenbauer pXi WFs from 2025.\n");
            }
            ...
            } else if (uFile == 1) // 3S1
            {
                ...
                printf("Oh man... something wrong in reading files in the setup of Haidenbauer pXi WFs from 2025.\n");
            }
...
DLM_Histo<complex<double>> ***Init_Lcp_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE,
                                                   const int &CUTOFF) {
    double RadiusStepSize;
    double RadiusMinimum;
    double RadiusMaximum;
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (TYPE == 0) {
                if (uFile == 0) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_1s0, &fImWf_1s0, &fDummy1,
                           &fDummy2, &fDummy3, &fDummy4);
                    // fDummy1 =0;
                    // fDummy2 = 0;
                    // fDummy3 = 0;
                    // fDummy4 = 0;
                } else if (uFile == 1) {
                    sscanf(cdummy, "  %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_3s1, &fImWf_3s1, &fDummy1,
                           &fDummy2, &fDummy3, &fDummy4);
                    // fDummy1 = 0;
                    // fDummy2 = 0;
                    // fDummy3 = 0;
                    ...
                    sscanf(cdummy, "  %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fDummy1, &fDummy2, &fReWf_Scp_1s0,
                           &fImWf_Scp_1s0, &fReWf_Scpp_1s0, &fImWf_Scpp_1s0);
                    // fDummy1 = 0;
                    // fDummy2 = 0;
                    // fDummy3 = 0;
                    // fDummy4 = 0;
                } else if (uFile == 3) {
                    sscanf(cdummy, "  %f %f %f %f %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fDummy1, &fDummy2,
                           &fReWf_Scp_3d13s1, &fImWf_Scp_3d13s1, &fReWf_Scp_3s1, &fImWf_Scp_3s1, &fReWf_Scpp_3d13s1,
                           &fImWf_Scpp_3d13s1, &fReWf_Scpp_3s1, &fImWf_Scpp_3s1);
                    // fDummy1 = 0;
                    // fDummy2 = 0;
                    // fDummy3 = 0;
                    // fDummy4 = 0;
            ...
            if (TYPE == 0) {
                ...
                if (uFile == 0) {
                    ...
                    HistoWF[2][0].SetBinContent(WhichBin,
                                                ClebschGordan_Scp * (fReWf_Scp_1s0 + fi * fImWf_Scp_1s0) * fRadius);
                    HistoPS[2][0].SetBinContent(WhichBin, 0.);

                    HistoWF[4][0].SetBinContent(WhichBin,
                                                ClebschGordan_Scpp * (fReWf_Scpp_1s0 + fi * fImWf_Scpp_1s0) * fRadius);
                    HistoPS[4][0].SetBinContent(WhichBin, 0.);
                } else if (uFile == 3) {
                    /// Ch.[3] is 3S1-3S1 Sc+p->Lcp
                    /// Ch.[5] is 3S1-3S1 Sc++n->Lcp
                    HistoWF[3][0].SetBinContent(WhichBin,
                                                ClebschGordan_Scp * (fReWf_Scp_3s1 + fi * fImWf_Scp_3s1) * fRadius);
                    HistoPS[3][0].SetBinContent(WhichBin, 0.);

                    HistoWF[5][0].SetBinContent(WhichBin,
                                                ClebschGordan_Scpp * (fReWf_Scpp_3s1 + fi * fImWf_Scpp_3s1) * fRadius);
                    HistoPS[5][0].SetBinContent(WhichBin, 0.);

                    /// Ch.[6] is 3D1-3S1 Sc+p->Lcp
                    /// Ch.[7] is 3D1-3S1 Sc++n->Lcp
                    HistoWF[6][0].SetBinContent(WhichBin, ClebschGordan_Scp *
                                                              (fReWf_Scp_3d13s1 + fi * fImWf_Scp_3d13s1) * fRadius);
                    HistoPS[6][0].SetBinContent(WhichBin, 0.);

                    HistoWF[7][0].SetBinContent(WhichBin, ClebschGordan_Scpp *
                                                              (fReWf_Scpp_3d13s1 + fi * fImWf_Scpp_3d13s1) * fRadius);
                    HistoPS[7][0].SetBinContent(WhichBin, 0.);
                } else {
...
DLM_Histo<complex<double>> ***Init_pSigmaPlus_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (uFile == f1S0) {
                sscanf(cdummy, " %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf1, &fImWf1, &fDummy, &fDummy,
                       &fDummy, &fDummy);
                HistoWF[0][0].SetBinContent(WhichBin, (fReWf1 + fi * fImWf1) * fRadius);
...
DLM_Histo<complex<double>> ***Init_pSigma0_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (uFile == f1S0) {
                sscanf(cdummy, " %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_Lp_S0p, &fImWf_Lp_S0p,
                       &fReWf_S0p_S0p, &fImWf_S0p_S0p, &fReWf_Sn_S0p, &fImWf_Sn_S0p);
            } else if (uFile == f3S1) {
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_Lp_S0p,
                       &fImWf_Lp_S0p, &fReWf_S0p_S0p, &fImWf_S0p_S0p, &fReWf_Sn_S0p, &fImWf_Sn_S0p, &fDummy, &fDummy,
...
// TYPE = 0 (no options)
DLM_Histo<complex<double>> ***Init_pXi_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE,
                                                   const int &CUTOFF) {
    double RadiusStepSize;
    double RadiusMinimum;
    double RadiusMaximum;
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (TYPE == 0) {
                if (uFile == fI0S0) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &fRadius, &fMomentum, &fDummy, &fDummy, &fReWf,
                           &fImWf, &fDummy, &fDummy, &fDummy, &fDummy);
                } else if (uFile == fI0S1) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f", &fRadius, &fMomentum, &fDummy, &fDummy, &fReWf, &fImWf,
                           &fDummy, &fDummy);
                } else if (uFile == fI1S0) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &fRadius, &fMomentum, &fDummy, &fDummy, &fDummy,
                           &fDummy, &fReWf, &fImWf, &fDummy, &fDummy);
                } else {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f", &fRadius, &fMomentum, &fDummy, &fDummy, &fDummy, &fDummy,
                           &fReWf, &fImWf);
                }
            } else {
...
DLM_Histo<complex<double>> ***Init_pXi_ESC16_IS(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uBin = 0; uBin < NumTomMomBins; uBin++) {
        TomMomentum[uBin] =
            sqrt(pow(Mass_p * pLab[uBin], 2) /
                 (pow(Mass_p, 2) + pow(Mass_Xim, 2) + 2 * Mass_p * sqrt(pow(Mass_Xim, 2) + pow(pLab[uBin], 2))));
    }
    TomMomentumBins[0] = 0;
    ...
    TomMomentumBins[NumTomMomBins] =
        TomMomentum[NumTomMomBins - 1] + (TomMomentum[NumTomMomBins - 1] - TomMomentumBins[NumTomMomBins - 1]);
        ...
        while (!feof(InFile)) {
            ...
            for (unsigned uRow = 0; uRow < NumRow; uRow++) {
                ...
                if (uRow == NumRow - 1 && NumFuckedUpEntriesAtTheEnd == 1) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f %f", &Values[0], &Values[1], &Values[2], &Values[3],
                           &Values[4], &Values[5], &Values[6], &Values[7], &Values[8]);
                } else if (NumFuckedUpEntriesAtTheEnd) {
                    sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &Values[0], &Values[1], &Values[2], &Values[3],
                ...
                for (unsigned uCol = 0; uCol < NumCol; uCol++) {
                    ...
                    // printf("fRadius=%f\n",fRadius);
                    HistoWF[WhichIsospin * NumSpinChannels + WhichSpin]->SetBinContent(WhichBin,
                                                                                       Values[uCol] * fRadius);
                }
            }
...
DLM_Histo<complex<double>> ***Init_pS0_ESC08(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uBin = 0; uBin < NumTomMomBins; uBin++) {
        TomMomentum[uBin] =
            sqrt(pow(Mass_p * pLab[uBin], 2) /
                 (pow(Mass_p, 2) + pow(Mass_S0, 2) + 2 * Mass_p * sqrt(pow(Mass_S0, 2) + pow(pLab[uBin], 2))));
    }
    TomMomentumBins[0] = 0;
    for (unsigned short uBin = 0; uBin < NumTomMomBins - 1; uBin++) {
        TomMomentumBins[uBin + 1] = (TomMomentum[uBin] + TomMomentum[uBin + 1]) * 0.5;
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            if (!fgets(cdummy, 511, InFile))
                continue;
            if (uFile == 0)
                sscanf(cdummy, "%s %f %s %s %f %s %f %s", buffer, &pLab, buffer, buffer, &PhaseShiftRe, buffer,
                       &PhaseShiftIm, buffer);
            else {
                // imagine: the last line has different format due to a space missing
                if (WhichBin[0] == NumTomMomBins - 1) {
                    sscanf(cdummy, "%s %s %s %s %s %f %s %f %s %f", buffer, buffer, buffer, buffer, buffer,
                           &PhaseShiftRe, buffer, &PhaseShiftIm, buffer, &PhaseShiftIm);
                } else {
                    sscanf(cdummy, "%s %s %s %f %s %s %f %s %f %s %f", buffer, buffer, buffer, &pLab, buffer, buffer,
                           &PhaseShiftRe, buffer, &PhaseShiftIm, buffer, &PhaseShiftIm);
                }
...
DLM_Histo<complex<double>> ***Init_pantip_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            if (TYPE == 0) {
                if (uFile == 0) { // 1S0
                    sscanf(cdummy, " %f %f %f %f %f %f", &fRadius, &fDummy, &fReWf_ppbar, &fImWf_ppbar, &fReWf_nnbar,
                           &fImWf_nnbar);
...
DLM_Histo<complex<double>> ***Init_pKminus_Kyoto2019(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f %f %f %f %f", &fMomentum, &fRadius, &fReWf_Kp, &fImWf_Kp,
                   &fReWf_Kn, &fImWf_Kn, &fReWf_pipSm, &fImWf_pipSm, &fReWf_pi0S0, &fImWf_pi0S0, &fReWf_pimSp,
...
DLM_Histo<complex<double>> ***Init_pK0_Kyoto2019(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            // printf("READING THE NEXT LINE ---> \n");
            if (TYPE < 10)
                sscanf(cdummy, " %f %f %f %f %f %f", &fMomentum, &fRadius, &fReWf_Kp, &fImWf_Kp, &fReWf_Kn, &fImWf_Kn);
            else
                sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f", &fMomentum, &fRadius, &fReWf_Kp, &fImWf_Kp,
                       &fReWf_pipS0, &fImWf_pipS0, &fReWf_pi0Sp, &fImWf_pi0Sp, &fReWf_pipL, &fImWf_pipL);
...
DLM_Histo<complex<double>> ***InitHaidenbauerKaonPlus(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            // printf("READING THE NEXT LINE ---> \n");
            sscanf(cdummy, " %f %f %f %f %f %f %f %f %f", &fRadius, &fMomentum, &fReWf, &fImWf, &fReWfAs, &fImWfAs,
                   &fReWfRot, &fImWfRot, &fPhaseShift);
...
DLM_Histo<complex<double>> ***Init_pd_Sebastian(const char *InputFolder, CATS &Kitty, const int &TYPE,
                                                const int &CUTOFF) {
...
DLM_Histo<complex<double>> ***Init_pDminus_Haidenbauer(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    while (!feof(InFile)) {
        ...
        sscanf(cdummy, " %f %f %f %f %f %f", &fRadius, &fMom, &fReWf_pDm_pDm, &fImWf_pDm_pDm, &fReWf_nDo_pDm,
               &fImWf_nDo_pDm);
        ...
        HistoWF[0][0].SetBinContent(WhichBin, (fReWf_pDm_pDm + fi * fImWf_pDm_pDm) * fRadius);
        HistoWF[1][0].SetBinContent(WhichBin, (fReWf_nDo_pDm + fi * fImWf_nDo_pDm) * fRadius);
...
DLM_Histo<complex<double>> ***Init_LAntiK_Vidana(const char *InputFolder, CATS &Kitty, const int &TYPE) {
    ...
    for (unsigned uFile = 0; uFile < NumFiles; uFile++) {
        ...
        while (!feof(InFile)) {
            ...
            // we fill up the radius bins with and offset of 1, due to the special zeroth bin
            WhichBin[1] = RadBin + 1;
            sscanf(cdummy, " %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f", &fDummy, &fRadius,
                   &fReWf_piMinXi0, &fImWf_piMinXi0, &fWf2_piMinXi0, &fReWf_pi0XiMin, &fImWf_pi0XiMin, &fWf2_pi0XiMin,
                   &fReWf_LKMin, &fImWf_LKMin, &fWf2_LKMin, &fReWf_KminSig0, &fImWf_KminSig0, &fWf2_KminSig0,
                   &fReWf_K0barSigMin, &fImWf_K0barSigMin, &fWf2_K0barSigMin, &fReWf_etaXiMin, &fImWf_etaXiMin,
                   &fWf2_etaXiMin);
