...
FemtoParticle::~FemtoParticle() {
    ...
    if (MisidParticle) {
        delete[] MisidParticle;
        MisidParticle = NULL;
    }
}

void FemtoParticle::SetFeed(const unsigned &WhichOne, FemtoParticle &Particle) {
    if (WhichOne >= NumFeedChannels)
        return;
    if (!NameIsAvailable(Particle.Name)) {
        printf("WARNING: The name %s is taken.\n", Particle.Name.Data());
        return;
    }
    // this makes sure that the Particle is not a "parent" of this
    if (!Particle.NameIsAvailable(Name)) {
        printf("WARNING: The name %s is taken.\n", Particle.Name.Data());
        return;
    }
...
bool FemtoParticle::NameIsAvailable(const TString &name) const {
    if (name == Name)
        return false;
    for (unsigned uFeed = 0; uFeed < NumFeedChannels; uFeed++) {
        if (!FeedParticle[uFeed])
            continue;
        if (FeedParticle[uFeed]->NameIsAvailable(name) == false)
            return false;
    }
    for (unsigned uMis = 0; uMis < NumMisidChannels; uMis++) {
        if (!MisidParticle[uMis])
            continue;
        if (MisidParticle[uMis]->NameIsAvailable(name) == false)
            return false;
    }
    return true;
}
TString FemtoParticle::GetName() const { return Name; }
unsigned FemtoParticle::GetNumFeed() const { return NumFeedChannels; }
...
unsigned FemtoParticle::GetNumMisid() const { return NumMisidChannels; }
...
int FemtoParticle::GetFeedID(const TString &name) const {
    ...
    for (unsigned uFeed = 0; uFeed < NumFeedChannels; uFeed++) {
        ...
        if (FeedParticle[uFeed]->GetName() == name)
            return uFeed + 1;
    }
    return -1;
}
...
void FemtoExperiment::SetParticle(const unsigned &WhichOne, const FemtoParticle &particle) {
    if (WhichOne >= NumParticles)
        return;
    for (unsigned uPart = 0; uPart < NumParticles; uPart++) {
        if (&particle == Particle[uPart]) {
            printf("WARNING: Duplicate particles!\n");
            return;
        }
        if (Particle[uPart]) {
            if (particle.GetName() == Particle[uPart]->GetName()) {
                printf("WARNING: Duplicate particles!\n");
                return;
            }
        }
    ...
    if (Fraction[WhichOne]) {
        delete[] Fraction[WhichOne];
        Fraction[WhichOne] = NULL;
    }
...
TString FemtoExperiment::GetName() const { return Name; }

void FemtoExperiment::SetFraction(const unsigned &WhichMother, const unsigned &WhichFeed, const double &feed) {
    if (WhichMother >= NumParticles)
        return;
    if (WhichFeed >= Particle[WhichMother]->GetNumContrib())
        return;
...
void FemtoExperiment::SetFraction(const TString &WhichMother, const TString &WhichFeed, const double &feed) {
    ...
    if (FeedID < 0) {
        printf("WARNING: %s is not a direct feed-down channel onto %s\n", WhichFeed.Data(), WhichMother.Data());
    }
...
int FemtoExperiment::FindParticle(const TString &WhichOne) const {
    for (unsigned uPart = 0; uPart < NumParticles; uPart++) {
        if (!Particle[uPart])
            continue;
        if (Particle[uPart]->GetName() == WhichOne) {
            return uPart;
        }
    }
...
FemtoPair::FemtoPair(const FemtoParticle &part1, const FemtoParticle &part2)
    ...
    if (Particle1.GetName() == Particle2.GetName() && &Particle1 != &Particle2) {
        printf("ERROR: Two different FemtoPair objects share the same name! This is not allowed, the FemtoPair object "
               "will be broken!\n");
    }
}
FemtoPair::~FemtoPair() {
...
bool FemtoPair::IsIt(const TString &Name1, const TString &Name2) const {
    if (Particle1.GetName() == Name1 && Particle2.GetName() == Name2)
        return true;
    if (Particle2.GetName() == Name1 && Particle1.GetName() == Name2)
        return true;
    return false;
}

void FemtoPair::SetFeedDownMatrix(const TString &Mother1, const TString &Mother2, const TString &Child1,
                                  const TString &Child2, TH2F *hfeed) {
    ...
    if (&Particle1 == &Particle2)
        hFeedDown[FeedID2][FeedID1] = hfeed;
}
void FemtoPair::SetFeedDownMatrix(const TString &Mother1, const TString &Mother2, TH2F *hfeed) {
...
const FemtoParticle *FemtoPair::GetParticle(const TString &NAME) const {
    if (Particle1.GetName() == NAME)
        return &Particle1;
    if (Particle2.GetName() == NAME)
        return &Particle2;
    else
        return NULL;
}
...
void FemtoExpPair::SetResolutionMatrix(TH2F *hres) { hResolution = hres; }
void FemtoExpPair::SetLargeMomCk(const int &TYPE) { LargeMomCk = TYPE; }
void FemtoExpPair::SetBlType(const int &TYPE) { BlType = TYPE; }
...
void FemtoExpPair::SetStandardInteraction(const TString &Inter, TH1F *TemplateData) {
    ...
    if (!histoDATA)
        histoDATA = hData;
    if (!histoDATA) {
        printf("\033[1;31mERROR:\033[0m SetStandardInteraction needs a data template to figure out the binning\n");
        return;
    }
    // printf("hData = %p\n",hData);
    ...
    if (PartPair.IsIt("Proton", "Proton")) {
        ...
        if (Inter == "CATS_NLO") {
            ...
            InitHaidenbauerNLO("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/CorrelationFiles_2018/"
                               "Haidenbauer/pLambdaNLO/",
                               Kitty[0], &WaveFunctionU, &PhaseShifts, &RadBins, NumRadBins);
            printf("Please keep in mind that InitHaidenbauerNLO works only for specific binning configurations!\n");

            for (unsigned uBin = 0; uBin < NumMomBins; uBin++) {
                // Kitty->UseExternalWaveFunction(uBin,0,0,WaveFunctionU[uBin][0][0], NumRadBins, RadBins,
                // PhaseShifts[uBin][0][0]); Kitty->UseExternalWaveFunction(uBin,1,0,WaveFunctionU[uBin][1][0],
                // NumRadBins, RadBins, PhaseShifts[uBin][1][0]);
            }
...
TString FemtoExpPair::GetStandardInteraction() { return StandardInter; }

const FemtoExperiment *FemtoExpPair::GetExperiment() const { return &Experiment; }
