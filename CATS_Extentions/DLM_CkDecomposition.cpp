...
DLM_CkDecomposition::DLM_CkDecomposition(const char *name, const unsigned &numchildren, DLM_Ck &ckfunction,
                                         const TH2F *hSigmaMatrix, const bool &InvertAxis)
...
void DLM_CkDecomposition::AddContribution(unsigned WhichCk, DLM_Histo<double> &fraction, int type,
                                          DLM_CkDecomposition *child, const TH2F *hResidualMatrix,
    ...
    if (hResidualMatrix)
        dlmFeedMatrix[WhichCk] = Convert_TH2F_DlmHisto(hResidualMatrix);
...
void DLM_CkDecomposition::AddPhaseSpace(const unsigned &WhichCk, const TH1F *hPhaseSpace) {
    ...
    if (hPhaseSpace)
        dlmPhaseSpace[WhichCk] = Convert_TH1F_DlmHisto(hPhaseSpace);
...
void DLM_CkDecomposition::AddPhaseSpace(const TH1F *hPhaseSpace) {
    ...
    if (hPhaseSpace)
        dlmPhaseSpaceMain = Convert_TH1F_DlmHisto(hPhaseSpace);
