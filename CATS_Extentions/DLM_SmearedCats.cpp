...
DLM_SmearedCats::DLM_SmearedCats(CATS **InCat, const unsigned &numCk) : cat(InCat), NumCk(numCk) {
...
DLM_SmearedCats::~DLM_SmearedCats() {
    ...
    if (CorrectedCkErr) {
        delete[] CorrectedCkErr;
        CorrectedCkErr = NULL;
    }
}

void DLM_SmearedCats::SetResolutionMatrix(TH2F *resolution) { hResolution = resolution; }
...
void DLM_SmearedCats::SetUseLednicky(const unsigned &WhichNr, const int &val, const double &grad,
                                     const double &ScattLen1, const double &EffRan1, const double &ScattLen3,
                                     const double &EffRan3, const double &ares, const double &arad,
                                     const double &lambda0) {
    if (WhichNr >= NumCk)
        return;
    LedniStatus[WhichNr] = true;
    if (EvalUsingEquation[WhichNr] != val) {
        EvalUsingEquation[WhichNr] = val;
        LedniStatus[WhichNr] = false;
    };
    if (GaussR != grad) {
        GaussR = grad;
        LedniStatus[WhichNr] = false;
    };
    if (ScattLenSin != ScattLen1) {
        ScattLenSin = ScattLen1;
        LedniStatus[WhichNr] = false;
    };
    if (EffRangeSin != EffRan1) {
        EffRangeSin = EffRan1;
        LedniStatus[WhichNr] = false;
    };
    if (ScattLenTri != ScattLen3) {
        ScattLenTri = ScattLen3;
        LedniStatus[WhichNr] = false;
    };
    if (EffRangeTri != EffRan3) {
        EffRangeTri = EffRan3;
        LedniStatus[WhichNr] = false;
    };
    if (aResidual != ares) {
    ...
    if (LambdaLedni != lambda0) {
        LambdaLedni = lambda0;
        LedniStatus[WhichNr] = false;
    };
}

void DLM_SmearedCats::UseCustomModel(const unsigned &WhichNr, double (*fModel)(const double &, const double *),
                                     double *Pars) {
    if (WhichNr >= NumCk)
        return;
    if (!fModel || !Pars)
        ...
        for (unsigned uBinTrue = 0; uBinTrue < cat[0]->GetNumMomBins(); uBinTrue++) {
            ...
            for (unsigned uCk = 0; uCk < NumCk; uCk++) {
                if (RespMatrix[uCk] && cat[uCk]) {
                    ...
                    case -1:
                        CkVal = ModelFunction(MomentumTrue, ModelParameter);
                        CkValErr = 0;
                        break;
                    case 0:
                        CkVal = cat[uCk]->EvalCorrFun(MomentumTrue);
                        CkValErr = cat[uCk]->EvalCorrFunErr(MomentumTrue);
                        break;
                    case 1:
                        CkVal = CkLednicky(MomentumTrue, false, false);
                        CkValErr = 0;
                        LedniStatus[uCk] = true;
                        break;
                    case 2:
                        CkVal = CkLednicky(MomentumTrue, false, true);
                        CkValErr = 0;
                        LedniStatus[uCk] = true;
                        break;
                    case 3:
                        CkVal = CkLednicky(MomentumTrue, true, false);
                        CkValErr = 0;
                        LedniStatus[uCk] = true;
                        break;
                    case 4:
                        CkVal = CkLednicky(MomentumTrue, true, true);
                        CkValErr = 0;
                        LedniStatus[uCk] = true;
                        break;
                    default:
                    ...
                    CorrectedCk[uBinSmear] +=
                        LambdaCoeff[uCk] * RespMatrix[uCk]->ResponseMatrix[uBinSmear][uBinTrue] * CkVal;
                    CorrectedCkErr[uBinSmear] +=
                        LambdaCoeff[uCk] * RespMatrix[uCk]->ResponseMatrix[uBinSmear][uBinTrue] * CkValErr;
                    if (CorrectedCkErr[uBinSmear] != CorrectedCkErr[uBinSmear]) {
                        if (CkValErr != CkValErr)
                            printf("TROUBLE CkValErr -> k=%f; cat[uCk]->GetCorrFunErr(0)=%f!!!\n", MomentumTrue,
                                   cat[uCk]->GetCorrFunErr(0));
                        if (LambdaCoeff[uCk] != LambdaCoeff[uCk])
                            printf("TROUBLE lam!!!\n");
                        if (RespMatrix[uCk]->ResponseMatrix[uBinSmear][uBinTrue] !=
...
double DLM_SmearedCats::GetCorrectedCk(const unsigned &WhichBin) { return CorrectedCk[WhichBin]; }

double DLM_SmearedCats::GetCorrectedCkErr(const unsigned &WhichBin) { return CorrectedCkErr[WhichBin]; }

double DLM_SmearedCats::EvalCorrectedCk(const double &Momentum) {
    unsigned NumMomBins = cat[0]->GetNumMomBins();
    if (Momentum < cat[0]->GetMomBinLowEdge(0) || Momentum > cat[0]->GetMomBinLowEdge(NumMomBins))
        return 0;
    if (NumMomBins == 1)
        return CorrectedCk[0];
    ...
    return (CkRange[1] * (Momentum - InterpolRange[0]) - CkRange[0] * (Momentum - InterpolRange[1])) /
...
double DLM_SmearedCats::CkLednicky(const double &Momentum, const bool &SinOnly, const bool &QS,
                                   const bool &WithLambda) {
    ...
    if (!SinOnly) {
        complex<double> ScattAmplTri =
            pow(1. / ScattLenTri + 0.5 * EffRangeTri * Momentum * Momentum - i * Momentum, -1.);
        CkValue += 3 * (0.5 * pow(abs(ScattAmplTri) / GaussR, 2) * (1 - (EffRangeTri) / (2 * sqrt(Pi) * GaussR)) +
...
bool DLM_SmearedCats::GetStatus() {
    ...
    for (unsigned uCk = 0; uCk < NumCk; uCk++) {
        if (!cat[uCk])
            continue;
        else if (EvalUsingEquation[uCk] == 0)
            Status *= cat[uCk]->CkStatus();
        else
            Status *= LedniStatus[uCk];
    }
    return Status;
}
