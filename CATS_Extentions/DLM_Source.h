...
class DLM_CleverMcLevyReso : public CatsSource {
  public:
    ...
    // rdtRandomBackwards = uniform emission of particles, but particles are only allowed to move away from one another
    // (90-180)
    enum RESO_DEC_TOP { rdtBackwards, rdtRandom, rdtRandomBackwards };
    void SetUpReso(const unsigned &whichparticle, const unsigned &whichreso, const double &weight, const double &mass,
                   const double &tau, const double &mass0, const double &mass1, const double &momSmear = 0,
                   const bool &massSmear = false, const RESO_DEC_TOP &rdt = rdtBackwards);
  ...
  private:
    ...
    // this should be the cumulative distribution of the probability of emission at a certain angle (in RAD between 0
    // and Pi), where the axis are inverted, i.e. y(x)
...
class DLM_CleverMcLevyResoTM : public CatsSource {
  ...
  private:
    ...
    // i.e. beta*gamma*tau of the reso itself. An approximation that one could consider: set beta*gamma of the prim is
    // lower then that of the reso (due to the lower momentum), hence we neglect this term and set it to 0
    unsigned MaxBGT_PR;
    unsigned NumBGT_PR;
    float **BGT_PR;
...
struct KdpPars { // This structure is named "myDataType"
    KdpPars() {}
    KdpPars(double zero) { operator=(zero); }
