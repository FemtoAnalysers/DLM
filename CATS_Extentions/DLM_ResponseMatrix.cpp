...
DLM_ResponseMatrix::DLM_ResponseMatrix(DLM_Histo<double> &ch, const DLM_Histo<float> *hs, const DLM_Histo<float> *hr,
                                       const bool &ia)
...
double **DLM_ResponseMatrix::GetMatrix(const int &WhichMatr) {
    ...
    default:
        printf("ERROR: DLM_ResponseMatrix::GetSparse says bad luck!\n");
        return NULL;
    }
}
...
void DLM_ResponseMatrix::MakeUnitMatrix(const int &WhichMatr) {
    ...
    for (int iBinX = 0; iBinX < NumMomBins; iBinX++) {
        for (int iBinY = 0; iBinY < NumMomBins; iBinY++) {
            Matrix[iBinY][iBinX] =
                double(iBinX == iBinY) / (CatHisto->GetBinSize(0, iBinX) * CatHisto->GetBinSize(0, iBinY));
        }
...
void DLM_ResponseMatrix::ConvertMatrix(const int &WhichMatr, const DLM_Histo<float> *input, const bool &InvAxis) {
    ...
    for (int iBinX = 0; iBinX < NumMomBins; iBinX++) {
        ...
        for (int iBinY = 0; iBinY < NumMomBins; iBinY++) {
            ...
            WeightX[0] = (input->GetBinUpEdge(Xaxis, WhichBinAtLowEdgeX) - MomLowEdgeX) /
                         input->GetBinSize(Xaxis, WhichBinAtLowEdgeX);
            if (NumOldBinsX > 1) {
                WeightX[NumOldBinsX - 1] = (MomUpEdgeX - input->GetBinLowEdge(Xaxis, WhichBinAtUpEdgeX)) /
                                           input->GetBinSize(Xaxis, WhichBinAtUpEdgeX);
            }

            for (int iOldBinY = 1; iOldBinY < NumOldBinsY - 1; iOldBinY++) {
                WeightY[iOldBinY] = 1;
            }
            WeightY[0] = (input->GetBinUpEdge(Yaxis, WhichBinAtLowEdgeY) - MomLowEdgeY) /
                         input->GetBinSize(Yaxis, WhichBinAtLowEdgeY);
            if (NumOldBinsY > 1) {
                WeightY[NumOldBinsY - 1] = (MomUpEdgeY - input->GetBinLowEdge(Yaxis, WhichBinAtUpEdgeY)) /
                                           input->GetBinSize(Yaxis, WhichBinAtUpEdgeY);
            }
...
double DLM_ResponseMatrix::BilinearInterpolation(const double &x0, const double &y0, const double &x1, const double &y1,
                                                 const double &f00, const double &f01, const double &f10,
                                                 const double &f11, const double &xVal, const double &yVal) {
    return (f00 * (x1 - xVal) * (y1 - yVal) + f10 * (xVal - x0) * (y1 - yVal) + f01 * (x1 - xVal) * (yVal - y0) +
            f11 * (xVal - x0) * (yVal - y0)) /
           (x1 - x0) / (y1 - y0);
}
...
void DLM_ResponseMatrix::NormalizeMatrix(const int &WhichMatr) {
    ...
    for (int iBinY = 0; iBinY < NumMomBins; iBinY++) {
        for (int iBinX = Sparse[iBinY][xAxisFirst]; iBinX <= Sparse[iBinY][xAxisLast]; iBinX++) {
            if (Norm[iBinY])
                Matrix[iBinY][iBinX] /= (Norm[iBinY] * CatHisto->GetBinSize(iBinX) * CatHisto->GetBinSize(iBinY));
            else
                Matrix[iBinY][iBinX] = 0;
        }
    }
