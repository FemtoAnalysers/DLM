...
class FemtoPair {
  public:
    ...
    void SetFeedDownMatrix(const TString &Mother1, const TString &Mother2, const TString &Child1, const TString &Child2,
                           TH2F *hfeed);

    void SetFeedDownMatrix(const TString &Mother1, const TString &Mother2, TH2F *hfeed);
...
class FemtoExpPair {
  public:
    ...
    void SetLargeMomCk(const int &TYPE);
    enum cktype { ckLinear, ckFlat, ckUnity };
    void SetBlType(const int &TYPE);
    enum bltype { blPol0, blPol1, blPol2, blSeparatePol0, blSeparatePol1, blSeparatePol2 };
  ...
  private:
    ...
    double BlRegionLow;
    double BlRegionUp;
    // how to model the baseline. Choose between pol0,1,2, as well as the same options but performing separate fitting
    // of the BL
    int BlType;
