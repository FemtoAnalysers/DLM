...
class DLM_Fitter1 {
  ...
  public:
    ...
    void SetSystem(const unsigned &WhichSyst, const TH1F &histo, const double &FromMeV, DLM_CkDecomp &decomp,
    ...
    void AddSameParameter(const TString &System, const unsigned &WhichPar, const TString &ParentSystem,
    ...
    void SetParameter(const unsigned &WhichSyst, const unsigned &WhichPar, const double &Value,
                      const double &ValueDown = -1e64, const double &ValueUp = 1e64);
    void SetParameter(const TString &WhichSyst, const unsigned &WhichPar, const double &Value,
                      const double &ValueDown = -1e64, const double &ValueUp = 1e64);
    ...
    // if true, there is not really a baseline region, in a sense that Ck is evaluated fully in the baseline region as
    // well. this was build in case a region of the fit needs to be cut-off, e.g. a peak N.B. if set to yes, it
    // overrides the SeparateBL!
    ...
    TGraphErrors *GetUnfoldedCk(const unsigned &WhichSyst, const unsigned &Regulator,
...
class DLM_Fitter2 {
  ...
  public:
    ...
    void SetLimitPolParX(const TString &WhichSyst, const fBlType &WhichBl, const int &WhichPar, const double &down,
                         const double &up);
    ...
    void SetLimitPolParY(const TString &WhichSyst, const fBlType &WhichBl, const int &WhichPar, const double &down,
                         const double &up);
    ...
    void SetLimitSplineKnotX(const TString &WhichSyst, const fBlType &WhichBl, const int &WhichKnot, const double &down,
                             const double &up);
    ...
    void SetLimitSplineKnotY(const TString &WhichSyst, const fBlType &WhichBl, const int &WhichKnot, const double &down,
                             const double &up);
...
//data, where the first set of bins are those corresponding to pp. The x-val is in arbitrary units and takes values from
0 to 1.
