...
class CatsLorentzVector {
  ...
  public:
    ...
    // cosine of the scattering angle has reversed sign !!! I.e. if you use this function, take the output with a shift
    // of pi (-cos)
    ...
    void Set(const double &tCrd, const double &xCrd, const double &yCrd, const double &zCrd, const double &engy,
             const double &xMom, const double &yMom, const double &zMom);
...
class CATSnode {
  ...
  public:
    CATSnode(CATSelder *elder, const short &depth, const unsigned &firstid, const unsigned &lastid, double *mean,
             double *len, const CATSnode *TemplateNode = NULL);
    ~CATSnode();
...
class CATSelder : public CATSnode {
  ...
  public:
    ...
    CATSelder(const CATSelder *TemplateElder, void *context, CATSparameters *Pars, int64_t *gbid, const unsigned &numel,
              const bool &renorm = true);
    void BaseConstructor(double *mean, double *len, void *context, CATSparameters *Pars, int64_t *gbid,
                         const unsigned &numel, const CATSelder *TemplateElder, const bool &renorm = true);
    ~CATSelder();
