...
double CATS::GetRedMass() const { return RedMass; }
...
short CATS::GetQuantumStatistics() const { return QuantumStatistics; }
...
void CATS::SetNumChannels(const unsigned short &numCh) {
    ...
    if (Spin) {
        if (Notifications >= nError) {
            printf("\033[1;31mERROR:\033[0m Due to a known existing bug, ones set, you are not allowed to change the "
                   "number of channels. Sorry :(\n");
        }
        return;
    }
...
unsigned short CATS::GetNumChannels() const { return NumCh; }
...
int CATS::GetQ1Q2() const { return Q1Q2; }
...
bool CATS::GetGamow() const { return Gamow; }

unsigned CATS::GetNumMomBins() const { return NumMomBins; }

unsigned CATS::GetNumIpBins() const { return NumIpBins; }

unsigned CATS::GetNumPairs() const { return NumPairs; }

void CATS::SetMomBins(const unsigned &nummombins, const double *mombins, const double *bincenter) {
    if (!nummombins) {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m Bad input in CATS::SetMomBins(const unsigned& nummombins, const double* "
                   "mombins)\n");
        return;
    }
    if (!mombins) {
...
void CATS::SetIpBins(const unsigned &numBbins, const double *imppar) {
    if (!numBbins) {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m Bad input in CATS::SetIpBins(const unsigned& numBbins, const double* "
                   "imppar)\n");
        return;
    }
    if (!imppar) {
...
void CATS::SetIpBins(const unsigned &numBbins, const double &MinImpPar, const double &MaxImpPar) {
    if (!numBbins) {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m Bad input in CATS::SetIpBins(const unsigned& numBbins, const double& "
                   "MinImpPar, const double& MaxImpPar)\n");
        return;
    }
...
void CATS::SetChannelWeight(const unsigned short &usCh, const double &weight) {
    if (usCh >= NumCh) {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m Bad input in CATS::SetChannelWeight(const unsigned short& usCh, const "
                   "double& weight)\n");
        return;
    }
...
double CATS::GetStartRad() const { return StartRad * NuToFm; }
...
double CATS::GetEpsilonProp() const { return EpsilonProp; }
...
double CATS::GetEpsilonConv() const { return EpsilonConv; }
...
double CATS::GetMaxRad() const { return MaxRad * NuToFm; }
...
double CATS::GetMaxRho() const { return MaxRho; }
...
unsigned short CATS::GetMaxPw() const { return MaxRho; }
...
void CATS::SetMaxNumThreads(const unsigned short &maxnumthreads) { MaxNumThreads = maxnumthreads; }
unsigned short CATS::GetMaxNumThreads() const { return MaxNumThreads; }
...
bool CATS::GetExcludeFailedBins() const { return ExcludeFailedConvergence; }
...
short CATS::GetGridMinDepth() const { return GridMinDepth; }
...
short CATS::GetGridManDepth() const { return GridMaxDepth; }
...
double CATS::GetGridEpsilon() const { return GridEpsilon; }
...
unsigned CATS::GetMaxPairsPerBin() const { return MaxPairsPerBin; }
...
unsigned CATS::GetMaxPairsToRead() const { return MaxPairsToRead; }
...
unsigned short CATS::GetMixingDepth() const { return MixingDepth; }
...
bool CATS::GetTauCorrection() const { return TauCorrection; }
...
bool CATS::GetUseAnalyticSource() const { return UseAnalyticSource; }
...
bool CATS::GetThetaDependentSource() const { return ThetaDependentSource; }
...
double CATS::GetTransportRenorm() const { return TransportRenorm; }
...
double CATS::GetPoorManRenorm() const { return PoorManRenorm; }
...
double CATS::GetSourceMinRange() const { return SourceMinRad; }
double CATS::GetSourceMaxRange() const { return SourceMaxRad; }
...
bool CATS::GetNormalizedSource() const { return NormalizedSource; }
...
bool CATS::GetAutoNormSource() const { return AutoNormSource; }

void CATS::SetTotPairMomCut(const double &minval, const double &maxval) {
    if (minval < 0 || maxval < minval) {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m Bad input in void CATS::SetTotPairMomCut(const double& minval, const "
                   "double& maxval)");
        return;
    }
...
void CATS::RemoveTotPairMomCut() { UseTotMomCut = false; }

void CATS::SetNotifications(const short &notify) { Notifications = notify; }
short CATS::GetNotifications() const { return Notifications; }
...
void CATS::GetPairInfo(const unsigned &uWhichPair, double &RelMom, double &RelPos, double &RelCosTh,
                       double &TotMom) const {
...
void CATS::GetPairInfo(const unsigned &uWhichPair, double *Output) const {
    GetPairInfo(uWhichPair, Output[0], Output[1], Output[2], Output[3]);
}
...
double CATS::GetCorrFunIp(const unsigned &WhichMomBin, const unsigned &WhichIpBin, double &Momentum,
                          double &ImpPar) const {
...
unsigned CATS::GetNumRadialWFpts(const unsigned &WhichMomBin, const unsigned short &usCh,
                                 const unsigned short &usPW) const {
    return SavedWaveFunBins[WhichMomBin][usCh][usPW];
}

complex<double> CATS::GetRadialWaveFunction(const unsigned &WhichMomBin, const unsigned short &usCh,
                                            const unsigned short &usPW, const unsigned &WhichRadBin) const {
    if (NumMomBins <= WhichMomBin || NumCh <= usCh || NumPW[usCh] <= usPW ||
        SavedWaveFunBins[WhichMomBin][usCh][usPW] <= WhichRadBin)
        return 0;
    return WaveFunctionU[WhichMomBin][usCh][usPW][WhichRadBin];
}

complex<double> CATS::EvalRadialWaveFunction(const unsigned &WhichMomBin, const unsigned short &usCh,
                                             const unsigned short &usPW, const double &Radius,
...
double CATS::EvalWaveFun2(const unsigned &uMomBin, const double &Radius, const double &CosTheta,
                          const unsigned short &usCh) {
...
complex<double> CATS::EvalAsymptoticRadialWF(const unsigned &WhichMomBin, const unsigned short &usCh,
                                             const unsigned short &usPW, const double &Radius, const bool &DivideByR) {
    if (NumMomBins <= WhichMomBin || NumCh <= usCh || NumPW[usCh] <= usPW)
        return 0;
    return EvalWaveFunctionU(WhichMomBin, Radius * FmToNu, usCh, usPW, DivideByR, true);
}

complex<double> CATS::EvalReferenceRadialWF(const unsigned &WhichMomBin, const unsigned short &usPW,
                                            const double &Radius, const bool &DivideByR) {
...
const double &CATS::FmNu() const { return FmToNu; }

const double &CATS::NuFm() const { return NuToFm; }
...
void CATS::SetShortRangePotential(const unsigned &usCh, const unsigned &usPW, double (*pot)(double *Pars),
                                  CATSparameters &Pars) {
    if (usCh >= NumCh) {
        if (Notifications >= nError)
...
/*
void CATS::SetShortRangePotential(const unsigned& usCh, const unsigned& usPW, double (*pot)(double* Pars), double*
Pars){ if(usCh>=NumCh){ if(Notifications>=nError) printf("\033[1;31mERROR:\033[0m Bad input in
CATS::SetShortRangePotential(...)\n"); return;
    }
...
*/
void CATS::SetShortRangePotential(const unsigned &usCh, const unsigned &usPW, const unsigned &WhichPar,
                                  const double &Value) {
    if (usCh >= NumCh) {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m Bad input in CATS::SetShortRangePotential(...)\n");
...
void CATS::SetShortRangeSofia(const double &cutoff) {
    ...
    if (use_sofia_model && cutoff == Sofia_Cutoff)
        return;
    Sofia_Cutoff = cutoff;
    SetWfType(wSofia);
    ComputedWaveFunction = false;
...
void CATS::SetWfType(const unsigned usCh, const unsigned usPW, const int type) {
    ...
    if (type == wSofia && internal_SetWfType == false) {
        if (Notifications >= nWarning)
            printf("\033[1;33mWARNING:\033[0m The Sofia model requires ALL channels and PWs to be evaluated with it! "
                   "Applying the Sofia model to ALL wave functions!\n");
        SetWfType(type);
        return;
    }
    WfType[usCh][usPW] = type;
    if (type == wSofia)
        use_sofia_model = true;
    else
        use_sofia_model = false;
}
...
void CATS::SetAnaSource(double (*FS)(void *, double *), void *context, const unsigned &numparameters) {
    if (ForwardedSource == FS && SourceContext == context && AnaSourcePar->GetNumPars() == numparameters)
        return;
    delete AnaSourcePar;
...
void CATS::SetAnaSource(const unsigned &WhichPar, const double &Value, const bool &SmallChange) {
    ...
    if (WhichPar >= AnaSourcePar->GetNumPars()) {
        if (Notifications >= nWarning)
            printf("\033[1;33mWARNING:\033[0m The source parameter was not set, as it exceeds the number of allowed "
                   "parameters.\n");
        return;
    }
    // if we use a member function, we assume that there are no parameters to be changed here
    // if(ForwardedSource){
    //     if(Notifications>=nWarning) printf("\033[1;33mWARNING:\033[0m Using a source member function does not allow
...
void CATS::SetExternalWaveFunction(const unsigned &usCh, const unsigned &usPW, DLM_Histo<complex<double>> &histWF,
                                   DLM_Histo<complex<double>> &histPS) {
    if (NumCh <= usCh || NumPW[usCh] <= usPW) {
        if (Notifications >= nError)
...
bool CATS::CkStatus() { return ComputedCorrFunction; }
bool CATS::SourceStatus() { return SourceUpdated; }
bool CATS::PotentialStatus() { return ComputedWaveFunction; }
...
void CATS::ComputeWaveFunction() {
    ...
    // the problem with the omp is that the same PotPar are used, we need separate instance for each thread if we want
    // it to work this is however difficult as currently I only pass a single pointer as PotPar, and I do not know how
    // many arguments there are the same problem should occur for the source
    ...
    for (unsigned uMPP = 0; uMPP < TotalNumberOfBins; uMPP++) {
        ...
        if (WfType[usCh][usPW] == wSchroedinger) {
            ...
            // index that keeps track of which value in the arrays correspond to the previous, current and next
            // computing step.
            short kOld;
            short kCurrent;
            short kNew;
            ...
            PropagatingFunction(PropFunWithoutSI[kCurrent], PropFunVal[kCurrent], PosRad[kCurrent], Momentum, usPW,
                                usCh);

            // the initial values for the wave function are set based on the solution without the strong potential.
            // this will of course lead to a wrong normalization in the asymptotic region, but this will be corrected
            // for later on, at this stage it is only important that the algorithm gets a meaningful guess so that we do
            // not encounter overflow problems during the calculation.
            ...
            // EstNumRadSteps is a fair small over-estimation of the actual bins needed. It is computed by assuming we
            // have the worst-case scenario up until rho=1.57 or r=4 fm (whichever occurs first at this Momentum) and
            // assumes that the rest as best case scenario. This number looks to be c.a. 10x bigger than the actual num.
            // bins, so use it for memory allocation.

            unsigned EstNumRadSteps =
                ceil(((MaxRad * Momentum) > MaxRho ? MaxRad - 1.57 / Momentum
                                                   : MaxRho / Momentum + ConvIntervalRadLength - 1.57 / Momentum) /
                     MaxDeltaRad) +
                MinConvSteps + 1 + ceil((4. * FmToNu < 1.57 / Momentum ? 4. * FmToNu : 1.57 / Momentum) / MinDeltaRad);
            ...
            while ((!Converged || (PosRad[kCurrent] < ConvergenceRadius + ConvIntervalRadLength)) &&
                ...
                Convergence = fabs((PropFunWithoutSI[kOld] - PropFunVal[kOld]) /
                                   (fabs(PropFunWithoutSI[kOld] + PropFunVal[kOld]) + 1e-64)) < EpsilonConv &&
                              fabs((PropFunWithoutSI[kCurrent] - PropFunVal[kCurrent]) /
                                   (fabs(PropFunWithoutSI[kCurrent] + PropFunVal[kCurrent]) + 1e-64)) < EpsilonConv &&
                              fabs((PropFunWithoutSI[kNew] - PropFunVal[kNew]) /
                                   (fabs(PropFunWithoutSI[kNew] + PropFunVal[kNew]) + 1e-64)) < EpsilonConv;
            ...
            // 1) изхвърли ги
            // 2) запаза ги, като нормировката я направи на база максимума в MaxRho-3.14 (btw. make 3.14 the min. value
            // for MaxRho!!!)
            ...
            if (!MaxConvergedNumWF && Notifications >= nWarning) {
                printf("\033[1;33mWARNING:\033[0m MaxConvergedNumWF is zero, which is not allowed and points to a bug "
                       "in the code!\n");
            ...
            if (MomBinConverged[uMomBin] || !ExcludeFailedConvergence) {
                ...
                if ((!BufferWaveFunction[StepOfMaxConvergedNumWF] ||
                     !BufferWaveFunction[StepOfMaxConvergedNumWF + 1]) &&
                    Notifications >= nWarning) {
                    printf("\033[1;33mWARNING:\033[0m BufferWaveFunction is zero, which is not allowed and points to a "
                           "bug in the code!\n");
                    printf("         Please contact the developers and do not trust your current results!\n");
                }
                double NumRatio =
                    BufferWaveFunction[StepOfMaxConvergedNumWF + 1] / BufferWaveFunction[StepOfMaxConvergedNumWF];
                double DownValue;
                double UpValue;
                double SignProduct;
                ...
                // a potential solution should be locked in a region where the ShiftedReferenceWave*NumericalSol have
                // the same sign. furthermore the ShiftedReferenceWave cannot possibly change sign in this region
                for (unsigned uShift = 0; uShift < MaximumShiftIter; uShift++) {
                    ...
                    if (DownValue * UpValue < 0) {
                        ...
                        SignProduct =
                            ReferencePartialWave(MaxConvRad + ShiftRad, Momentum, usPW, q1q2) * MaxConvergedNumWF;
                        SignRefAtLimits = ReferencePartialWave(MaxConvRad + DownShift, Momentum, usPW, q1q2) *
                                          ReferencePartialWave(MaxConvRad + UpShift, Momentum, usPW, q1q2);
                ...
                ShiftRad = NewtonRapson(&CATS::AsymptoticRatio, DeltaRadAtMaxConv, usPW, Momentum, q1q2,
                                        MaxConvRad + DownShift, MaxConvRad + UpShift, NumRatio, NR_Status) -
                           MaxConvRad;
                // if(!NR_Status&&Notifications>=nWarning) {printf("The above warning triggered for bin nr. %u
                ...
                // we start from r=0, than proceed by adding the limit between different bins as the mid-way to the next
                // point
                ...
                // the very last point we simply set as the double distance between the last bin-limit and the last
                // radius value
                WaveFunRad[uMomBin][usCh][usPW][SWFB] =
                    2 * BufferRad[SWFB - 1] - WaveFunRad[uMomBin][usCh][usPW][SWFB - 1];
        ...
        // the case with external wave function
            ...
            for (unsigned uPoint = 0; uPoint < SWFB; uPoint++) {
                ...
                WaveFunctionU[uMomBin][usCh][usPW][uPoint] =
                    CPF[uMomBin] * ExternalWF[usCh][usPW]->Eval(EvalPoint) * FmToNu;
            }
            ...
            if (Notifications >= nError)
                printf("          \033[1;31mERROR:\033[0m Major bug in ComputeWaveFunction \033[0m contact the "
                       "developers!\n");
            return;
        }
...
short CATS::LoadData(const unsigned short &NumBlankHeaderLines) {
    ...
    while (!feof(InFile)) {
        ...
        for (int iPart = 0; iPart < NumPartInEvent; iPart++) {
            ...
            if (KittyParticle.GetE() == 0) {
                if (Notifications >= nWarning)
                    printf(
                        "\033[1;33mWARNING!\033[0m Possible bad input-file, there are particles with zero energy!\n");
                continue;
            }
    ...
    if (!NumPairs) {
        ...
        if (Notifications >= nWarning)
            printf("\033[1;31m          WARNING:\033[0m There were no same-events pairs found! The computation cannot "
                   "proceed!\n");
    } else {
        LoadedData = true;
        if (UseTotMomCut) {
...
unsigned CATS::LoadDataBuffer(const unsigned &WhichIpBin, CatsDataBuffer *KittyBuffer) {
    ...
    for (unsigned uPair = 0; uPair < KittyBuffer->GetNumPairs(); uPair++) {
        ...
        RelCosThCom = (PairDif->GetPx() * PairDif->GetX() + PairDif->GetPy() * PairDif->GetY() +
    ...
    for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++) {
        ...
        for (unsigned uIpBin = 0; uIpBin < NumIpBins; uIpBin++) {
            ...
            for (unsigned uGrid = 0; uGrid < NumGridPts; uGrid++) {
                ...
                kbCorrFunErr[uMomBin][uIpBin] +=
                    pow(Integrand * kbSourceGrid[uMomBinSource][uIpBin]->GetGridError(uGrid), 2);
            } // uGrid
            ...
            if (SourceInt > 1 + 1e-4) {
                if (Notifications >= nWarning)
                    printf("\033[1;33mWARNING:\033[0m The source had to be renormalized (Integrated value=%.4e), "
                           "please check the validity of your input function!\n",
                           SourceInt);
                // if(AnaSourcePar)
                // printf("AnaSourcePar: p0=%e; p1=%e",AnaSourcePar->GetParameter(0),AnaSourcePar->GetParameter(1));
                // if(ForwardedSourcePar)
                // printf("ForwardedSourcePar: p0=%e;
                // p1=%e",ForwardedSourcePar->GetParameter(0),ForwardedSourcePar->GetParameter(1));
            ...
            if (NormalizedSource && SourceIntCut < 1.) {
                // this is an additional scale factor, evaluated based on the weight of channels with non-zero
                // asymptotic solution
            ...
            // I am not sure if this even makes sense, consider in the future to use only the NormalizedSource case
                ...
                kbCorrFunErr[uMomBin][uIpBin] =
                    sqrt(kbCorrFunErr[uMomBin][uIpBin]) * double(SourceInt) / double(SourceIntCut);
            }
        ...
        if (UseAnalyticSource || NumIpBins <= 1 || MixingDepth == 1) {
            ...
            // this is where I will always end up with an analytic source
                ...
                for (unsigned uGrid = 0; uGrid < NumGridPts; uGrid++) {
                    ...
                    // if(use_sofia_model){
                    // printf("\033[1;36m  DEBUG (2):\033[0m %u SourceVal*WaveFunVal = %.4f *
                    // %.4f\n",uMomBin,SourceVal,WaveFunVal); usleep(5e3);
                    // }
                    Integrand = SourceVal * WaveFunVal;
                ...
                if (SourceInt > 1 + 1e-4) {
                    ...
                    // printf(" SourceInt=%f\n",SourceInt);
                    // if(AnaSourcePar)
                    // printf("AnaSourcePar: p0=%e;
                    // p1=%e\n",AnaSourcePar->GetParameter(0),AnaSourcePar->GetParameter(1)); if(ForwardedSourcePar)
                    // printf("ForwardedSourcePar: p0=%e;
                ...
                // printf("\033[1;36mDEBUG (2):\033[0m Integrated value=%.4e <> %.4e
...
void CATS::SetUpSourceGrid() {
    ...
    if (UseAnalyticSource) {
        ...
        BaseSourceGrid = new CATSelder(DIM, GridMinDepth, MAXDEPTH, LIMIT, MEAN, LENGTH, this, AnaSourcePar, NULL, 0,
                                       AutoNormSource);
    } else if (NumPairs) {
        // sorts the whole data according to the GridBoxId (all bins!)
        // this will setup the base grid
        SortAllData();
        BaseSourceGrid = new CATSelder(DIM, GridMinDepth, MAXDEPTH, LIMIT, MEAN, LENGTH, NULL, NULL, GridBoxId,
                                       NumPairs, AutoNormSource);
    } else {
        BaseSourceGrid = NULL;
    }
    ...
    for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++) {
        ...
        if (UseAnalyticSource) {
            ...
            kSourceGrid[uMomBin] = new CATSelder(BaseSourceGrid, NULL, NULL, &GridBoxId[ArrayPosition],
                                                 LoadedPairsPerMomBin[uMomBin], AutoNormSource);
    ...
    if (!UseAnalyticSource && NumIpBins > 1) {
        ...
        for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++) {
            ...
            for (unsigned uIpBin = 0; uIpBin < NumIpBins; uIpBin++) {
                if (UseAnalyticSource) {
                    kbSourceGrid[uMomBin][uIpBin] =
                        new CATSelder(BaseSourceGrid, this, AnaSourcePar, NULL, 0, AutoNormSource);
                } else if (LoadedPairsPerBin[uMomBin][uIpBin]) {
                    kbSourceGrid[uMomBin][uIpBin] = new CATSelder(BaseSourceGrid, NULL, NULL, &GridBoxId[ArrayPosition],
                                                                  LoadedPairsPerBin[uMomBin][uIpBin], AutoNormSource);
...
void CATS::PropagatingFunction(double &Basic, double &Full, const double &Radius, const double &Momentum,
    ...
    Basic = 2 * RedMass * CoulombPotential(Radius) + double(usPW) * (double(usPW) + 1) / (Radius * Radius + 1e-64) -
            Momentum * Momentum;
...
double CATS::CoulombPartialWave(const double &Radius, const double &Momentum, const unsigned short &usPW,
    ...
    // N.B. gsl_sf_coulomb_wave_F_array are defined for Rho>0, but in principle the Coulomb functions are symmetric for
    // odd l and anti-symmetric for even l. However here I assume that Momentum>0, and if rho is negative so is the
    // Momentum. Thus the final result for u_l should be symmetric for even l and antisymmetric for odd l. This is
    // implemented here.
    if (Rho < 0 && usPW % 2 == 1) {
        Result = -Result;
    }
...
/*
double CATS::CoulombPartialWaveSquareWell(const double& Radius, const double& Momentum, const unsigned short& usPW,
const int& q1q2) const{ if(Radius*NuToFm>SwWidth){ return CoulombPartialWave(Radius,Momentum);
    }
    ...
    //N.B. gsl_sf_coulomb_wave_F_array are defined for Rho>0, but in principle the Coulomb functions are symmetric for
odd l
    //and anti-symmetric for even l. However here I assume that Momentum>0, and if rho is negative so is the Momentum.
Thus
...
// note that this function is never corrected for the Gamow factor, as in the asymptotic region one could just take the
// exact solution instead of correcting a plane wave by Gamow factor
...
double CATS::AsymptoticRatio(const double &Radius, const double &Momentum, const unsigned short &usPW,
                             const int &q1q2) const {
    return ReferencePartialWave(Radius + CurrentRhoStep / Momentum, Momentum, usPW, q1q2) /
           (ReferencePartialWave(Radius, Momentum, usPW, q1q2) + 1e-64);
}
...
double CATS::NewtonRapson(double (CATS::*Function)(const double &, const double &, const unsigned short &, const int &)
    ...
    for (unsigned iIter = 0; iIter < maxIter; iIter++) {
        ...
        if (!DeltaF) {
            DeltaX = 1;
            if (Notifications >= nWarning) {
                printf("\033[1;33mWARNING:\033[0m Something is fishy with the NewtonRapson solver! Might be a bug! "
...
unsigned CATS::GetBoxId(double *particle) {
    ...
    // of the parameters, that can only take values 0 or 1. Each time WhichPart[x] is increased to 2, than it is set to
    // zero and WhichPart[x+1] is increased, i.e. we continue to iterate over the next parameter.
    char WhichPart[Dim];
    bool ThisBox;
...
complex<double> CATS::EvalWaveFunctionU(const unsigned &uMomBin, const double &Radius, const unsigned short &usCh,
                                        const unsigned short &usPW, const bool &DivideByR,
                                        const bool &Asymptotic) const {
    ...
    /*
        if(true){
            ...
            double rpw = ReferencePartialWave(Radius+PhaseShift[uMomBin][usCh][usPW]/Momentum, Momentum, usPW,
       Q1Q2)*MultFactor; double Radius_fm = Radius*NuToFm; if(uMomBin==5 && usCh==0 && usPW==0 && Radius_fm>4 &&
       Radius_fm<4.02){ printf("%i k*=%.0f r*=%.2f Re[Result]=%.4f Imag[Result]=%.4f rpw=%.4f ps=%.3f\n",
                    use_sofia_model, Momentum, Radius_fm, real(Result), imag(Result), rpw,
       PhaseShift[uMomBin][usCh][usPW]);
            }
    ...
    */
    ...
    if (!use_sofia_model && RadBin < SWFB && !Asymptotic) {
        ...
        if (Result == 1e6 && Notifications >= nWarning)
            printf(
                "\033[1;33mWARNING:\033[0m DeltaRad==0, which might point to a bug! Please contact the developers!\n");
        // printf("hello %i\n",Asymptotic);
        // usleep(100e3);
    ...
    // below the 1st bin ==> assume zero
        ...
        //     printf("%u %.3f %.4f %.4f\n", use_sofia_model, Radius, real(EvalBinnedFun(Radius, SWFB, WFR, NULL,
        //     WFU))*MultFactor, ReferencePartialWave(Radius+PhaseShift[uMomBin][usCh][usPW]/Momentum, Momentum, usPW,
        //     Q1Q2)*MultFactor);
        // }
        return ReferencePartialWave(Radius + PhaseShift[uMomBin][usCh][usPW] / Momentum, Momentum, usPW, Q1Q2) *
               MultFactor;
    }
}
...
double CATS::EffectiveFunctionTheta(const unsigned &uMomBin, const double &Radius, const double &CosTheta,
    ...
    for (unsigned short usPW = 0; usPW < MaxPw; usPW++) {
        ...
        if (IdenticalParticles && (usPW + Spin[usCh]) % 2)
            continue;
        if (usPW < NumPW[usCh] &&
            (ShortRangePotential[usCh][usPW] || ExternalWF[usCh][usPW] || WfType[usCh][usPW] == wSofia)) {
            if (LegPol[usPW] == 1e6)
            ...
            // please check if a sqrt is needed for 2*l+1. I think not, because above, when we integrate Pl, the
            // integration of Pl itself results in 1/(2l+1)
...
unsigned CATS::GetMomBin(const double &Momentum) const { return GetBin(Momentum, MomBin, NumMomBins + 1); }
unsigned CATS::GetIpBin(const double &bVal) const { return GetBin(bVal, IpBin, NumIpBins + 1); }
unsigned CATS::GetRadBin(const double &Radius, const unsigned &uMomBin, const unsigned short &usCh,
                         const unsigned short &usPW) const {
...
template <class Type>
Type CATS::EvalBinnedFun(const double &xVal, const unsigned &NumBins, const double *Bins, const double *BinCent,
    ...
    if (InterpolRange[1] - InterpolRange[0]) {
        return (FunRange[1] * (xVal - InterpolRange[0]) - FunRange[0] * (xVal - InterpolRange[1])) /
               (InterpolRange[1] - InterpolRange[0]);
    } else {
        printf("\033[1;33mWARNING:\033[0m EvalBinnedFun could not properly extrapolate! The output of the Eval "
...
double CATS::EvaluateThePotential(const unsigned short &usCh, const unsigned short &usPW, const double &Momentum,
...
CATSelder *CATS::GetTheElder(const double &Momentum) { return kSourceGrid[MomDepSource ? GetMomBin(Momentum) : 0]; }
