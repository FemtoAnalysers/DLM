...
void RootFile_DlmCk(const TString &RootFileName, const TString &GraphName, DLM_CkDecomposition *CkToPlot,
                    bool PlotIndividualContributions) {
    TFile *RootFile = new TFile(RootFileName, "update");
    if (!RootFile)
        RootFile = new TFile(RootFileName, "recreate");
    ...
    graph.Set(NumBins);
    for (unsigned uBin = 0; uBin < NumBins; uBin++) {
        graph.SetPoint(uBin, CkToPlot->GetCk()->GetBinCenter(0, uBin),
                       CkToPlot->EvalCk(CkToPlot->GetCk()->GetBinCenter(0, uBin)));
    }
    graph.Write("", TObject::kOverwrite);
...
void CATS_GaussSource(CATS &Kitty, const double &SourceSize) {
    ...
    // this is important, since if its `false` it is assumed that the source will be sampled from a transport model, and
    // the Gaussian function will not be used
    Kitty.SetUseAnalyticSource(true);
    // if true, the source is automatically renormalized in the range 0-64 fm. Nice to dummy proof the source, but
    // problematic for sources with large tails for the Gaussian example above, both options should be completely
    // identical
...
// fracreso gives the fraction of resonance feeding into your protons, taureso is the avg lifetime of the resonances,
// massreso is the avg mass the cutoff sets the limit for the maximum allowed relative momentum of the daughters
// (ideally it should be a small number, in reality c.a. 200 MeV works) the returned pointer is to the source object,
// that must not be deleted while the CATS object is used. You should delete it after that, to avoid memory leaks
DLM_CleverMcLevyResoTM *CATS_ResoSource_pp(CATS &Kitty, const double &SourceSize, const double &Alpha,
                                           const bool &FixAlpha, const double &fracreso, const double &taureso,
                                           const double &massreso, const double &cutoff) {
    ...
    // provided by the base class 'CatsSource', from which you should inherit (see the implementation of
    // DLM_CleverMcLevyResoTM as an example)
    ...
    // sets up a discrete grid size, on which only a finite amount of source evaluations will be performed to save CPU
    // time the source function will be extrapolated to the desired parameters.
    if (FixAlpha)
        CleverMcLevyResoTM_pp->InitStability(1, Alpha - 1e-6, Alpha + 1e-6); // only a single grid point for the Alpha
    else
        CleverMcLevyResoTM_pp->InitStability(21, 1, 2);
    // this sets the possible source sizes between 0.4 and 3 fm. Typically should provide good cover of all ranges, but
    // take care you do not go outside
    ...
    // this is a TNtuple containing the relevant information from your transport model, which is to be used to model the
    // kinematics. The variables are those used in eq.19 of the analysis note here:
    // https://alice-notes.web.cern.ch/node/891 if you do not have access there, write me an email N.B. ALL these
    // observables should be evaluated in the CM frame of the two final particle you study! To set up such an TNtuple:
    // be creative :) In case of difficulties (or if you need EPOS output) write me or Max an email and we can discuss.
    // we actually have two TNtuple, one for the case of primary-secondary protons, and one for secondary-secondary.
    // Note that for non-identical particles you will also have to consider separately secondary-primary.
    ...
    // this is where the magic happens. The idea is, that we give the source some amount of vectors corresponding to
    // s_res1/2 in fig. 20 of the analysis note the source will than build the source by random sampling the core and
    // putting on top some of these vectors, to evaluate the resulting separation between the daughters (final particles
    // of interest). We can either use the information fully from our TNtuple, i.e. take the information about Tau,
    // Mass, Momentum and angles from our transfer model, or use some averaged values. The latter is used since in EPOS
    // we often do not have all resonance we want, so we used some dummy resonances just to obtain a sample with the
    // correct average mass and kinematic properties, although the lifetimes could differ, and the individual masses
    // wrong.
    for (unsigned uEntry = 0; uEntry < N_EposDisto_p_pReso; uEntry++) {
        ...
        // plug in the result for the primordial-resonance case. For identical particle it is symmetric, apart the sign
        // of the cosine.vid
    ...
    if (FixAlpha && (Alpha == 1 || Alpha == 2))
        CleverMcLevyResoTM_pp->InitNumMcIter(1000000);
    else
        CleverMcLevyResoTM_pp->InitNumMcIter(100000);
    ...
    Kitty.SetUseAnalyticSource(true);
    // if true, the source is automatically renormalized in the range 0-64 fm. Nice to dummy proof the source, but
    // problematic for sources with large tails For the core+reso case we could easily have large tails, so this is
    // quite important to set as false
    Kitty.SetAutoNormSource(false);
...
void CATS_pp_AV18(CATS &Kitty, const bool &pwaves, const bool &dwaves) {
    ...
    // the main idea is to always pass the function fDlmPot but with different input parameters, based on which the
    // interaction is set up automatically this works not only for pp, but many other systems are included. The fDlmPot
    // is only used as an interface to easily use different potentials, that are hard coded in DLM_Potentials.h Feel
    // free to expand the data base of this file if you think others will benefit from it. For further details contact
    // Dimi The input parameters are by default 9 and are defined as follows: 0: potential flag (defines which potential
    // to use, see the enumerators in DLM_Potentials.h for more info) 1: a second flag, that can be used if needed
    // (depending on the definition of the potential) 2: total isospin 3: 2 x isospin of particle 1 4: 2 x isospin of
    // particle 2 5: total spin 6: l quantum number 7: j quantum number
    CATSparameters cPars_pp_1S0(CATSparameters::tPotential, 8, true);
    cPars_pp_1S0.SetParameter(0, NN_ReidV8); // choose the AV18
    cPars_pp_1S0.SetParameter(
        1, v18_Coupled3P2); // default option, which takes the 3P2 channel from a coupled-channel computation, but in
                            // CATS only the first diagonal potential elements is used
...
double ExampleFitter_pp(double *x, double *par) {
    ...
    // it can be additionally set by calling the relevant contributions, e.g. to set the radius of pL->pp one should
    // use: Ck_ExampleFitter_pp->GetContribution("pLambda")->GetCk()->SetSourcePar(0,par[0]); N.B. (2): the SetSourcePar
    // functions assumes we do not significantly change the radius, and when passing this argument into CATS it forces
    // CATS to use the computing grid that is already set (saves a LOT of time). But will only work for source size
    // variations of up to 20-30% (50% if you push it) before running into possible numerical bias. So make sure your
    // starting fit values are reasonable
...
void Ck_pp_Decomposition(const TString &SourceType) {
    ...
    if (SourceType != "Gauss" && SourceType != "CoreReso") {
    ...
    if (SourceType == "Gauss")
        CATS_GaussSource(Kitty_pp_s, SourceSize);
    // initialize a Gaussian core + reso source
    DLM_CleverMcLevyResoTM *CRS_pp = NULL;
    if (SourceType == "CoreReso")
        CRS_pp = CATS_ResoSource_pp(Kitty_pp_s, SourceSize, 2, true, 0.6422, 1.65, 1362, 200);
    ...
    if (SourceType == "CoreReso")
        CRS_pp_sp = CATS_ResoSource_pp(Kitty_pp_sp, SourceSize, 2, true, 0.6422, 1.65, 1362, 200);
    ...
    if (SourceType == "CoreReso")
        CRS_pp_spd = CATS_ResoSource_pp(Kitty_pp_spd, SourceSize, 2, true, 0.6422, 1.65, 1362, 200);
    ...
    // this is done using the class DLM_CkDecomposition. To set it up, we need the momentum smearing matrix, as well as
    // any residual transformation matrices In short: this object can smear the DLM_Ck histogram, and build the
    // correlation function with the inclusion of feed-down channels. The feed-downs are transfered recursively, i.e.
    // say you define for pL a feed down from pXi and for pp feed down for pL, than pXi->pL->pp is automatically
    // included
    ...
    // the object is initialized by: a name of your choice, number of contributions to C(k) APART from the primary,
    // DLM_Ck, resolution matrix
    ...
    // #WhichContribution,#Type(cFeedDown or cFake),OPTIONALLY: DLM_CkDecomposition of the feed-down contribution,
    // transformation matrix if the last two entries are not given, it is assumed that the feed-down correlation is flat
    ...
    printf(" chi2/ndf = %.2f/%i = %.2f\n", fit_pp->GetChisquare(), fit_pp->GetNDF(),
           fit_pp->GetChisquare() / double(fit_pp->GetNDF()));
