...
void TreParticle::SetPtEtaPhi(const DLM_Histo<float> &pdf) {
    if (pdf.GetDim() > 3) {
        ...
        if (Database.PrintLevel >= 1 && ShowMessage) {
            printf("\033[1;31mERROR:\033[0m (TreParticle::SetPtEtaPhi) The momentum distribution must have 1,2 or 3 "
                   "dimensions\n");
        }
        return;
    }
...
void TreParticle::SetPtotEtaPhi(const DLM_Histo<float> &pdf) {
    if (pdf.GetDim() > 3) {
        printf("\033[1;31mERROR:\033[0m (TreParticle::SetPtotEtaPhi) The momentum distribution must have 1,2 or 3 "
               "dimensions\n");
        return;
    }
    if (PtotEtaPhi) {
...
void TreParticle::SetPz(const float &widthZ) {
    if (PtEtaPhi == NULL && PtPzPhi == NULL) {
        printf("\033[1;31mERROR:\033[0m (TreParticle::SetPz) The width of Pz can only be set in case we have ONLY set "
               "the pT distribution.\n");
        return;
    }
    if (PtEtaPhi) {
        if (PtEtaPhi->GetDim() != 1) {
            printf("\033[1;31mERROR:\033[0m (TreParticle::SetPz) The width of Pz can only be set in case we have ONLY "
                   "set the pT distribution.\n");
            return;
        }
    }
    if (PtPzPhi) {
...
//   we sample all if available, if phi is missing => flat, if eta is missing we sample from flat cos_theta and evaluate
//   eta this way
    ...
    if (PtEtaPhi) {
        ...
        while (InAccepance == false) {
            ...
            if (Auto_eta) {
                if (Pz_Width == 0) {
                    printf(
                        "\033[1;33mWARNING:\033[0m (TreParticle::SamplePxPyPz) Pz=0, did you forgot to use SetPz?\n");
                }
            ...
            InAccepance = true;
            if (pt < Acceptance_pT[0] || pt > Acceptance_pT[1])
                InAccepance = false;
            if (eta < Acceptance_Eta[0] || eta > Acceptance_Eta[1])
                InAccepance = false;
            if (cos_th < Acceptance_CosTh[0] || cos_th > Acceptance_CosTh[1])
                InAccepance = false;
            if (phi < Acceptance_Phi[0] || phi > Acceptance_Phi[1])
                InAccepance = false;
        ...
        do {
            ...
            ptot = sqrt(pow(RanGen->Gauss(0, Px_Width), 2.) + pow(RanGen->Gauss(0, Py_Width), 2.) +
                        pow(RanGen->Gauss(0, Pz_Width), 2.));
            pt = ptot * sin_th;
...
double TreParticle::AcceptanceMin_pT() const { return Acceptance_pT[0]; }
double TreParticle::AcceptanceMax_pT() const { return Acceptance_pT[1]; }
double TreParticle::AcceptanceMin_Eta() const { return Acceptance_Eta[0]; }
double TreParticle::AcceptanceMax_Eta() const { return Acceptance_Eta[1]; }
double TreParticle::AcceptanceMin_CosTh() const { return Acceptance_CosTh[0]; }
double TreParticle::AcceptanceMax_CosTh() const { return Acceptance_CosTh[1]; }
double TreParticle::AcceptanceMin_Phi() const { return Acceptance_Phi[0]; }
double TreParticle::AcceptanceMax_Phi() const { return Acceptance_Phi[1]; }

DLM_Histo<float> *TreParticle::GetPtEtaPhi() const { return PtEtaPhi; }
DLM_Histo<float> *TreParticle::GetPtotEtaPhi() const { return PtotEtaPhi; }
DLM_Histo<float> *TreParticle::GetPtPzPhi() const { return PtPzPhi; }
...
std::string TreParticle::GetName() const { return TreName; }

float TreParticle::GetMass() const { return Mass[1]; }

float TreParticle::GetMassLow() const { return Mass[0]; }

float TreParticle::GetMassUp() const { return Mass[2]; }

float TreParticle::GetWidth() const { return Width[1]; }

float TreParticle::GetWidthLow() const { return Width[0]; }

float TreParticle::GetWidthUp() const { return Width[2]; }
...
float TreParticle::GetAbundanceLow() const { return Abundance[0]; }

float TreParticle::GetAbundanceUp() const { return Abundance[2]; }
...
float TreParticle::GetRadiusLow() const { return Radius[0]; }

float TreParticle::GetRadiusUp() const { return Radius[2]; }
...
float TreParticle::GetRadiusSlopeLow() const { return RadSlope[0]; }

float TreParticle::GetRadiusSlopeUp() const { return RadSlope[2]; }

float TreParticle::GetDelayTau() const { return DelayTau[1]; }
float TreParticle::GetDelayTauLow() const { return DelayTau[0]; }
float TreParticle::GetDelayTauUp() const { return DelayTau[2]; }

unsigned char TreParticle::GetNumDecays() const { return NumDecays; }

const TREPNI *TreParticle::GetDatabase() const { return &Database; }
...
float TreChain::GetBranching() const { return Branching[1]; }

unsigned char TreChain::GetNumDaughters() const { return NumDaughters; }
const TreParticle *TreChain::GetMother() const { return &Mother; }
const TreParticle *TreChain::GetDaughter(const unsigned char &whichone) const {
    if (whichone >= NumDaughters)
        return NULL;
...
TREPNI::TREPNI(const unsigned short &version)
    : Len_DtbsName(32), Len_PrtclName(24), Version(version), MaxMemSteps(1024), NumFunctions(64) {
    ...
    for (short us = 0; us < NumFunctions; us++)
        ErrorOccured[us] = 0;
    // QA_passed = false;
    TotAbundance = 0;
    ...
    for (unsigned uPart = 0; uPart < NumParticles; uPart++) {
        for (unsigned char uDec = 0; uDec < Particle[uPart]->NumDecays; uDec++) {
            if (Particle[uPart]->Decay[uDec]->NumDaughters < 2) {
                static bool ShowMessage = true;
                if (PrintLevel >= 1 && ShowMessage) {
                    printf("\033[1;31mERROR:\033[0m (TREPNI::QA) The particle '%s' has a decay channel with less than "
                           "2 daughters\n",
                           Particle[uPart]->TreName);
                    if (SingleError)
                        ShowMessage = false;
...
bool TREPNI::QA_Abundance() const {
    ...
    if (LowAbund > TotAbundance || UpAbund < TotAbundance) {
        ...
        if (PrintLevel >= 1 && ShowMessage) {
            printf(
                "\033[1;31mERROR:\033[0m (TREPNI::QA) The total yield of all particles is outside the allowed limit\n");
            if (SingleError)
                ShowMessage = false;
        }
        AllesGut = false;
...
void TREPNI::SetTotalYield(const float &totyield) { TotAbundance = totyield; }
...
TreParticle *TREPNI::NewParticle(const char *name) {
    ...
    if (name) {
        Particle[NumParticles]->SetName(name);
    }
    return Particle[NumParticles++];
}

TreParticle *TREPNI::GetParticle(const unsigned &whichone) const { return Particle[whichone]; }
...
TreParticle *TREPNI::GetParticle(const std::string &name) const { return GetParticle(name.c_str()); }
...
unsigned TREPNI::GetNumParticles() const { return NumParticles; }
...
void TREPNI::SetSeed(const unsigned &seed) { RanGen->SetSeed(seed); }
...
std::string TREPNI::GetParticleName(const int& id) const{
  ...
  if(LineID>=NumParticles&&NumParticles){
    if(PrintLevel>=2){
      ...
      ErrorID=1;
      if( (ErrorOccured[getparticlename]&(1 << ErrorID))==0 ){
        printf("\033[1;33mWARNING:\033[0m (TREPNI::GetParticleName) There are only %u number of particles
defined\n",NumParticles);
        //occupy the bit in case we dont want to repeat this error
        if(SingleError){
          ErrorOccured[getparticlename] ^= (1 << ErrorID);
    ...
    if(PrintLevel>=1){
      ErrorID=2;
      if( (ErrorOccured[getparticlename]&(1 << ErrorID))==0 ){
        printf("\033[1;31mERROR:\033[0m (TREPNI::GetParticleName) There are only %u number of particles defined and %u
...
void TREPNI::SetParticle(const char* name, const double& mass_min, const double& mass_max,
  ...
  if(mass_max<mass_min){
    if(PrintLevel>=1){
      ErrorID=4;
      if( (ErrorOccured[setparticle]&(1 << ErrorID))==0 ){
        printf("\033[1;31mERROR:\033[0m (TREPNI::SetParticle) The lower limit of the mass is larger than the upper
limit\n"); if(SingleError) ErrorOccured[setparticle] ^= (1 << ErrorID);
      }
    }
    return;
  ...
  if(gamma_max<gamma_min){
    if(PrintLevel>=1){
      ErrorID=6;
      if( (ErrorOccured[setparticle]&(1 << ErrorID))==0 ){
        printf("\033[1;31mERROR:\033[0m (TREPNI::SetParticle) The lower limit of the width is larger than the upper
limit\n"); if(SingleError) ErrorOccured[setparticle] ^= (1 << ErrorID);
      }
    }
    return;
