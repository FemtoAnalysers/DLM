...
class CECA {
  public:
    ...
    // if false, the time component is updated based on the distance traveled (hadr component), given the particle
    // momentum,
    void SetFixedHadr(const bool &yesno);
    // if 0 (default), the fragments from which the particles are formed travel with the same velocity as the particles
    // themselves if the value is between 0 and 1, it corresponds to the beta of the fragments (constant) This affects
    // both the Tau parameter, as well as Hadronization
