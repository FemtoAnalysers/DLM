...
const TreParticle *CecaParticle::Trepni() const { return trepni; }
CatsParticle *CecaParticle::Cats() const { return cats; }
const TreChain *CecaParticle::Decay() const { return decay; }
CatsParticle *CecaParticle::Mother() const { return mother; }
void CecaParticle::SetTrepni(const TreParticle &prt_tre) { trepni = &prt_tre; }
void CecaParticle::SetCats(const CatsParticle &prt_cats) { *cats = prt_cats; }
void CecaParticle::SetDecay(const TreChain &prt_dec) { decay = &prt_dec; }
...
void CecaParticle::SetOrigin(const char &origin) { Origin = origin; }
...
bool CecaParticle::WithinAcceptance() const {
    if (cats->GetPt() < trepni->AcceptanceMin_pT())
        return false;
    if (cats->GetPt() > trepni->AcceptanceMax_pT())
        return false;
    if (cats->GetPseudoRap() < trepni->AcceptanceMin_Eta())
        return false;
    if (cats->GetPseudoRap() > trepni->AcceptanceMax_Eta())
        return false;
    if (cats->GetPphi() < trepni->AcceptanceMin_Phi())
        return false;
    if (cats->GetPphi() > trepni->AcceptanceMax_Phi())
        return false;
    return true;
}
...
CECA::~CECA() {
    ...
    if (Ghetto_kstar_rstar) {
        delete Ghetto_kstar_rstar;
        Ghetto_kstar_rstar = NULL;
    }
    if (Ghetto_kstar_rstar_PP) {
        delete Ghetto_kstar_rstar_PP;
        Ghetto_kstar_rstar_PP = NULL;
    }
    if (Ghetto_kstar_rstar_PR) {
        delete Ghetto_kstar_rstar_PR;
        Ghetto_kstar_rstar_PR = NULL;
    }
    if (Ghetto_kstar_rstar_RP) {
        delete Ghetto_kstar_rstar_RP;
        Ghetto_kstar_rstar_RP = NULL;
    }
    if (Ghetto_kstar_rstar_RR) {
        delete Ghetto_kstar_rstar_RR;
        Ghetto_kstar_rstar_RR = NULL;
    }
    if (Ghetto_kstar_rstar_mT) {
        delete Ghetto_kstar_rstar_mT;
        Ghetto_kstar_rstar_mT = NULL;
    }
    if (Ghetto_kstar_rcore_mT) {
        delete Ghetto_kstar_rcore_mT;
        Ghetto_kstar_rcore_mT = NULL;
    }
    if (Ghetto_kstar_reso_mT) {
        delete Ghetto_kstar_reso_mT;
        Ghetto_kstar_reso_mT = NULL;
    }
    if (Ghetto_mT_rstar) {
        delete Ghetto_mT_rstar;
        Ghetto_mT_rstar = NULL;
    }
    ...
    if (GhettoFemto_rcore) {
        delete GhettoFemto_rcore;
        GhettoFemto_rcore = NULL;
    }
    if (Ghetto_mT_costh) {
        delete Ghetto_mT_costh;
        Ghetto_mT_costh = NULL;
    }
    if (GhettoSP_pT_th) {
        delete GhettoSP_pT_th;
        GhettoSP_pT_th = NULL;
    }
    if (GhettoSP_pT_1) {
        delete GhettoSP_pT_1;
        GhettoSP_pT_1 = NULL;
    }
    if (GhettoSP_pT_2) {
        delete GhettoSP_pT_2;
        GhettoSP_pT_2 = NULL;
    }
    if (GhettoSPr_X) {
        delete GhettoSPr_X;
    ...
    if (Ghetto_RP_AngleRcP1) {
        delete Ghetto_RP_AngleRcP1;
        Ghetto_RP_AngleRcP1 = NULL;
    }
    if (Ghetto_PR_AngleRcP2) {
        delete Ghetto_PR_AngleRcP2;
        Ghetto_PR_AngleRcP2 = NULL;
    }
    if (Ghetto_RR_AngleRcP1) {
        delete Ghetto_RR_AngleRcP1;
        Ghetto_RR_AngleRcP1 = NULL;
    }
    if (Ghetto_RR_AngleRcP2) {
        delete Ghetto_RR_AngleRcP2;
        Ghetto_RR_AngleRcP2 = NULL;
    }
    if (Ghetto_RR_AngleP1P2) {
        delete Ghetto_RR_AngleP1P2;
        Ghetto_RR_AngleP1P2 = NULL;
    }
    if (Ghetto_ScatteringAngle) {
        delete Ghetto_ScatteringAngle;
        Ghetto_ScatteringAngle = NULL;
    }
    if (Ghetto_PP_AngleRcP1) {
        delete Ghetto_PP_AngleRcP1;
        Ghetto_PP_AngleRcP1 = NULL;
    }
    if (Ghetto_PP_AngleRcP2) {
        delete Ghetto_PP_AngleRcP2;
        Ghetto_PP_AngleRcP2 = NULL;
    }
    if (Ghetto_PP_AngleP1P2) {
        delete Ghetto_PP_AngleP1P2;
        Ghetto_PP_AngleP1P2 = NULL;
    }
    if (GhettoFemto_mT_rstar) {
        delete GhettoFemto_mT_rstar;
        GhettoFemto_mT_rstar = NULL;
    }
    if (GhettoFemto_mT_rcore) {
        delete GhettoFemto_mT_rcore;
        GhettoFemto_mT_rcore = NULL;
    }
    if (GhettoFemto_mT_kstar) {
        delete GhettoFemto_mT_kstar;
        GhettoFemto_mT_kstar = NULL;
    }
    if (GhettoFemtoPrimordial_mT_kstar) {
        delete GhettoFemtoPrimordial_mT_kstar;
        GhettoFemtoPrimordial_mT_kstar = NULL;
    }
    if (GhettoFemto_pT1_pT2) {
        delete GhettoFemto_pT1_pT2;
        GhettoFemto_pT1_pT2 = NULL;
    }
    if (GhettoFemto_pT1_div_pT) {
        delete GhettoFemto_pT1_div_pT;
        GhettoFemto_pT1_div_pT = NULL;
    }
    if (Ghetto_mT_mTwrong) {
        delete Ghetto_mT_mTwrong;
        Ghetto_mT_mTwrong = NULL;
    }
    if (GhettoFemto_mT_mTwrong) {
        delete GhettoFemto_mT_mTwrong;
        GhettoFemto_mT_mTwrong = NULL;
    }
    if (Ghetto_MtBins) {
        delete[] Ghetto_MtBins;
        Ghetto_MtBins = NULL;
    }
    if (ThreadClock) {
        delete[] ThreadClock;
...
float CECA::GetDisplacementX() const { return Displacement[0]; }

void CECA::SetFixedHadr(const bool &yesno) { FixedHadr = yesno; }
...
float CECA::GetDisplacementY() const { return Displacement[1]; }
...
float CECA::GetDisplacementZ() const { return Displacement[2]; }
...
float CECA::GetDisplacement() const {
    return sqrt(Displacement[0] * Displacement[0] + Displacement[1] * Displacement[1] +
...
void CECA::SetTauEbe(const float &fwidth) { TauEbe = fwidth; }
...
float CECA::GetHadronizationX() const { return Hadronization[0]; }
...
float CECA::GetHadronizationY() const { return Hadronization[1]; }
...
float CECA::GetHadronizationZ() const { return Hadronization[2]; }
...
float CECA::GetHadronization() const {
    return sqrt(Hadronization[0] * Hadronization[0] + Hadronization[1] * Hadronization[1] +
                Hadronization[2] * Hadronization[2]) /
           sqrt(3.);
}

void CECA::SetHadrFluctuation(const float &fluct) { HadrFluct = fluct; }
float CECA::GetHadrFluctuation() const { return HadrFluct; }
void CECA::SetPropagateMother(const bool &yesno) { PropagateMother = yesno; }
...
float CECA::GetTau() const { return Tau; }

void CECA::SetThermalKick(const float &kick) { ThermalKick = kick; }
...
void CECA::SetTargetStatistics(const unsigned long long &yield) { TargetYield = yield; }

unsigned long long CECA::GetStatistics() { return AchievedYield; }
...
double CECA::GetFemtoLimit() { return FemtoLimit; }

void CECA::SetEventMult(const unsigned short &emult) { EMULT = emult; }
...
void CECA::SetDebugMode(const bool &debugmode) { DebugMode = debugmode; }
void CECA::SetThreadTimeout(const unsigned &seconds) {
    Timeout = seconds ? seconds : 1; // a minimum of 1 second
}
void CECA::SetGlobalTimeout(const unsigned &seconds) { GlobalTimeout = seconds ? seconds : -1; }
void CECA::SetSeed(const unsigned &thread, const unsigned &seed) {
    if (thread >= MaxThreads)
        return;
    RanGen[thread]->SetSeed(seed);
}
void CECA::EqualizeFsiTime(const bool &yesno) { EqualFsiTau = yesno; }
...
unsigned CECA::GoSingleCore(const unsigned &ThId) {
    ...
    if (DebugMode) {
        // #pragma omp critical
        //{
        //   printf(" -> The exectution finished after %u calls, generating %u
        //   multiplets\n",DebugCounter,NumMultiplets); printf("\n");
        // }
    }
...
void CECA::GoBabyGo(const unsigned &num_threads) {
    ...
    if (num_threads == 0) {
        ...
        if (NumThreads > MaxThreads) {
            printf("\033[1;33mWARNING:\033[0m CECA::GoBabyGo says you demand more threads (%u) than available on your "
                   "system (%u).\n",
                   NumThreads, MaxThreads);
...
#pragma omp parallel for
        ...
        if (DebugMode) {
            ...
            if (TargetYield < 10 * 1000)
                printf(" Achieved/Target Yield = %llu / %llu @ %s\n", AchievedYield, TargetYield, strtime);
            else if (TargetYield < 10 * 1000 * 1000)
                printf(" Achieved/Target Yield = %llu / %llu k @ %s\n", AchievedYield / 1000, TargetYield / 1000,
                       strtime);
            else
                printf(" Achieved/Target Yield = %llu / %llu M @ %s\n", AchievedYield / 1000000, TargetYield / 1000000,
                       strtime);
...
bool CECA::ParticleInList(const TreParticle *prt) const { return ParticleInList(prt->GetName()); }
...
unsigned CECA::GenerateEvent(const unsigned &ThId) {
    ...
    while (true) {
        ...
        for (unsigned short uMult = 0; uMult < EMULT; uMult++) {
            ...
            if (Primordial.back()->Trepni()->GetNumDecays()) {
                ...
                if (FSP_is_primordial) {
                    printf("\033[1;33mWARNING:\033[0m (CECA::GenerateEvent) The final state particle '%s' has a decay "
                           "channel. Ignoring!\n",
        ...
        // this is WRONG, as we do not search the full chain. particle_list is only the list with multiplets at final
        // state
                ...
                if (particle->Trepni()->GetName() == Needed) {
                    ...
                    for (unsigned char uDaugh = 0; uDaugh < ndaugh; uDaugh++) {
                        if (particle->Decay()->GetDaughter(uDaugh)->GetName() == particle_list.at(iNeeded)) {
                            ...
                            particle_list.pop_back();
                            FoundIt = true;
                            // when upgrading, be careful here, we may have multiple particles of interest in the final
                            // state also, worth checking if we end up having two identical particles produced from the
                            // same decay, these should be taken with care (source will be zero...)
                            break;
                        }
                    }
                }
    ...
    for (CecaParticle *primordial : Primordial) {
        ...
        // and that direction of velocity is what determines the "crossing point" of the particle with the emission
        // source
        double rh_len = -1;
        int ResampleCount = 0;
        ...
        while (true) {
            ...
            if (ResampleCount == 0) {
                for (int xyz = 0; xyz < 3; xyz++) {
                    ...
                    rh[xyz] = RanGen[ThId]->Gauss((Hadronization[xyz] + hadr_ebe[xyz]),
                                                  (Hadronization[xyz] + hadr_ebe[xyz]) * HadrFluct);
                }
                ...
                rh_len = sqrt(pow(rh[0] * sin_th * cos_phi, 2.) + pow(rh[1] * sin_th * sin_phi, 2.) +
                              pow(rh[2] * cos_th, 2.));
                if (TauFluctuation < 0)
                    tau = RanGen[ThId]->Exp(fabs(Tau + tau_ebe));
                else
                    tau = (Tau + tau_ebe) + RanGen[ThId]->Gauss(0, (Tau + tau_ebe) * TauFluctuation);
                if (ProperTau)
                    tau *= primordial->Cats()->Gamma();
            }
            ...
            // the probability with which it has been rejected (i.e. resampling is largest for particles that really are
            ...
            */
            ...
            for (int xyz = 0; xyz < 3; xyz++) {
                ...
                if (FragmentBeta) {
                    if (beta_tot)
                        rtot[xyz] += beta[xyz] * tau * (FragmentBeta / beta_tot);
                } else
            ...
            if (!FixedHadr) {
                if (FragmentBeta)
                    tau += rh_len * (FragmentBeta / beta_tot);
                else
                    tau += rh_len;
            }
            ...
            for (CecaParticle *primordial2 : Primordial) {
                ...
                for (int ip = 0; ip < 2; ip++) {
                    // we need the effective distance between the two particles, that is modulated based on the lorentz
                    // contaction to do that, we find the projection of the unity vector of the primordial onto the
                    // radius vector connecting the two particles. The length of the projection is the weight with which
                    // the contracted radius is taken
            ...
            if (ResampleCount >= ResampleLimit) {
                printf("\033[1;33mWARNING:\033[0m CECA::GenerateEvent says that it cannot separate the particles at "
                       "the set requirement.\n");
                printf("   RejectProb = %e\n", RejectProb);
                printf("   To solve the issue, verify there is enought displacement and the particle radius is not too "
                       "large.\n");
                break;
            }
            ResampleCount++;
        ...
        if (IsResonance) {
            CatsParticle *daughters =
                primordial->Cats()->Decay(primordial->Decay()->GetDaughterMasses(), PropagateMother);
    ...
    for (std::vector<unsigned> pid : Permutations) {
        ...
        */
        ...
        for (unsigned ID : pid) {
            ...
            if (Primary.at(ID)->IsUseful() == false) {
        ...
        for (unsigned char ud = 0; ud < SDIM; ud++) {
            ...
            if (prt_cm[ud].Mother())
                prt_cm[ud].Mother()->Boost(boost_v);
        ...
        if (SDIM == 3) {
            ...
            std::vector<double> v_p1 = {prt_cm[0].Cats()->GetPx(), prt_cm[0].Cats()->GetPy(),
                                        prt_cm[0].Cats()->GetPz()};
            std::vector<double> v_p2 = {prt_cm[1].Cats()->GetPx(), prt_cm[1].Cats()->GetPy(),
                                        prt_cm[1].Cats()->GetPz()};
            std::vector<double> v_p3 = {prt_cm[2].Cats()->GetPx(), prt_cm[2].Cats()->GetPy(),
                                        prt_cm[2].Cats()->GetPz()};
            ...
            // for(unsigned uv=0; uv<3; uv++){
            //   dot_k12_k3_12 += (m2*v_p1.at(uv) - m1*v_p2.at(uv))/(m1+m2) *
            //   ((m1+m2)*v_p3.at(uv)-m3*(v_p1.at(uv)+v_p2.at(uv)))/mtot;
            // }
            ...
            if (Q3 < 1000) {
                // if(hyp_rad<0.1){
                // printf("hyp_rad, rho_val, ratio, Q3, mT = %.3f %.3f %.3f %.3f
...
#pragma omp critical
        {
            ...
            if (SDIM == 2) {
                ...
                if (klab < 100) {
                    // printf("kstar_10 = (%.2f, %.2f, %.2f)
                    // %.2f\n",cm_rel.GetPx()*0.5,cm_rel.GetPy()*0.5,cm_rel.GetPz()*0.5,cm_rel.GetP()*0.5); printf("k0 =
                    // (%.2f, %.2f, %.2f)
                    ...
                    ////printf("klab = (%.2f, %.2f, %.2f)
                    ///%.2f\n",lab_rel.GetPx()*0.5,lab_rel.GetPy()*0.5,lab_rel.GetPz()*0.5,lab_rel.GetP()*0.5);
                    // printf("rstar = (%.2f, %.2f, %.2f)
                    // %.2f\n",cm_rel.GetX(),cm_rel.GetY(),cm_rel.GetZ(),cm_rel.GetR()); printf("r0 = (%.2f, %.2f, %.2f)
                ...
                if (kstar < FemtoLimit) {
                    ...
                    if (prt_cm[0].IsUsefulPrimordial() && prt_cm[1].IsUsefulPrimordial()) {
                        ...
                        if (prt_cm[0].Trepni()->GetName() == ListOfParticles.at(0)) {
                            ...
                            if (SetUp_RSM) {
                                BGT = RanGen[ThId]->Exponential(prt_cm[0].Mother()->GetWidth() *
                                                                prt_cm[0].Mother()->GetMass() /
                                                                prt_cm[0].Mother()->GetP()) *
                                      hbarc;
                        ...
                        if (prt_cm[1].Trepni()->GetName() == ListOfParticles.at(1)) {
                            ...
                            if (SetUp_RSM) {
                                BGT = RanGen[ThId]->Exponential(prt_cm[1].Mother()->GetWidth() *
                                                                prt_cm[1].Mother()->GetMass() /
                                                                prt_cm[1].Mother()->GetP()) *
                                      hbarc;
                        ...
                        if (prt_cm[0].Trepni()->GetName() == ListOfParticles.at(0) &&
                            ...
                            if (SetUp_RSM) {
                                BGT1 = RanGen[ThId]->Exponential(prt_cm[0].Mother()->GetWidth() *
                                                                 prt_cm[0].Mother()->GetMass() /
                                                                 prt_cm[0].Mother()->GetP()) *
                                       hbarc;
                                BGT2 = RanGen[ThId]->Exponential(prt_cm[1].Mother()->GetWidth() *
                                                                 prt_cm[1].Mother()->GetMass() /
                                                                 prt_cm[1].Mother()->GetP()) *
                                       hbarc;
                            ...
                            if (SetUp_RSM_UNI) {
                                ...
                                clvp1.SetTXYZ(0, RanGen[ThId]->Gauss(0, 1), RanGen[ThId]->Gauss(0, 1),
                                              RanGen[ThId]->Gauss(0, 1));
                                clvp2.SetTXYZ(0, RanGen[ThId]->Gauss(0, 1), RanGen[ThId]->Gauss(0, 1),
                                              RanGen[ThId]->Gauss(0, 1));
                                SetUp_RSM->AddBGT_RR(BGT1, clvrc.GetCosAngleR(clvp1), BGT2, clvrc.GetCosAngleR(clvp2),
                                                     clvp1.GetCosAngleR(clvp2));
                            }
                            if (SetUp_RSM_BB) {
                                BGT1 = RanGen[ThId]->Exponential(prt_cm[0].Mother()->GetWidth() *
                                                                 prt_cm[0].Mother()->GetMass() /
                        ...
                        if (prt_cm[0].Trepni()->GetName() == ListOfParticles.at(1) &&
                            ...
                            Ghetto_RR_AngleP1P2->Fill(AngleP1P2);
                            if (SetUp_RSM) {
                                BGT1 = RanGen[ThId]->Exponential(prt_cm[1].Mother()->GetWidth() *
                                                                 prt_cm[1].Mother()->GetMass() /
                                                                 prt_cm[1].Mother()->GetP()) *
                                       hbarc;
                                BGT2 = RanGen[ThId]->Exponential(prt_cm[0].Mother()->GetWidth() *
                                                                 prt_cm[0].Mother()->GetMass() /
                                                                 prt_cm[0].Mother()->GetP()) *
                                       hbarc;
                                SetUp_RSM->AddBGT_RR(BGT1, cos(Pi - AngleRcP2), BGT2, cos(Pi - AngleRcP1),
                                                     cos(AngleP1P2));
                            }
                            if (SetUp_RSM_UNI) {
                                BGT1 = RanGen[ThId]->Exponential(prt_cm[0].Mother()->GetWidth() *
                    ...
                    if (exp_file_flag) {
                        ...
                        if (exp_file_flag == 2) {
                            fprintf(file_ptr, "%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\n", kstar, rstar, mT,
                                    sqrt(pow(Displacement[0] + disp_ebe[0], 2.) +
                                         pow(Displacement[1] + disp_ebe[1], 2.) +
                                         pow(Displacement[2] + disp_ebe[2], 2.)) /
                                        sqrt(3.),
                                    sqrt(pow(Hadronization[0] + hadr_ebe[0], 2.) +
                                         pow(Hadronization[1] + hadr_ebe[1], 2.)) /
                                        sqrt(2.),
                                    Tau + tau_ebe);
                        }
                        // fprintf(file_ptr, " kstar\t\trstar\t\tcos_th_star\t\tmT\t\tPR1\t\tPR2\n");
                        else if (exp_file_flag == 10) {
                            fprintf(file_ptr, "%.5e\t%.5e\t%.5e\t%.5e\t%i\t%i\n", kstar, rstar, cos_th_star, mT,
                                    !prt_cm[0].IsUsefulPrimordial(), !prt_cm[1].IsUsefulPrimordial());
                        } else {
                            fprintf(file_ptr, "%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\t%.5e\n", kstar,
                                    rstar, mT, Displacement[0], Displacement[1], Displacement[2], Hadronization[0],
                                    Hadronization[1], Hadronization[2], Tau);
                        }
                ...
                if (kstar < FemtoLimit) {
                    ...
                    // printf("FFS 1\n");
                    // N.B. without this if statement, we get a different rcore, as it depends on the mass of the
                    // particles i.e. the core is not the same for all species, even if the single particle emission IS
                    // the same
                    if (prt_cm[0].IsUsefulPrimordial() && prt_cm[1].IsUsefulPrimordial()) {
                        if (mT >= Ghetto_MinMt && mT <= Ghetto_MaxMt) {
...
#pragma omp parallel for
    for (unsigned uPair = 0; uPair < NumPairs; uPair++) {
        ...
        */
        ...
        // Particle1.Print();
        if (!prim1)
            Daughter = Particle1.Decay(M_proton, M_pi, false);
        ...
        // Particle2.Print();
        if (!prim2)
            Daughter = Particle2.Decay(M_proton, M_pi, false);
        ...
        */
        ...
        rcore = sqrt(pow(Particle1.GetX() - Particle2.GetX(), 2.) + pow(Particle1.GetY() - Particle2.GetY(), 2.) +
        ...
        double CosP1P2 = (Particle1.GetPx() * Particle2.GetPx() + Particle1.GetPy() * Particle2.GetPy() +
                          Particle1.GetPz() * Particle2.GetPz()) /
...
#pragma omp critical
            {
                ...
                double Angle;
                Angle = acos(CosRcP1);
                Old_RcP1->AddAt(&Angle);
                Angle = acos(CosRcP2);
                Old_RcP2->AddAt(&Angle);
                if (!prim1 && !prim2) {
                    Angle = acos(CosP1P2);
                    Old_P1P2->AddAt(&Angle);
                }
            }
        }
