...
class TreParticle {
  ...
  private:
    ...
    float *Width;
    float *Abundance;
    // the idea is that each particle has a size, and eventually no particles within its radius will be allowed to be
    // produced at first this will include ANY particle, perhaps in the future think on level of quarks the blockadge
    // will follow fermi-dirac, with a mean Radius and slope RadSlope
    float *Radius;
    float *RadSlope;
    float *DelayTau;
