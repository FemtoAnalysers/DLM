...
double NewtonRapson(double (*FUN)(const double &, const double *), const double &xMin, const double &xMax,
    ...
    for (unsigned iIter = 0; iIter < maxIt; iIter++) {
        ...
        if (!DeltaF) {
            DeltaX = 1;
            {
                printf("\033[1;33mWARNING (DLM_RootFinder):\033[0m DeltaX is fishy with the NewtonRapson solver! "
                       "Please contact the developers!\n");
            }
        } else
            DeltaX = -fVal * EpsX / DeltaF;
        xVal += DeltaX;
        int counter = 0;
        ...
        while (fabs(fValNew) > fabs(fVal) && counter < 16) {
            ...
            if (xVal < xmin || xVal > xmax) {
                {
                    printf("\033[1;33mWARNING (DLM_RootFinder):\033[0m xVal is fishy with the NewtonRapson solver! "
                           "Please contact the developers!\n");
                }
            }
        ...
        if (counter == 16 && fabs(fValNew) > fabs(fVal)) {
            {
                printf("\033[1;33mWARNING (DLM_RootFinder):\033[0m The backtracking of the NewtonRapson root-finder "
                       "failed!\n");
            }
        }
