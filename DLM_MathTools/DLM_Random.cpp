...
int DLM_Random::Integer(const int &to) { return Integer(0, to); }
int DLM_Random::Int(const int &from, const int &to) { return Integer(from, to + 1); }
int DLM_Random::Int(const int &to) { return Integer(0, to + 1); }
double DLM_Random::Gauss(const double &mean, const double &sigma) { return (NormDist[0](*MT_RanGen)) * sigma + mean; }
double DLM_Random::Cauchy(const double &mean, const double &sigma) {
    return (CauchyDist[0](*MT_RanGen)) * sigma / sqrt(2.) + mean;
}
double DLM_Random::Exponential(const double &lambda) { return (ExpDist[0](*MT_RanGen)) / lambda; }
double DLM_Random::Exp(const double &mean) { return (ExpDist[0](*MT_RanGen)) * mean; }
double DLM_Random::Stable(const double &stability, const double &location, const double &scale,
    ...
    if (stability == 1) {
        ...
        return ((Pi * 0.5 + skewness * U) * tan(U)) / E -
               skewness * log((Pi * 0.5 * W * cos(U)) / (Pi * 0.5 + skewness * U)) * scale / sqrt(2) + location;
    } else {
        const double E = atan(-S) / stability;
        /*
        double RESULT =
        pow(1.+S*S,0.5/stability)*sin(stability*(U+E))/pow(cos(U),1./stability)*pow(cos(U-stability*(U+E))/W,(1.-stability)/stability)*scale/sqrt(2)+location;
        if(RESULT!=RESULT){
            printf("RESULT=%e\n",RESULT);
            printf(" S=%e\n",S);
            printf(" stability=%e\n",stability);
        ...
        */
        return pow(1. + S * S, 0.5 / stability) * sin(stability * (U + E)) / pow(cos(U), 1. / stability) *
                   pow(cos(U - stability * (U + E)) / W, (1. - stability) / stability) * scale / sqrt(2) +
               location;
    }
}
...
double DLM_Random::StableR(const unsigned short &dim, const double &stability, const double &location,
                           const double &scale, const double &skewness) {
    if (!dim)
        return 0;
    double Result = 0;
...
double DLM_Random::StableDiffR(const unsigned short &dim, const double &stability, const double &location,
                               const double &scale, const double &skewness) {
    // printf("stability=%f\n",stability);
    // printf("location=%f\n",location);
    // printf("scale=%f\n",scale);
...
double DLM_Random::StableNolan(const unsigned short &dim, const double &stability, const double &location,
                               const double &scale, const double &skewness) {
    return StableDiffR(dim, stability, location, scale * 0.5 * stability, skewness, stability, location,
                       scale * 0.5 * stability, skewness);
}
double DLM_Random::GaussDiffR(const unsigned short &dim, const double &mean1, const double &sigma1, const double &mean2,
                              const double &sigma2) {
    return StableDiffR(dim, 2, mean1, sigma1, 0, 2, mean2, sigma2, 0);
}
double DLM_Random::CauchyDiffR(const unsigned short &dim, const double &mean1, const double &sigma1,
                               const double &mean2, const double &sigma2) {
    return StableDiffR(dim, 1, mean1, sigma1, 0, 1, mean2, sigma2, 0);
}
double DLM_Random::StableDiffR(const unsigned short &dim, const double &stability1, const double &location1,
                               const double &scale1, const double &skewness1, const double &stability2,
                               const double &location2, const double &scale2, const double &skewness2) {
    ...
    for (unsigned short us = 0; us < dim; us++) {
        Result += pow(
            Stable(stability2, location2, scale2, skewness2) - Stable(stability1, location1, scale1, skewness1), 2.);
...
double DLM_Random::StableDiffR(const unsigned short &dim, const double *stability1, const double *location1,
    ...
    double Result = 0;
    for (unsigned short us = 0; us < dim; us++) {
        Result += pow(Stable(stability2[us], location2[us], scale2[us], skewness2[us] ? skewness2[us] : 0) -
                          Stable(stability1[us], location1[us], scale1[us], skewness1[us] ? skewness1[us] : 0),
                      2.);
    }
    return sqrt(Result);
}
double DLM_Random::StableDiffR(const unsigned short &dim, const double &stability1, const double *location1,
    ...
    double Result = 0;
    for (unsigned short us = 0; us < dim; us++) {
        Result += pow(Stable(stability2, location2[us], scale2[us], skewness2[us] ? skewness2[us] : 0) -
                          Stable(stability1, location1[us], scale1[us], skewness1[us] ? skewness1[us] : 0),
                      2.);
    }
    return sqrt(Result);
}
