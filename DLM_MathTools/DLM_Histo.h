...
template <class Type> class DLM_Histo1D {
  public:
    ...
    Type GetBinCenter(const unsigned &WhichBin) const {
        if (WhichBin >= NumBins)
            return 0;
        if (BinCenter[WhichBin] != 1.331e121)
            return BinCenter[WhichBin];
    ...
    Type Eval(const Type &xVal) const {
        if (xVal < BinRange[0] || xVal > BinRange[NumBins])
            return 0;
        if (NumBins == 1)
            return BinValue[0];
        unsigned WhichBin = GetBin(xVal);
    ...
    unsigned GetNbins() const { return NumBins; }
    ...
    bool Add(const DLM_Histo1D &other, const Type &Scale) {
        if (NumBins != other.NumBins)
            return false;
        for (unsigned uBin = 0; uBin <= NumBins; uBin++)
            if (BinRange[uBin] != other.BinRange[uBin])
                return false;
        for (unsigned uBin = 0; uBin < NumBins; uBin++)
            BinValue[uBin] += other.BinValue[uBin] * Scale;
        return true;
    ...
    void ScaleToBinWidth() {
        for (unsigned uBin = 0; uBin < NumBins; uBin++)
            BinValue[uBin] /= GetBinWidth(uBin);
    }

    bool operator=(const DLM_Histo1D &other) { return Copy(other, 1); }
    bool operator+=(const DLM_Histo1D &other) { return Add(other, 1); }
...
template <class Type> class DLM_Histo {
  public:
    ...
    void SetUp(const unsigned short &sDim, const unsigned &numbins, const double *bins,
        ...
        for (unsigned uBin = 0; uBin < numbins; uBin++) {
            if (bins[uBin + 1] <= bins[uBin]) {
                ...
                printf("\033[1;31mERROR:\033[0m DLM_Histo: The bin ranges should be in ascending order and a bin-width "
                       "of 0 is not allowed!\n");
                return;
            }
            // else
        ...
        if (!BinRange[sDim])
            BinRange[sDim] = new double[numbins + 1];
        if (!BinCenter[sDim])
            BinCenter[sDim] = new double[numbins];
    ...
    void SetUp(const unsigned short &sDim, const unsigned &numbins, const double &xmin, const double &xmax) {
        ...
        if (NumBins[sDim] == 1) {
            ...
            for (unsigned uBin = 0; uBin <= NumBins[sDim]; uBin++) {
                ...
                if (uBin)
                    BinCenter[sDim][uBin - 1] = (BinRange[sDim][uBin - 1] + BinRange[sDim][uBin]) * 0.5;
            }
        }
        Initialized = false;
        CumUpdated = false;
    ...
    bool Initialize(const bool &ZeroElements = true) {
        ...
        if (!Dim || !NumBins || !BinRange || !BinCenter)
            return false;
        if (!Initialized) {
            TotNumBins = 1;
            // printf("Initialize 2\n");
            ...
            // printf(" BinError = %p\n",BinError);
            for (unsigned short sDim = 0; sDim < Dim; sDim++) {
                // printf("sDim=%u; NumBins[sDim]=%u; BinRange[sDim]=%p;
                // BinCenter[sDim]=%p\n",sDim,NumBins[sDim],BinRange[sDim],BinCenter[sDim]);
                if (!NumBins[sDim] || !BinRange[sDim] || !BinCenter[sDim]) {
                    TotNumBins = 0;
                    return false;
                }
    ...
    bool AddHisto(const DLM_Histo &other, const bool witherror = true, const Type &scale = 1) {
        ...
        if (SameStructure(other)) {
            ...
            if (witherror)
                for (unsigned uBin = 0; uBin < TotNumBins; uBin++)
                    BinError[uBin] = sqrt(BinError[uBin] * BinError[uBin] +
                                          scale * scale * other.BinError[uBin] * other.BinError[uBin]);
        } else {
            if (other.Dim != Dim) {
                return false;
            }
            ...
            for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
                ...
                if (witherror) {
                    BinError[uBin] = sqrt(BinError[uBin] * BinError[uBin] +
                                          scale * scale * other.Eval(xVal, true) * other.Eval(xVal, true));
                }
            }
            delete[] BinId;
            delete[] xVal;
    ...
    bool AddWeightedHisto(const DLM_Histo &other, const Type &scale = 1) {
        ...
        for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
            ...
            BinValue[uBin] = BinValue[uBin] / BinError[uBin] / BinError[uBin] +
                             other.Eval(xVal) / other.Eval(xVal, 1) / other.Eval(xVal, 1);
            BinValue[uBin] *= VAR;
    ...
    */
        ...
        if (witherror)
            for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
                ...
                if (!other.Eval(xVal))
                    ...
                    BinError[uBin] =
                        pow(other.Eval(xVal), -2.) * sqrt(pow(BinError[uBin] * other.Eval(xVal, true), 2.) +
                                                          pow(BinError[uBin] * other.Eval(xVal), 2.) +
                                                          pow(BinValue[uBin] * other.Eval(xVal, true), 2.));
            }
        for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
    ...
    bool DivideHistoBinByBin(const DLM_Histo &other, const bool witherror = true) {
        ...
        if (witherror)
            for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
                GetBinCoordinates(uBin, BinId);
                if (!other.BinValue[uBin])
                    BinError[uBin] = 1e121;
                else
                    BinError[uBin] =
                        pow(other.BinValue[uBin], -2.) * sqrt(pow(BinError[uBin] * other.BinError[uBin], 2.) +
                                                              pow(BinError[uBin] * other.BinValue[uBin], 2.) +
                                                              pow(BinValue[uBin] * other.BinError[uBin], 2.));
            }
        for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
    ...
    double SampleYield(const bool &UnderOverFlow = false) const { return Sample(UnderOverFlow, true); }
    ...
    double SamplePdf(const bool &UnderOverFlow = false) const { return Sample(UnderOverFlow, false); }
    ...
    Type IntegralError() { return INT_ERROR; }
    ...
    Type FastIntegral(const double *xMin, const double *xMax, const bool &Normalized = false) {
        ...
        for (unsigned uBin = 0; uBin < TotNumBins; uBin++) {
            ...
            for (unsigned short sDim = 0; sDim < Dim; sDim++) {
                OutsideRange *=
                    (BinCenter[sDim][BinId[sDim]] < xMin[sDim] || BinCenter[sDim][BinId[sDim]] > xMax[sDim]);
            }
            if (OutsideRange)
                continue;
            VALUE = BinValue[uBin];
    ...
    unsigned short GetDim() const { return Dim; }
    void Rebin(const unsigned dimX, const unsigned dimY = 0, const unsigned dimZ = 0) {
        unsigned *reb_fact = new unsigned[Dim];

        if (dimX == 0) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo Rebin(...) function failed, check your arguments!\n");
            return;
        }
        if (dimY == 0 && dimZ != 0) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo Rebin(...) function failed, check your arguments!\n");
    ...
    void SetBinContent(const unsigned &WhichX, const unsigned &WhichY, const Type &Val) {
        ...
        if (Dim != 2) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo SetBinContent function failed, this set up works only for "
                   "Dim=2!\n");
            return;
        }
        if (WhichX >= NumBins[0])
    ...
    void SetBinError(const unsigned &WhichX, const unsigned &WhichY, const Type &Val) {
        ...
        if (Dim != 2) {
            printf(
                "\033[1;33mWARNING:\033[0m DLM_Histo SetBinError function failed, this set up works only for Dim=2!\n");
            return;
        }
        if (WhichX >= NumBins[0])
            return;
    ...
    void AddAt(const double &xValue, const Type &Val) {
        ...
        if (Dim != 1) {
            printf(
                "\033[1;33mWARNING:\033[0m DLM_Histo AddAt/Fill function failed, this set up works only for Dim=1!\n");
            return;
        }
        AddAt(&xValue, Val);
    }
    void AddAt(const double &xValue, const double &yValue, const Type &Val) {
        ...
        if (Dim != 2) {
            printf(
                "\033[1;33mWARNING:\033[0m DLM_Histo AddAt/Fill function failed, this set up works only for Dim=2!\n");
            return;
        }
        double axis[2];
        axis[0] = xValue;
    ...
    void AddAt(const double &xValue, const double &yValue, const double &zValue, const Type &Val) {
        ...
        if (Dim != 3) {
            printf(
                "\033[1;33mWARNING:\033[0m DLM_Histo AddAt/Fill function failed, this set up works only for Dim=3!\n");
            return;
        }
        double axis[3];
        axis[0] = xValue;
    ...
    void Fill(const double &xValue) { AddAt(xValue, 1); }
    void Fill(const double &xValue, const double &yValue) { AddAt(xValue, yValue, 1); }
    void Fill(const double &xValue, const double &yValue, const double &zValue) { AddAt(xValue, yValue, zValue, 1); }
    void SetBinContent(const unsigned *WhichBin, const Type &Val) {
        if (!Initialized) {
            InitWarning();
            return;
    ...
    double GetLowEdge(const unsigned short &sDim) const { return GetBinLowEdge(sDim, 0); }
    double GetUpEdge(const unsigned short &sDim) const { return GetBinUpEdge(sDim, NumBins[sDim] - 1); }
    ...
    double GetBinSize(const unsigned short &sDim, const unsigned &WhichBin) const {
        ...
        if (WhichBin >= NumBins[sDim])
            return 0;
        // printf("sDim=%u; WhichBin=%u; BR1=%f;
    ...
    unsigned FindBin(const unsigned short &sDim, const double &xVal) const { return GetBin(sDim, xVal); }
    unsigned GetBin(const unsigned short &sDim, const double &xVal) const {
        ...
        // makes sure that the xVal is in BinRange. If not, the returned value is either
        // NumBins or NumBins+1, depending on if we have an underflow or overflow
        if (xVal < BinRange[sDim][0])
            return NumBins[sDim];
        if (xVal > BinRange[sDim][NumBins[sDim]])
            return NumBins[sDim] + 1;
        while (true) {
    ...
    */
    ...
    Type Eval(const double *xVal, const bool &EvalTheError = false) const {
        ...
        for (unsigned short sDim = 0; sDim < Dim; sDim++) {
            ...
            else if ((xVal[sDim] < xValue1[sDim] && xBin1[sDim] > 0 && xBin1[sDim] < NumBins[sDim]) ||
                     xBin1[sDim] == NumBins[sDim] - 1) {
                xBin2[sDim] = xBin1[sDim];
        ...
        for (unsigned uPer = 0; uPer < NumPermutations; uPer++) {
            ...
            if (EvalTheError) {
                Result += (GetBinError(BinArray) * Weight);
            } else {
    ...
    Type EvalError(const double *xVal) const { return Eval(xVal, true); }

    Type Eval(const double xVal, const bool &EvalTheError = false) const {
        if (Dim != 1) {
            printf(
                "\033[1;33mWARNING:\033[0m DLM_Histo Eval(xVal) function failed, this set up works only for Dim=1!\n");
            return Type(0);
        }
    ...
    Type EvalError(const double xVal) const {
        if (Dim != 1) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo EvalError(xVal) function failed, this set up works only for "
                   "Dim=1!\n");
            return Type(0);
        }
    ...
    Type Eval2D(const double xVal, const double yVal, const bool &EvalTheError = false) const {
        if (Dim != 2) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo Eval2D(xVal) function failed, this set up works only for "
                   "Dim=2!\n");
            return Type(0);
        }
        double xPars[2];
    ...
    Type EvalError2D(const double xVal, const double yVal) const {
        if (Dim != 2) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo EvalError2D(xVal) function failed, this set up works only for "
                   "Dim=2!\n");
            return Type(0);
        }
        double xPars[2];
    ...
    Type Eval3D(const double xVal, const double yVal, const double zVal, const bool &EvalTheError = false) const {
        if (Dim != 3) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo Eval3D(xVal) function failed, this set up works only for "
                   "Dim=3!\n");
            return Type(0);
        }
        double xPars[3];
    ...
    Type EvalError3D(const double xVal, const double yVal, const double zVal) const {
        if (Dim != 3) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo EvalError3D(xVal) function failed, this set up works only for "
                   "Dim=3!\n");
            return Type(0);
        }
        double xPars[3];
    ...
    bool SetEqualTo(const DLM_Histo &other) {
        if (Dim != other.Dim) {
            printf("\033[1;33mWARNING:\033[0m DLM_Histo::SetEqualTo function failed, this set up works only for histos "
                   "with the same Dim.\n");
            return false;
        }
    ...
    bool operator=(const DLM_Histo &other) {
        ...
        if (!SameStructure(other)) {
            ...
            // printf("Dim done\n");
            for (unsigned short sDim = 0; sDim < Dim; sDim++)
                SetUp(sDim, other.NumBins[sDim], other.BinRange[sDim]);
            // printf("SetUp done\n");
            if (!Initialize(false))
                return false;
        }
    ...
    bool operator==(const DLM_Histo &other) {
        if (!SameStructure(other))
            return false;
        for (unsigned uBin = 0; uBin < TotNumBins + 2; uBin++) {
            if (BinValue[uBin] != other.BinValue[uBin])
                return false;
            if (uBin < TotNumBins) {
                if (BinError[uBin] != other.BinError[uBin])
                    return false;
                if (BinCenter[uBin] != other.BinCenter[uBin])
                    return false;
            }
    ...
    bool operator+=(const DLM_Histo &other) { return AddHisto(other, true, 1); }
    bool operator-=(const DLM_Histo &other) { return AddHisto(other, true, -1); }
    bool operator*=(const DLM_Histo &other) { return MultiplyHisto(other, true); }
    bool operator/=(const DLM_Histo &other) { return DivideHisto(other, true); }
    ...
    bool QuickWrite(const char *FileName, bool Overwrite = false) {
        ...
        if (myFileIN.fail() == false && Overwrite == false) {
            printf("\033[1;31mERROR:\033[0m The file %s exists. Change name or use QuickWrite(FileName, true) to "
                   "overwrite.\n",
                   FileName);
            return false;
        }
    ...
    bool QuickLoad(const char *FileName, const int Version = -1) {
        if (Initialized) {
            printf("\033[1;31mERROR:\033[0m Cannot load from file, the target object is already initialized (it should "
                   "be blank).\n");
            return false;
        }
        if (Version != -1) {
  ...
  protected:
    ...
    bool SameStructure(const DLM_Histo &other) const {
        if (Dim != other.Dim)
            return false;
        for (unsigned short sDim = 0; sDim < Dim; sDim++) {
            if (NumBins[sDim] != other.NumBins[sDim])
                return false;
            for (unsigned uBin = 0; uBin < NumBins[sDim]; uBin++) {
                if (BinRange[sDim][uBin] != other.BinRange[sDim][uBin])
                    return false;
                if (BinCenter[sDim][uBin] != other.BinCenter[sDim][uBin])
                    return false;
            }
            if (BinRange[sDim][NumBins[sDim]] != other.BinRange[sDim][NumBins[sDim]])
                return false;
        }
        if (TotNumBins != other.TotNumBins)
            return false;
        return true;
    }
    ...
    void UpdateCum(const bool &yield) {
        ...
        if (!CumulativeValue)
            CumulativeValue = new Type[TotNumBins + 2];
        for (unsigned uBin = 0; uBin < TotNumBins + 2; uBin++) {
            double Renorm = 1;
            ...
            if (uBin == 0)
                CumulativeValue[uBin] = BinValue[uBin] * Renorm;
            else
                CumulativeValue[uBin] = CumulativeValue[uBin - 1] + BinValue[uBin] * Renorm;
    ...
    void Sample(double *axisValues, const bool &UnderOverFlow, const bool &yield, DLM_Random *RanGen) {
        ...
        while (true) {
            ...
            if (BinCandidate == 0)
                value_down = double(CumulativeValue[BinCandidate]);
            else
                value_down = double(CumulativeValue[BinCandidate] + CumulativeValue[BinCandidate - 1]) * 0.5;
            if (BinCandidate == TopBin)
                value_up = double(CumulativeValue[BinCandidate]);
            else
                value_up = double(CumulativeValue[BinCandidate] + CumulativeValue[BinCandidate + 1]) * 0.5;
            // printf(" value_down = %f\n",value_down);
            // printf(" value_up = %f\n",value_up);
            // printf(" rannum = %f\n",rannum);
