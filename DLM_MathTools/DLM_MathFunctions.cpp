...
double factrl(const unsigned n) {
    ...
    if (!factrl_array[WhichThread]) {
        ...
        for (int i = 1; i < 171; i++)
            factrl_array[WhichThread][i] = i * factrl_array[WhichThread][i - 1];
    }
...
bool ApproxEqualF(const float val1, const float val2) {
    if (fabs(val1 - val2) / fabs(val1 * val2) < 5e-7)
        return true;
    return false;
}
bool ApproxEqualD(const double val1, const double val2) {
    if (fabs(val1 - val2) / fabs(val1 * val2) < 5e-15)
        return true;
    return false;
}
...
double atanPhi(const double &y, const double &x) {
    if (x == 0) {
        if (y > 0)
            ...
            return Pi; // actually it can be ANY number between 0 and TwoPi, so we take the average
    } else if (x > 0 && y > 0) {
...
double Transverse(const double &py, const double &px) { return sqrt(py * py + px * px); }

double GetPval(const double &chi2, const double &ndf) { return gsl_sf_gamma_inc_Q(ndf * 0.5, chi2 * 0.5); }
...
double FindDeltaChi2_GivenNsigma(const double &chi2, const double *pars) { return GetNsigma(chi2, pars[1]) - pars[0]; }
...
double GetDeltaChi2(const double &nsigma, const unsigned &nfreepars) {
    ...
    if (nsigma < 1)
        return NewtonRapson(FindDeltaChi2_GivenNsigma, 0, double(nfreepars) * 2., pars);
    if (nsigma < 3)
        return NewtonRapson(FindDeltaChi2_GivenNsigma, 0, double(nfreepars) * 8., pars);
    if (nsigma < 5)
        return NewtonRapson(FindDeltaChi2_GivenNsigma, 0, double(nfreepars) * 32., pars);
    if (nsigma < 10)
        return NewtonRapson(FindDeltaChi2_GivenNsigma, 0, double(nfreepars) * 128., pars);
    return 1e128;
}

double GetPvalFromNsig(const double &nsigma) { return GetPval(GetDeltaChi2(nsigma, 1), 1); }

/*
void BinomialPermutations(const unsigned offset, const unsigned& k,
                                                    std::vector<std::vector<unsigned>>& permutations,
std::vector<unsigned>& elements){ if(k==0){ std::vector<unsigned> dummy; permutations.push_back(dummy);
...
void BinomialPermutations(unsigned offset, unsigned k, vector<unsigned> &elements,
...
double Sill_kstar(double *KSTAR, double *par) {
    ...
    double Derivative_dM_dkStar =
        kstar / sqrt(kstar * kstar + dmass_1 * dmass_1) + kstar / sqrt(kstar * kstar + dmass_2 * dmass_2);
...
double SillPhaseSpaceKstar(double *xval, double *par) {
    ...
    double PhaseSpace = ResonancePt * Temperature == 0
                            ? 1
                            : (MotherMass / sqrt(MotherMass * MotherMass + ResonancePt * ResonancePt)) *
                                  exp(-sqrt(MotherMass * MotherMass + ResonancePt * ResonancePt) / Temperature);
    double PhaseSpaceNorm =
        ResonancePt * Temperature == 0 ? 1 : exp(-sqrt(Thresh * Thresh + ResonancePt * ResonancePt) / Temperature);
