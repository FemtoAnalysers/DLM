...
DLM_DecayGen::DLM_DecayGen(const unsigned &seed) : SEED(seed), MaxNumDaughters(4), MinNumDaughters(2) {
...
void DLM_DecayGen::SetSillyThreshold(const double &nthr) { SillyThreshold = nthr; }
...
void DLM_DecayGen::DirtyDist(const unsigned &numdecays) {
    ...
    for (unsigned uIter = 0; uIter < TrueNumIter; uIter++) {
        ...
        do {
            ...
            if ((fabs(Sum.M() - Mother.M()) < Epsilon * ExcessE && fabs(Sum.P() - Mother.P()) < Epsilon * ExcessE) &&
        ...
        } while (fabs(Sum.M() - Mother.M()) > EpsilonFinal * ExcessE ||
                 fabs(Sum.P() - Mother.P()) > EpsilonFinal * ExcessE);
