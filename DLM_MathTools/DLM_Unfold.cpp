...
DLM_Unfold::~DLM_Unfold() {
    ...
    if (DLM_FITTER_ARRAY_SPLINE3_X) {
        delete DLM_FITTER_ARRAY_SPLINE3_X;
        DLM_FITTER_ARRAY_SPLINE3_X = NULL;
    }
    if (DLM_FITTER_ARRAY_SPLINE3_Y) {
        delete DLM_FITTER_ARRAY_SPLINE3_Y;
        DLM_FITTER_ARRAY_SPLINE3_Y = NULL;
    }
    if (SparseFirst) {
        delete[] SparseFirst;
    ...
    if (FitUnfoldFinal) {
        delete FitUnfoldFinal;
        FitUnfoldFinal = NULL;
    }
    if (dlmFoldedWorkHorse) {
        delete dlmFoldedWorkHorse;
        dlmFoldedWorkHorse = NULL;
    }
    if (dlmUnfoldedWorkHorse) {
        delete dlmUnfoldedWorkHorse;
        dlmUnfoldedWorkHorse = NULL;
    }
    if (dlmFoldedDimiWorkHorse) {
        delete dlmFoldedDimiWorkHorse;
        dlmFoldedDimiWorkHorse = NULL;
    }
    if (dlmUnfoldedDimiWorkHorse) {
        delete dlmUnfoldedDimiWorkHorse;
        dlmUnfoldedDimiWorkHorse = NULL;
    }
    if (dlmDimiWorkHorse) {
        delete dlmDimiWorkHorse;
        dlmDimiWorkHorse = NULL;
    }
    if (hData) {
        delete hData;
    ...
    if (BinByBinFolded) {
        delete[] BinByBinFolded;
        BinByBinFolded = NULL;
    }
    if (BinByBinUnfolded) {
        delete[] BinByBinUnfolded;
        BinByBinUnfolded = NULL;
    }
}
...
void DLM_Unfold::SetResponse(TH2F *response) {
    ...
    for (unsigned uBinY = 0; uBinY < response->GetYaxis()->GetNbins(); uBinY++) {
        ...
        for (unsigned uBinX = 0; uBinX < response->GetXaxis()->GetNbins(); uBinX++) {
            ...
            if (response->GetBinContent(uBinX + 1, uBinY + 1) != 0 &&
                SparseLast[uBinY] <= response->GetXaxis()->GetNbins()) {
                SparseLast[uBinY] = uBinX;
            }
        }
        // this is done so that we have a single bin left for extrapolation
        if (SparseFirst[uBinY])
            SparseFirst[uBinY]--;
        if (SparseLast[uBinY] < response->GetXaxis()->GetNbins() - 1)
            SparseLast[uBinY]++;
...
void DLM_Unfold::SetResponse(DLM_Histo<float> *response) {
    ...
    for (unsigned uBinY = 0; uBinY < hResponse->GetYaxis()->GetNbins(); uBinY++) {
        ...
        for (unsigned uBinX = 0; uBinX < hResponse->GetXaxis()->GetNbins(); uBinX++) {
            ...
            if (hResponse->GetBinContent(uBinX + 1, uBinY + 1) != 0 &&
                SparseLast[uBinY] <= hResponse->GetXaxis()->GetNbins()) {
                SparseLast[uBinY] = uBinX;
            }
        }
        // this is done so that we have a single bin left for extrapolation
        if (SparseFirst[uBinY])
            SparseFirst[uBinY]--;
        if (SparseLast[uBinY] < hResponse->GetXaxis()->GetNbins() - 1)
            SparseLast[uBinY]++;
        // printf("Y %u: %u %u\n",uBinY,SparseFirst[uBinY],SparseLast[uBinY]);
    }
}
...
void DLM_Unfold::SetUnfoldSeconds(const double &seconds) { MaxTime = seconds; }

void DLM_Unfold::SetUnfoldMinutes(const double &minutes) { MaxTime = minutes * 60.; }
...
void DLM_Unfold::SetSilentMode(const bool &silentmode) { Silent = silentmode; }
...
double DLM_Unfold::SplineFitFolded(double *xVal, double *pars) {
    ...
    if (xCurrent <= xLast) {
        ...
        if (!dlmFoldedWorkHorse)
            dlmFoldedWorkHorse = new DLM_Histo<float>(*dlmUnfoldedWorkHorse);
...
double DLM_Unfold::SplineFit(double *xVal, double *pars) {
    ...
    if (NumKnots > MAX_KNOTS)
        NumKnots = MAX_KNOTS;
    if (!DLM_FITTER_ARRAY_SPLINE3_X)
        DLM_FITTER_ARRAY_SPLINE3_X = new double[MAX_KNOTS];
    if (!DLM_FITTER_ARRAY_SPLINE3_Y)
        DLM_FITTER_ARRAY_SPLINE3_Y = new double[MAX_KNOTS];
    for (int iKnot = 0; iKnot < NumKnots; iKnot++) {
        ...
        if (DLM_FITTER_ARRAY_SPLINE3_Y[iKnot] == 1e6 && iKnot)
            DLM_FITTER_ARRAY_SPLINE3_Y[iKnot] = pars[3 + NumKnots + iKnot - 1];
    }
...
double DLM_Unfold::DimiFitFolded(double *xVal, double *pars) {
    ...
    if (xCurrent <= xLast) {
        ...
        if (!dlmFoldedDimiWorkHorse)
            dlmFoldedDimiWorkHorse = new DLM_Histo<float>(*dlmDimiWorkHorse);
...
DLM_Histo<float> *DLM_Unfold::Unfold(const int &TYPE) {
    ...
    if (NumBins <= 1) {
        printf(
            "\033[1;33mWARNING!\033[0m Smearing a less than 2 bins is truely the next level! Sorry, cannot do it!\n");
        return NULL;
    }
    ...
    int NumKnots = 2;
    for (unsigned uBin = 0; uBin < dlmData->GetNbins(); uBin++) {
        if (dlmData->GetBinCenter(0, uBin) < RangeMinU || dlmData->GetBinCenter(0, uBin) > RangeMaxU)
            continue;
        if (uBin <= 2) {
            NumKnots++;
        } else if ((uBin - 2) % 3 == 0) {
    ...
    for (unsigned uBin = 0; uBin < dlmData->GetNbins(); uBin++) {
        if (dlmData->GetBinCenter(0, uBin) < RangeMinU || dlmData->GetBinCenter(0, uBin) > RangeMaxU)
            continue;
        if (uBin <= 2) {
            Knot_x[NumKnots] = dlmData->GetBinCenter(0, uBin);
            NumKnots++;
        } else if ((uBin - 2) % 3 == 0) {
            Knot_x[NumKnots] = dlmData->GetBinCenter(0, uBin);
            NumKnots++;
    ...
    if (TYPE == 1) {
        ...
        FitFoldOriginal = new TF1("FitFoldOriginal", this, &DLM_Unfold::DimiFit, RangeMinU, RangeMaxU,
                                  dlmData_reb2->GetNbins(), "DLM_Unfold", "DimiFit");
        ...
        // dlmDimiWorkHorse = new DLM_Histo<float>(*dlmData_reb2);
        FitFoldFinal = new TF1("FitFoldFinal", this, &DLM_Unfold::DimiFitFolded, RangeMinU, RangeMaxU,
                               dlmData_reb2->GetNbins(), "DLM_Unfold", "DimiFitFolded");
        ...
        for (unsigned uRefit = 0; uRefit < 1; uRefit++) {
            ...
            for (unsigned uPar = 0; uPar < dlmData_reb2->GetNbins(); uPar++) {
                ...
                if (dlmData_reb2->GetBinCenter(0, uPar) < RangeMinU ||
                    dlmData_reb2->GetBinCenter(0, uPar) > RangeMaxU) {
            ...
            if (f_chi2 < FitFoldOriginal->GetChisquare() * 100)
                break;
            if (!Silent)
                printf(" Refitting (%u, chi2=%.1f)\n", uRefit, f_chi2);
        }
        f_chi2 = FitFoldFinal->GetChisquare();
        ...
        // if(dlmDimiWorkHorse){delete [] dlmDimiWorkHorse; dlmDimiWorkHorse=NULL;}
        // dlmDimiWorkHorse = new DLM_Histo<float>(*dlmData_reb2);
        FitUnfoldFinal = new TF1("FitUnfoldFinal", this, &DLM_Unfold::DimiFit, RangeMinU, RangeMaxU,
                                 dlmData_reb2->GetNbins(), "DLM_Unfold", "DimiFit");
        for (unsigned uPar = 0; uPar < dlmData_reb2->GetNbins(); uPar++) {
        ...
        gROOT->cd();
        FitFoldOriginal = new TF1("FitFoldOriginal", this, &DLM_Unfold::SplineFit, RangeMinU, RangeMaxU,
                                  3 + NumKnots * 2, "DLM_Unfold", "SplineFit");
        ...
        for (unsigned uKnot = 0; uKnot < NumKnots; uKnot++) {
            ...
            if (uKnot == 0) {
                ...
                HistVal = hData->GetBinContent(hData->FindBin(Knot_x[uKnot]));
            }
            ...
            if (uKnot == 0) {
                FitFoldOriginal->FixParameter(3 + NumKnots + uKnot, HistVal);
            }
        }
        FitFoldOriginal->SetNpx(256);
        ...
        gROOT->cd();
        FitFoldFinal = new TF1("FitFoldFinal", this, &DLM_Unfold::SplineFitFolded, RangeMinU, RangeMaxU,
                               3 + NumKnots * 2, "DLM_Unfold", "SplineFitFolded");
        ...
        for (unsigned uRefit = 0; uRefit < 1; uRefit++) {
            ...
            for (unsigned uKnot = 0; uKnot < NumKnots; uKnot++) {
                ...
                // the edge knot and the first 3 bins (so the first 4 knots) for a ME disto have to have values BELOW
                // the folded solution if(uKnot<4){
                ...
                FitFoldFinal->SetParLimits(3 + NumKnots + uKnot, 0,
                                           FitFoldOriginal->GetParameter(3 + NumKnots + uKnot) * 10);
                if (uKnot == 0) {
                    FitFoldFinal->FixParameter(3 + NumKnots + uKnot,
                                               FitFoldOriginal->GetParameter(3 + NumKnots + uKnot));
                }
                //}
            }
            FitFoldFinal->SetNpx(256);
        ...
        gROOT->cd();
        FitUnfoldFinal = new TF1("FitUnfoldFinal", this, &DLM_Unfold::SplineFitUnfolded, RangeMinU, RangeMaxU,
                                 3 + NumKnots * 2, "DLM_Unfold", "SplineFitUnfolded");
        for (unsigned uPar = 0; uPar < 3 + NumKnots * 2; uPar++) {
            FitUnfoldFinal->SetParameter(uPar, FitFoldFinal->GetParameter(uPar));
        }
        FitUnfoldFinal->SetNpx(256);
        ...
        gROOT->cd();
        FitFoldOriginal = new TF1("FitFoldOriginal", "([0]*x+[1]*x*x+[2]*x*x*x+[3]*x*x*x*x)*exp(-pow(x/[4],[5]))",
                                  RangeMinU, RangeMaxU);
        ...
        gROOT->cd();
        FitFoldFinal =
            new TF1("FitFoldFinal", "([0]*x+[1]*x*x+[2]*x*x*x+[3]*x*x*x*x)*exp(-pow(x/[4],[5]))", RangeMinU, RangeMaxU);
        ...
        for (unsigned uRefit = 0; uRefit < 11; uRefit++) {
            ...
            hData->Fit(FitFoldFinal, "Q, S, N, R, M");
            f_chi2 = FitFoldFinal->GetChisquare();
            if (f_chi2 < FitFoldOriginal->GetChisquare() * 10)
                break;
            if (!Silent)
                printf(" Refitting (%u, chi2=%.1f)\n", uRefit, f_chi2);
        }
        ...
        gROOT->cd();
        FitUnfoldFinal = new TF1("FitUnfoldFinal", "([0]*x+[1]*x*x+[2]*x*x*x+[3]*x*x*x*x)*exp(-pow(x/[4],[5]))",
                                 RangeMinU, RangeMaxU);
        for (unsigned uPar = 0; uPar < 6; uPar++) {
            FitUnfoldFinal->SetParameter(uPar, FitFoldFinal->GetParameter(uPar));
        }
    ...
    for (unsigned uSeed = SEEDmin; uSeed < SEEDmin + NumIter; uSeed++) {
        ...
        /*
            ...
            DLM_Histo<float> CPU_Unfold;// = new
           TH1F("CPU_Unfold","CPU_Unfold",128,0,4.*double(JobTimeLimit)/double(NumIter)/60.); DLM_Histo<float>
           Chi2Ndf_CPU_Unfold;// = new TH2F("Chi2Ndf_CPU_Unfold","Chi2Ndf_CPU_Unfold",
        ...
        */
        ...
        if (MaxIterTime < AvgTimePerSeedRemaining * BonusFactor)
            MaxIterTime = AvgTimePerSeedRemaining * BonusFactor;
        ...
        if (AllowedTime > MaxIterTime)
            AllowedTime = MaxIterTime;
        ...
        for (unsigned uBin = 0; uBin < dlmData->GetNbins(); uBin++) {
            ...
            if (uSeed) {
                ...
                RandomValue = dlmData->GetBinContent(uBin);
        ...
        // bootstrap the sampling histogram ones, to avoid a bias in the chi2 determination (where the original solution
        // is used as a benchmark) here we should sample from the unfolded soltion, in fact we use the fit from upstairs
        ...
        for (unsigned uBF = 0; uBF < NumBackAndForth; uBF++) {
            ...
            for (unsigned uBin = 0; uBin < 2 * dlmSampleUnfolded.GetNbins(); uBin++) {
                ...
                for (unsigned ub = 0; ub < dlmSampleUnfolded.GetNbins(); ub++) {
                    ...
                    if (xAxis > RangeMaxU)
                        break;
                    if (Impact[ub] > HighestImpact) {
                        HighestImpact = Impact[ub];
                    }
                    //  printf("Impact[%u]=%.3e\n",ub,Impact[ub]);
                    //  usleep(50e3);
                }
                ...
                if (rangen.Uniform() > ProbToCompute)
                    continue;
                if (!Polishing && WhichBin > dlmSampleUnfolded.GetNbins() - BinDepth)
                    continue;
                // if(ProbToCompute<1) printf(" -> go\n");
                if (Polishing && WhichBin < BinDepth - 1)
                    continue;
                unsigned MaxStepsWithoutImprovement =
                    (!Polishing) ? MaxStepsWithoutImprovement1 : MaxStepsWithoutImprovement2;
                ...
                for (unsigned uED = 1 * unsigned(Polishing); uED < ErrorDepth + 1 * unsigned(Polishing); uED++) {
                    ...
                    while (SuccessRate[uBin_From] > MinSuccessRate || NumAttempts[uBin_From] < MinAttempts) {
                        ...
                        for (unsigned uBin2 = uBin_From; uBin2 < uBin_To; uBin2++) {
                            ...
                            /*
                            //if(StepsWithoutImprovement%2) RandomValue =
                            rangen.Uniform(OriginalValue-0.5*Error*Scale,OriginalValue+0.5*Error*Scale);
                            if(NumAttempts[uBin_From]%3==0) RandomValue =
                            rangen.Uniform(OriginalValue-0.5*Error*Scale,OriginalValue+0.5*Error*Scale);
                        ...
                        for (unsigned uBin2 = 0; uBin2 < dlmData_boot.GetNbins(); uBin2++) {
                            ...
                            Chi2_BinFit =
                                pow((dlmUnfolded.GetBinContent(uBin2) - FitUnfoldFinal->Eval(xAxis)) / (Error), 2.);
                            Chi2 += Chi2_BinData;
                            Chi2 += Chi2_BinFit;
                            Chi2_Data += Chi2_BinData;
                            Chi2_Fit += Chi2_BinFit;
                            if (uBin2 >= uBin_From && uBin2 < uBin_To) {
                                if (Chi2_BinData > Chi2_BinDataMax)
                                    Chi2_BinDataMax = Chi2_BinData;
                                if (Chi2_BinFit > Chi2_BinFitMax)
                                    Chi2_BinFitMax = Chi2_BinFit;
                            }
                            Ndf++;
                        }
                        ...
                        if (BinByBinChi2[WhichBin].size() >= HistoryLength) {
                            BinByBinChi2[WhichBin].erase(BinByBinChi2[WhichBin].begin());
                        }
                        ...
                        // the idea: on the first iteration, make the solution close to the expectation (find the
                        // desired global min)
            ...
            if (!Silent && PcOutput) {
                printf("\r\033[K%s%s", ctext1, ctext2);
            }
            if (!Silent)
                printf(" --> %.1f\n", f_chi2);
    ...
    for (unsigned uBin = 0; uBin < dlmUnfoldResult->GetNbins(); uBin++) {
        ...
        if (xAxis > RangeMaxU)
            break;
        dlmUnfoldResult->SetBinError(uBin,
                                     sqrt(dlmUnfoldResult->GetBinError(uBin) -
                                          dlmUnfoldResult->GetBinContent(uBin) * dlmUnfoldResult->GetBinContent(uBin)));
    }
...
DLM_Histo<float> *DLM_Unfold::UnfoldDimi() {
    ...
    for (unsigned uReb = 1; uReb >= 1; uReb /= 2) {
        ...
        TF1 *FitTempFold = new TF1(TString::Format("FitFoldFinal_%u", uReb), this, &DLM_Unfold::DimiFitFolded,
                                   RangeMinU, RangeMaxU, dlmData_reb2->GetNbins(), "DLM_Unfold", "DimiFitFolded");
        double xVal;
        for (unsigned uPar = 0; uPar < dlmData_reb2->GetNbins(); uPar++) {
            ...
            double UpLimit;
            if (FitFoldFinal)
                UpLimit = FitFoldFinal->Eval(xVal) * 1.5;
            else
                UpLimit = dlmData_reb2->GetBinContent(uPar) * 1.5;
        ...
        // printf("new\n");
        FitUnfoldFinal = new TF1(TString::Format("FitUnfoldFinal_%u", uReb), this, &DLM_Unfold::DimiFit, RangeMinU,
                                 RangeMaxU, dlmData_reb2->GetNbins(), "DLM_Unfold", "DimiFit");
        // printf("uPar\n");
        for (unsigned uPar = 0; uPar < dlmData_reb2->GetNbins(); uPar++) {
            xVal = dlmData_reb2->GetBinCenter(0, uPar);
...
void DLM_Unfold::Fold(const DLM_Histo<float> &DATA, DLM_Histo<float> &RESULT) {
    ...
    */
    ...
    for (uBin[1] = 0; uBin[1] < NumBins; uBin[1]++) {
        ...
        if (Momentum[1] > RangeMaxF || Momentum[1] < RangeMinF)
            continue;
        double Integral = 0;
