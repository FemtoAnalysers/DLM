...
const double Bessel::c1[7] = {-1.142022680371168e0, 6.5165112670737e-3, 3.087090173086e-4, -3.4706269649e-6,
                              6.9437664e-9,         3.67795e-11,        -1.356e-13};
const double Bessel::c2[8] = {1.843740587300905e0, -7.68528408447867e-2, 1.2719271366546e-3, -4.9717367042e-6,
...
void Bessel::besselik(const double nu, const double x) {
    ...
    double a, a1, b, c, d, del, del1, delh, dels, e, f, fact, fact2, ff, gam1, gam2, gammi, gampl, h, p, pimu, q, q1,
        q2, qnew, ril, ril1, rimu, rip1, ripl, ritemp, rk1, rkmu, rkmup, rktemp, s, sum, sum1, x2, xi, xi2, xmu, xmu2,
        xx;
    int i, l, nl;
    if (x <= 0.0 || nu < 0.0)
...
void Bessel::airy(const double x) {
    static const double PI = 3.141592653589793238, ONOVRT = 0.577350269189626, THR = 1. / 3., TWOTHR = 2. * THR;
    double absx, rootx, z;
    absx = fabs(x);
    rootx = sqrt(absx);
    z = TWOTHR * absx * rootx;
...
std::complex<double> spherical_bessel_jn(int n, std::complex<double> x) {
    if (n == 0) {
        ...
        return ((2.0 * n - 1.0) * spherical_bessel_jn(n - 1, x) -
                std::complex<double>(n) * spherical_bessel_jn(n - 2, x)) /
               x; // Corrected: explicit conversion to complex
    }
}