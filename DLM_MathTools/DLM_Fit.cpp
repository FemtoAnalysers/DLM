...
DLM_FitSolution::DLM_FitSolution(const DLM_FitSolution &other) : DLM_FitSolution() { operator=(other); }
DLM_FitSolution::~DLM_FitSolution() { delete Par; }

DLM_FitSolution DLM_FitSolution::operator-(const DLM_FitSolution &other) const {
    DLM_FitSolution result;
    if (Par->size() != other.Par->size())
        return result;
...
void DLM_FitSolution::operator+=(const DLM_FitSolution &other) { Add(other, 1); }
void DLM_FitSolution::operator-=(const DLM_FitSolution &other) { Add(other, -1); }
...
DLM_Fit::DLM_Fit(const unsigned num_threads)
    : MaxThreads(std::thread::hardware_concurrency() ? num_threads ? num_threads <= std::thread::hardware_concurrency()
                                                                         ? num_threads
...
void DLM_Fit::SetInterProcess(const bool &yesno) { InterProcess = yesno; }
...
void DLM_Fit::SetParMinMag(const unsigned &WhichPar, const int &min) { SetParOrdMag(WhichPar, min, 127); }
void DLM_Fit::SetParMaxMag(const unsigned &WhichPar, const int &max) { SetParOrdMag(WhichPar, -127, max); }
...
bool DLM_Fit::SetData(const unsigned &WhichSet, DLM_Histo<float> &data) {
    ...
    for (unsigned uTh = 0; uTh < MaxThreads; uTh++) {
        if (Model[uTh]->at(WhichSet)) {
            delete Model[uTh]->at(WhichSet);
            Model[uTh]->at(WhichSet) = NULL;
        }
...
float DLM_Fit::Chi2() const { return chi2[ThIdSol]; }
unsigned DLM_Fit::Npts() const { return NumDataPts[ThIdSol]; }
...
bool DLM_Fit::PrepareForWalk() {
    ...
    if (Solution->size() < NumBestSols) {
        if (Solution->size()) {
            printf("\033[1;31mFATAL ERROR:\033[0m (DLM_Fit::PrepareForWalk) Solution->size() shows a bug, contact the "
                   "developers!\n");
            return false;
        }
        ...
        for (unsigned uPar = 0; uPar < NumPars; uPar++) {
            ...
            // in case a signle ord of mag is studied, do nothing, otherwise set the max ord mag to the corresponding
            // value and make sure the manitssa is reset to reach the maximum value (which will still be within the set
            // limits) N.B. in any other case nothing is done. 1) if the max order of mag is larger than the limit, it
            // is obvious why. 2) if the max ord mag is lower than the actual parameter limits, is impossible. Thus if
            // set so -> ignored (no warnings presently)
            ...
            // printf("   (+): exp in [%i, %i]; mantissa in [%.4f,
            // %.4f]\n",MinExp_Pos[uPar],MaxExp_Pos[uPar],MinManVal_Pos[uPar],MaxManVal_Pos[uPar]); printf("   (-): exp
            // in [%i, %i]; mantissa in [%.4f,
        ...
        for (unsigned uBS = 0; uBS < NumBestSols * NumBestSols; uBS++) {
            ...
            for (unsigned uPar = 0; uPar < NumPars; uPar++) {
                if (ParIsFixed(uPar)) {
                    ...
                    // non-trivial: if we generate a number that has the max exp, than the mantissa should be 0.5 -
                    // MaxManVal
                    //              if we generate a number that has the min exp, than the mantissa should be MinManVal
                    //              - 1.0
                    if (RndSign == 1) {
    ...
    // i.e. this is now the guided random walk
        ...
        if (Solution->back().Chi2 < 0) {
            printf("\033[1;31mFATAL ERROR:\033[0m (DLM_Fit::PrepareForWalk) Chi2<0 shows a bug, contact the "
                   "developers!\n");
            return false;
        }
        ...
        // }
        // if( Solution->at(Solution->size()-NumBestSols*NumBestSols).Chi2 - Solution->back().Chi2 <= DeltaChi2) return
        // true; if( Solution->back().Chi2 == Solution->at(Solution->size()-2).Chi2) return true;
        ...
        if (PotentialTargets.size() != NumBestSols) {
            printf("\033[1;31mFATAL ERROR:\033[0m (DLM_Fit::PrepareForWalk) PotentialTargets.size() != NumBestSols "
                   "shows a bug, contact the developers!\n");
            return false;
        }
        ...
        for (unsigned target : Targets) {
            ...
            for (unsigned uBS = BestSolIdLow; uBS <= BestSolIdUp; uBS++) {
                ...
                if (uBS > target && ElementInVector(Targets, uBS))
                    continue;

                // Solution->at(target) and Solution->at(uBS) represents the two sets of parameters, along the line of
                // which the parameters for the next soltion are evaluated in GetSuitableParameters

                if (Solution->at(uBS).Chi2 == Solution->at(target).Chi2) {
                    printf("\033[1;31mFATAL ERROR:\033[0m (DLM_Fit::PrepareForWalk) sol2.Chi2==sol1.Chi2 shows a bug, "
                ...
                if (Solution->at(SolB).Chi2 < Solution->at(SolA).Chi2) {
                    printf("\033[1;31mFATAL ERROR:\033[0m (DLM_Fit::PrepareForWalk) solB.Chi2<solA.Chi2 shows a bug, "
                           "contact the developers!\n");
                    return false;
                }
                ...
                for (unsigned iter = 0; iter < 1; iter++) {
                    ...
                    for (unsigned uPar = 0; uPar < NumPars; uPar++) {
                        if (ParIsFixed(uPar) && Solution->back().Par->at(uPar) != Solution->at(SolA).Par->at(uPar)) {
                            printf("\033[1;33mWARNING:\033[0m Suspicious behaviour DLM_Fit::PrepareForWalk, contact "
                                   "the developers! Report: a fixed par was changed!\n");
...
void DLM_Fit::OrderSolutions() { std::sort(Solution->rbegin(), Solution->rend()); }
...
bool DLM_Fit::ParIsFree(const unsigned &uPar) const { return !ParIsFixed(uPar); }
