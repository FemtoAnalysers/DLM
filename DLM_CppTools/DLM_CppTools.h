...
template <typename VT> bool ElementInVector(std::vector<VT> &vec, const VT &elem) {
    for (VT &el : vec) {
        if (el == elem)
            return true;
    }
...
template <typename TYPE> void ResizeArray(TYPE *&array, const unsigned &OldSize = 0, const unsigned &NewSize = 0) {
