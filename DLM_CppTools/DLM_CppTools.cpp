...
void ShowTime(long long Time, char *str, short show, bool compact, short NumUnits) {
    ...
    long long
        max_value[NumUnits]; // how much units of this unit constitute the "higher" unit (i.e. 365d = 1y, 24h = 1d etc.)
    ...
    char Unit_name[NumUnits][2][2]
                  [16]; // name of the units [][0][] -> singular; [][1][] -> plural; [][][0/1] -> full/compact form
    ...
    for (short sUnit = NumUnits - 2; sUnit >= 0; sUnit--) {
        switch (sUnit) {
        case 0:
            value_in_sec[0] = value_in_sec[1] * 365;
            break;
        case 1:
            value_in_sec[1] = value_in_sec[2] * 24;
            break;
        case 2:
            value_in_sec[2] = value_in_sec[3] * 60;
            break;
        case 3:
            value_in_sec[3] = value_in_sec[4] * 60;
            break;
        case 4:
            value_in_sec[4] = value_in_sec[5] * 1000;
            break;
        case 5:
            value_in_sec[5] = value_in_sec[6] * 1000;
            break;
        default:
            break;
        }
...
DLM_Timer::DLM_Timer() { Start(); }
DLM_Timer::~DLM_Timer() {}

void DLM_Timer::Start() { gettimeofday(&start, NULL); }
