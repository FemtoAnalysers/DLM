...
void SetAnalysisFolderPath(char *folder_path) { INPUT_FOLDER = folder_path; }

TString OUTPUT_FOLDER = "./";
void SetOutputFolderPath(char *folder_path) { OUTPUT_FOLDER = folder_path; }
...
void pi_d_source() {
    ...
    std::vector<float> vectors[] = {core_radii_mTint, core_radii_mT0, core_radii_mT1,
                                    core_radii_mT2,   core_radii_mT3, core_radii_mT4};
...
//-1 = pip mt int
//-101 = pim mt int
void MainAnalysisFunction(char *Description, std::vector<int> fit_types, int mt_bin, int NumIter, bool Bootstrap,
                          bool DataVar, bool FitVar, int SEED) {
    ...
    // these are the scenario B and A (B = 2/3 became default it seems, the other is systematics), how the pT of the
    // p-pi relates to d-pi
    ...
    if (fabs(mt_bin) >= 100 && mt_bin < 1000)
        Dpp_weight = 0;
    ...
    TFile *fReso =
        new TFile(TString::Format("%s/ReconstructionFiles/AnalysisResults3996.root", INPUT_FOLDER.Data()), "read");
    ...
    hProj =
        (TH1F *)h_pT_Nucl->ProjectionY(TString::Format("h_pT_Nucl_Proj"), h_pT_Nucl->GetXaxis()->FindBin(0.5 * 0.001),
                                       h_pT_Nucl->GetXaxis()->FindBin(299.5 * 0.001));
    ...
    TFile fOutputFile(TString::Format("%s/%s_mT%i_B%i_DV%i_FV%i_S%i.root", OUTPUT_FOLDER.Data(), Description, mt_bin,
                                      Bootstrap, DataVar, FitVar, SEED),
                      "recreate");
    ...
    pi_d_Tree->Branch("data_type", &DATA_TYPE,
                      "data_type/I"); // ABC, A=0 => no boot, A=1 = bootstrap, BC = variation ID. If negative => default
    ...
    if (Kitty.GetNumSourcePars() > 1)
        CkKitty_Default.SetSourcePar(1, Kitty.GetAnaSourcePar(1));
    ...
    for (int uIter = 0; uIter < NumIter; uIter++) {
        ...
        do {
            ...
            if (mt_bin == -1) {
                if (iDataVar == -1 || iDataVar == -2) {
                    ...
                    InputDataFileName =
                        TString::Format("%s/InputFiles/CF_Pid_Var%i.root", INPUT_FOLDER.Data(), iDataVar + 1);
                ...
                if (fList1)
                    fList2 = (TList *)fList1->FindObject(MeListName2);
                if (fList2)
                    hME = (TH1F *)fList2->FindObject(MeHistName);
                if (hME) {
                    hME->GetXaxis()->SetLimits(hME->GetXaxis()->GetXmin() * 1000., hME->GetXaxis()->GetXmax() * 1000.);
                }
                if (fList2)
                    hSE = (TH1F *)fList2->FindObject(SeHistName);
                if (hSE) {
            ...
            // mt int pim_d used for approvals at CF and PF
                ...
                if (iDataVar == -1 || iDataVar == -2) {
                    InputDataFileName =
                        TString::Format("%s/InputFiles/CF_AntiPiondeuteron_Var0.root", INPUT_FOLDER.Data());
                    DefaultHistoName = "hCk_ReweightedPidVar0MeV_0";
                    MeListName1 = "PairDist";
                    MeListName2 = "PairReweighted";
                    MeHistName = "hTotalME _Rebinned_5_Reweighted";
                    ...
                    InputDataFileName =
                        TString::Format("%s/InputFiles/CF_AntiPid_Var%i.root", INPUT_FOLDER.Data(), iDataVar + 1);
                    DefaultHistoName = TString::Format("hCk_ReweightedReweightedPidVar%iMeV_1MeV_0", iDataVar + 1);
                    MeListName1 = "PairDist";
                    MeListName2 = "PairReweighted";
                    MeHistName = TString::Format("hTotalME_PidVar%i_Rebinned_5_Reweighted", iDataVar + 1);
            ...
            // mt diff pip_d used for approvals at PF
                ...
                if (iDataVar == -1 || iDataVar == -2) {
                    InputDataFileName = TString::Format("%s/InputFiles/mTBin%i/CF_Piondeuteron_Var0.root",
                    ...
                    InputDataFileName = TString::Format("%s/InputFiles/mTBin%i/CF_Pid_Var%i.root", INPUT_FOLDER.Data(),
                                                        abs(mt_bin) % 100 + 1, iDataVar + 1);
                    DefaultHistoName = TString::Format("hCk_ReweightedReweightedPidVar%iMeV_1MeV_0", iDataVar + 1);
                    MeListName1 = "PairDist";
                    MeListName2 = "PairReweighted";
                    MeHistName = TString::Format("hTotalME_PidVar%i_Rebinned_5_Reweighted", iDataVar + 1);
        ...
        if (mt_bin < 0)
            LAM_GEN = lambda_gen_mTint;
        else
            LAM_GEN = lambda_gen_avg.at(mt_bin % 100);
        ...
        if (FitVar) {
            ...
            if (LAM_GEN >= 0) {
                if (mt_bin < 0)
                    LAM_GEN = lambda_gen_mTint * lambda_gen_relvar.at(rangen.Integer(lambda_gen_relvar.size()));
                else
                    LAM_GEN = lambda_gen_avg.at(mt_bin % 100) *
                              lambda_gen_relvar.at(rangen.Integer(lambda_gen_relvar.size()));
            }
            if (AMP_DELTA >= 0)
                AMP_DELTA = amplitude_delta.at(rangen.Integer(amplitude_delta.size()));
        ...
        do {
            ...
            if (LAM_GEN >= 0)
                ...
                if (mt_bin < 0) {
                    ...
                    fData->SetParLimits(2, fabs(lambda_gen_mTint * lambda_gen_relvar.at(0)),
                                        fabs(lambda_gen_mTint * lambda_gen_relvar.at(1)));
                } else {
                    fData->SetParameter(2, fabs(lambda_gen_avg.at(mt_bin % 100)));
                    fData->SetParLimits(2, fabs(lambda_gen_avg.at(mt_bin % 100) * lambda_gen_relvar.at(0)),
                                        fabs(lambda_gen_avg.at(mt_bin % 100) * lambda_gen_relvar.at(1)));
                }
            }

            if (AMP_DELTA >= 0)
                fData->FixParameter(4, AMP_DELTA);
            else {
            ...
            if (mt_bin < 0) {
                ...
                fData->FixParameter(12, pip_gamma_Dpp.at(mt_bin % 100) * Dpp_weight +
                                            pip_gamma_D0.at(mt_bin % 100) * (1. - Dpp_weight)); // gamma
                fData->SetParameter(14, pip_temp_Dpp.at(mt_bin % 100) * Dpp_weight +
                                            pip_temp_D0.at(mt_bin % 100) * (1. - Dpp_weight)); // temp
            }
            ...
            for (unsigned uBin = 0; uBin < hDataToFit->GetNbinsX(); uBin++) {
                ...
                if (hDataToFit->GetBinError(uBin + 1))
                    CHI2_500 += pow((hDataToFit->GetBinContent(uBin + 1) - fData->Eval(MOM)) /
                                        hDataToFit->GetBinError(uBin + 1),
                                    2.);
                else
            ...
            if (hME) {
                for (unsigned uBin = 0; uBin < hME->GetNbinsX(); uBin++) {
                    ...
                    if (MOM < FIT_RANGE) {
                        Integral_Primary +=
                            hME->GetBinContent(uBin + 1) * (gFemto.Eval(MOM) - (1. - par_fmt)); // original
