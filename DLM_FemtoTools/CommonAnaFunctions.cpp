...
//   no sor var so far
void DLM_CommonAnaFunctions::SetUpCats_pp(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                          const int &SourceVar) {
    ...
    if (SOURCE == "NULL" || SOURCE == "") {
        ...
        if (SourceVar == 0) {
            CleverMcLevyReso[0].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
            CleverMcLevyReso[0].SetUpReso(1, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards);
        } else {
            CleverMcLevyReso[0].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom); // rdtBackwards,rdtRandom
            CleverMcLevyReso[0].SetUpReso(1, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
        ...
        if (SourceVar == 2) {
            DLM_Histo<double> *HISTO =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 400, 700);
        ...
        if (SourceVar == 2) {
            DLM_Histo<double> *HISTO =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 400, 600);
            // DLM_Histo<double>* HISTO = ConvertThetaAngleHisto(
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        // EPOS, 2 is with fixed mass, 3 is with EPOS mass, 4 is 3 body with fixed mass, 5 is 3 body with EPOS mass
            ...
            if (SourceVar % 100 == 4 || SourceVar % 100 == 5)
                F_EposDisto_p_pReso =
                    new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/Only3body_p_pReso_3body.root");
            else
                F_EposDisto_p_pReso = new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_p_pReso.root");
            // printf("F_EposDisto_p_pReso=%p
            ...
            if (SourceVar % 100 == 4 || SourceVar % 100 == 5)
                F_EposDisto_pReso_pReso =
                    new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/Only3body_pReso_pReso_3body.root");
            else
                F_EposDisto_pReso_pReso =
                    new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_pReso_pReso.root");
            // printf("F_EposDisto_p_pReso=%p
        ...
        Kitty.SetInputFileName("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/EPOS_OUTPUT_FILES/"
                               "EPOS_LBF_pp91/pp_nist0_030219.f19");
    ...
    if (POT == "AV18" || POT == "AV18_s" || POT == "AV18_sp") {
        ...
        if (PotVar == 1)
            POT_FLAG = v18_SingleChannelMagic;
...
void DLM_CommonAnaFunctions::SetUpCats_ppic(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                            const int &SourceVar) {
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        for (unsigned uEntry = 0; uEntry < T_EposDisto_p_pi->GetEntries(); uEntry++) {
            ...
            if (Type == PPid) {
                ...
                Tau2 = 0;
                fM1 = SourceVar >= 0 ? 1362 : 1462; // this was wrong all along for the ppic, had the pionReso mass
                                                    // instead of the protonReso
                if (k_D > k_CutOff)
                    continue;
                RanVal1 = RanGen.Exponential(fM1 / (fP1 * Tau1));
    ...
    if (SourceVar < 0)
        RED_MASS = Mass_L * Mass_pic / (Mass_L + Mass_pic);
...
void DLM_CommonAnaFunctions::SetUpCats_pipi(CATS &Kitty, const TString &SOURCE, const int &SourceVar) {
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        // back-to-back
            ...
            printf("\033[1;33mWARNING:\033[0m The CommonAnaFunction is still under construction (McGauss_ResoTM "
                   "back-to-back)\n");
            goto CLEAN_SetUpCats_pipi;
        }
        // EPOS, 2(4) is with fixed mass, 3 is with EPOS mass
        // 2 is with omega included, 4 is without omega, 8 is neglecting the angular distr. from EPOS and taking them
        // random
        else {
            unsigned NumPart = 0;
            unsigned NumOmega = 0;
            ...
            TFile *F_EposDisto_p_pReso =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/ForMax_pi_piReso_withOmega.root");
            TNtuple *T_EposDisto_p_pReso = (TNtuple *)F_EposDisto_p_pReso->Get("InfoTuple_ClosePairs");
            unsigned N_EposDisto_p_pReso = T_EposDisto_p_pReso->GetEntries();
            T_EposDisto_p_pReso->SetBranchAddress("k_D", &k_D);
            T_EposDisto_p_pReso->SetBranchAddress("P1", &fP1);
            ...
            TFile *F_EposDisto_pReso_pReso =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/ForMax_piReso_piReso_withOmega.root");
            TNtuple *T_EposDisto_pReso_pReso = (TNtuple *)F_EposDisto_pReso_pReso->Get("InfoTuple_ClosePairs");
            unsigned N_EposDisto_pReso_pReso = T_EposDisto_pReso_pReso->GetEntries();
            T_EposDisto_pReso_pReso->SetBranchAddress("k_D", &k_D);
            T_EposDisto_pReso_pReso->SetBranchAddress("P1", &fP1);
...
void DLM_CommonAnaFunctions::SetUpCats_pipi_broken(CATS &Kitty, const TString &SOURCE, const int &SourceVar) {
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        // EPOS, 2 is with fixed mass, 3 is with EPOS mass
            ...
            TFile *F_EposDisto_p_pReso =
                new TFile("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/MixedEvents/"
                          "Max/WithoutOmega/ForMax_pi_piReso.root");
            TNtuple *T_EposDisto_p_pReso = (TNtuple *)F_EposDisto_p_pReso->Get("InfoTuple_ClosePairs");
            unsigned N_EposDisto_p_pReso = T_EposDisto_p_pReso->GetEntries();
            T_EposDisto_p_pReso->SetBranchAddress("k_D", &k_D);
            T_EposDisto_p_pReso->SetBranchAddress("P1", &fP1);
            ...
            TFile *F_EposDisto_pReso_pReso =
                new TFile("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/MixedEvents/"
                          "Max/WithoutOmega/ForMax_piReso_piReso.root");
            TNtuple *T_EposDisto_pReso_pReso = (TNtuple *)F_EposDisto_pReso_pReso->Get("InfoTuple_ClosePairs");
            unsigned N_EposDisto_pReso_pReso = T_EposDisto_pReso_pReso->GetEntries();
            T_EposDisto_pReso_pReso->SetBranchAddress("k_D", &k_D);
            T_EposDisto_pReso_pReso->SetBranchAddress("P1", &fP1);
...
//   XXX are the cutoff
void DLM_CommonAnaFunctions::SetUpCats_pL(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                          const int &SourceVar) {
    CATSparameters *cPars = NULL;
    ...
    if (SOURCE == "NULL" || SOURCE == "") {
        ...
        printf(
            "\033[1;33mWARNING:\033[0m The CommonAnaFunction is still under construction (GaussExpTotSimple_2body)\n");
        ...
        if (SourceVar == 0) {
            CleverMcLevyReso[1].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
            CleverMcLevyReso[1].SetUpReso(1, 0, 1. - 0.3562, 1462.93, 4.69, Mass_L, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards);
        } else {
            CleverMcLevyReso[1].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom); // rdtBackwards,rdtRandom
        ...
        if (SourceVar == 2) {
            ...
            DLM_Histo<double> *HISTO_p =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 400, 600);
            DLM_Histo<double> *HISTO_L =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 270, 470);
        ...
        // improved reso
            ...
            CleverMcLevyReso[1].SetUpReso(0, 0, 0.4368, 1231.98, 1.67, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom);
            CleverMcLevyReso[1].SetUpReso(0, 1, 0.2054, 1636.98, 1.62, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom);
            CleverMcLevyReso[1].SetUpReso(1, 0, 0.4864, 1384.54, 5.34, Mass_L, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom);
            CleverMcLevyReso[1].SetUpReso(1, 1, 0.1573, 1705.26, 2.70, Mass_L, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom);

            DLM_Histo<double> *HISTO_p_0 =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 200, 400);
            DLM_Histo<double> *HISTO_p_1 =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 880, 1000);
            DLM_Histo<double> *HISTO_L_0 =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 160, 360);
            DLM_Histo<double> *HISTO_L_1 =
                ConvertThetaAngleHisto("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/Using_CATS3/Output/"
                                       "MixedEvents/AngleStudy_1/EPOS_HM_4PI/pp/DimiPhi_pp.root",
                                       "h_rkAngle_Mom2", 620, 820);
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        // EPOS, 2 is with fixed mass, 3 is with EPOS mass
            ...
            TFile *F_EposDisto_p_LamReso =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_p_LamReso.root");
            ...
            TFile *F_EposDisto_pReso_Lam =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_pReso_Lam.root");
            ...
            TFile *F_EposDisto_pReso_LamReso =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_pReso_LamReso.root");
    ...
    if (POT == "LO") {
        ExternalWF = Init_pL_Haidenbauer(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pLambdaLO_600/", Kitty, 0, 600);
        NumChannels = 2;
    } else if (POT == "LO_Coupled_S") {
        ExternalWF =
            Init_pL_Haidenbauer(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pLambdaLO_Coupling/", Kitty, 1, 600);
        NumChannels = 4;
    } else if (POT == "NLO") {
        ExternalWF = Init_pL_Haidenbauer(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pLambdaNLO/", Kitty, 10, 600);
        NumChannels = 2;
    }
    // s and p waves
    else if (POT == "NLO_sp") {
        ExternalWF = Init_pL_Haidenbauer(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pLambdaNLO/", Kitty, 12, 600);
        NumChannels = 4;
    } else if (POT == "NLO_Coupled_S") {
        ExternalWF =
            Init_pL_Haidenbauer(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pLambdaNLO_Coupling/", Kitty, 11, 600);
        NumChannels = 4;
        ...
        //
            ...
            ExternalWF = Init_pL_Haidenbauer2019(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pLambdaN2LO/", Kitty,
                                                 TYPE, CUTOFF);
        } else {
    ...
    for (unsigned uCh = 0; uCh < NumChannels; uCh++) {
        ...
        if ((POT == "UsmaniFit" || POT == "UsmaniPLB") && cPotPars1S0 && uCh == 0)
            ...
            if (false) {
                ...
                // Kitty.UseExternalWaveFunction(uMomBin,uCh,0,WaveFunctionU[uMomBin][uCh][0], NumRadBins, RadBins,
                // PhaseShifts[uMomBin][uCh][0]); printf("Look at that view (%u)!\n",uCh);
...
void DLM_CommonAnaFunctions::SetUpCats_pSp(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                           const int &SourceVar) {
    CATSparameters *cPars = NULL;
    CATSparameters *pPars = NULL;
    ...
    // the WF obtained from Johann
        ...
        ExternalWF = Init_pSigmaPlus_Haidenbauer(CatsFilesFolder[0] + "/Interaction/Haidenbauer/pSigmaPlus/", Kitty, 0);
        NumChannels = 2;
    } else {
...
void DLM_CommonAnaFunctions::SetUpCats_pS0(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                           const int &SourceVar) {
    CATSparameters *cPars = NULL;
...
void DLM_CommonAnaFunctions::SetUpCats_pXim(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                            const int &SourceVar) {
    ...
    if (SOURCE == "NULL" || SOURCE == "") {
        ...
        if (SourceVar == 0) {
            CleverMcLevyReso[2].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
        } else {
            CleverMcLevyReso[2].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom); // rdtBackwards,rdtRandom
        ...
        if (SourceVar == 0) {
            CleverMcLevyReso[2].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
            CleverMcLevyReso[2].SetUpReso(1, 0, 0.0001, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
        } else {
            CleverMcLevyReso[2].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom); // rdtBackwards,rdtRandom
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        // EPOS, 2 is with fixed mass, 3 is with EPOS mass
            ...
            TFile *F_EposDisto_pReso_Xim =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_pReso_Xim.root");
...
void DLM_CommonAnaFunctions::SetUpCats_pXi0(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                            const int &SourceVar) {
...
void DLM_CommonAnaFunctions::SetUpCats_pOmegam(CATS &Kitty, const TString &POT, const TString &SOURCE,
                                               const int &PotVar, const int &SourceVar) {
    CATSparameters *cPars = NULL;
    CATSparameters *pPars = NULL;
    ...
    if (SOURCE == "Gauss") {
        ...
        if (SourceVar == 0) {
            CleverMcLevyReso[3].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
        } else {
            CleverMcLevyReso[3].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom); // rdtBackwards,rdtRandom
        ...
        if (SourceVar == 0) {
            CleverMcLevyReso[3].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards); // rdtBackwards,rdtRandom
            CleverMcLevyReso[3].SetUpReso(1, 0, 0.0001, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtBackwards);
        } else {
            CleverMcLevyReso[3].SetUpReso(0, 0, 1. - 0.3578, 1361.52, 1.65, Mass_p, Mass_pic, false, false,
                                          DLM_CleverMcLevyReso::rdtRandom); // rdtBackwards,rdtRandom
    ...
    //(SourceVar/10)*10 is the cutoff value (e.g. 192 is cutoff value of 190 and type 2)
        ...
        // EPOS, 2 is with fixed mass, 3 is with EPOS mass
            ...
            TFile *F_EposDisto_pReso_Omega =
                new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/EposDisto_pReso_Omega.root");
...
void DLM_CommonAnaFunctions::SetUpCats_LKVidana(CATS &Kitty, const TString &SOURCE, const TString &DataSample) {
    ...
    if (DataSample == "pp13TeV_HM_LKMAC") {
        ...
        lam_source = 0.9806;
        ExternalWF =
            Init_LAntiK_Vidana("/home/valentina/thor/cernbox/CATSFiles/Models_WFs_Theoreticians/Vidana/", Kitty, 0);
    }
    if (SOURCE == "Gauss") {
        ...
        Kitty.SetNormalizedSource(
            true); // do not touch the source, set to true so CATS adds (1-λs), if false it changes the distribution
    } else {
        printf("\033[1;31mERROR:\033[0m Non-existing source '%s'\n", SOURCE.Data());
    ...
    for (unsigned uCh = 0; uCh < NumChannels; uCh++) {
        if (ExternalWF) {
            ...
            printf(
                "\033[1;31mERROR:\033[0m SetUpCats_LKVidana says that you should NEVER see this message! BIG BUG!\n");
            goto CLEAN_SetUpCats_pApHaide;
        }
...
void DLM_CommonAnaFunctions::SetUpCats_Lcp(CATS &Kitty, const TString &SOURCE, const int &TYPE, const int &CUTOFF) {
    ...
    rad1 = 1.2;
    ExternalWF = Init_Lcp_Haidenbauer("/Users/sartozza/cernbox/CATSFiles/Models_WFs_Theoreticians/Johann/Lc_Proton/",
                                      Kitty, TYPE, CUTOFF);
...
DLM_Ck *DLM_CommonAnaFunctions::SetUpLednicky_pL(const unsigned &NumMomBins, const double *MomBins,
                                                 const TString &POT) {
    double p0, p1, p2, p3;
    if (POT == "Lednicky_ND") {
...
void DLM_CommonAnaFunctions::SetUpBinning_pp(const TString &DataSample, unsigned &NumMomBins, double *&MomBins,
                                             double *&FitRegion, const int &MomBinVar, const int &FitRegVar) {
    double kMin;
    double kMax;
    double kStep;
    if (DataSample == "pp13TeV_MB_Run2paper") {
        if (MomBinVar == 0) {
            ...
            return;
               ...
               DataSample == "pp13TeV_HM_RotPhiDec19" || DataSample == "pp13TeV_HM_DimiJun20" ||
               DataSample == "pp13TeV_HM_DimiJul20" || DataSample == "pp13TeV_HM_DimiMay21" ||
               DataSample == "pp13TeV_HM_DimiJun21") {
        if (MomBinVar == 0) {
            kMin = 4;
            kStep = 4;
            kMax = 376; //(i.e. max=376 MeV)
...
void DLM_CommonAnaFunctions::SetUpBinning_pL(const TString &DataSample, unsigned &NumMomBins, double *&MomBins,
                                             double *&FitRegion, const int &MomBinVar, const int &FitRegVar) {
    ...
    if (DataSample == "pp13TeV_MB_Run2paper") {
        ...
        unsigned NumCoarseBinsAbove =
            floor((kMax - double(NumCoarseBinsBelow) * kCoarseStep - double(NumFineBins) * kFineStep) / kCoarseStep);
...
void DLM_CommonAnaFunctions::SetUpBinning_LKmin(const TString &DataSample, unsigned &NumMomBins, double *&MomBins,
                                                double *&FitRegion, const int &MomBinVar, const int &FitRegVar) {
    double kMin;
    double kMax;
    double kStep;
...
void DLM_CommonAnaFunctions::GetPurities_L(const TString &DataSample, const int &Variation, double *Purities) {
    ...
    if (DataSample == "pp13TeV_MB_Run2paper") {
        PurityLambda = 0.96768;
               ...
               DataSample == "pp13TeV_HM_RotPhiDec19") {
        ...
        if (Variation == 0)
            ...
            PurityLambda = 0.9595;
               ...
               DataSample == "pp13TeV_HM_DimiMay21" || DataSample == "pp13TeV_HM_DimiJun21") {
        PurityLambda = 1.0; // use for SB corrected correlations
    } else if (DataSample == "pPb5TeV_Run2paper") {
...
void DLM_CommonAnaFunctions::GetPurities_L_Vale(const TString &DataSample, const int &Variation, double *Purities,
                                                int SamplePurity) {
    double PurityLambdaVars[45] = {0.9465,   0.965985, 0.955872, 0.966306, 0.956546, 0.965578, 0.965118, 0.967649,
                                   0.951028, 0.958578, 0.958875, 0.958578, 0.956233,
...
void DLM_CommonAnaFunctions::GetPurities_K(const TString &DataSample, const int &Variation, double *Purities,
                                           int SamplePurity) {
    double PurityKaonVars[45] = {0.997177, 0.997259, 0.997129, 0.997177, 0.997093, 0.994909, 0.99508,  0.994934,
                                 0.997216, 0.997183, 0.997093, 0.994909, 0.995998, 0.996075, 0.996066, 0.996075,
                                 0.994954, 0.994928, 0.996075, 0.996031, 0.997259, 0.994892, 0.997093, 0.994903,
                                 0.994985, 0.997158, 0.996066, 0.996079, 0.997259, 0.996009, 0.997098, 0.996042,
                                 0.994996, 0.997139, 0.997104, 0.994965, 0.997062, 0.996085, 0.997221, 0.99616,
...
void DLM_CommonAnaFunctions::GetFractions_p(const TString &DataSample, const int &Variation, double *Fractions) {
    ...
    // ratio between feed-down lambdas and all feed downs, it is by default 0.7, i.e. 70% of the feed-down into protons
    // is from lambdas
...
void DLM_CommonAnaFunctions::GetFractions_L(const TString &DataSample, const int &Variation, double *Fractions) {
    ...
    // the fraction could be lower. You can modify it here. This is by how much the PrimLambda fractional yield is
    // reduced, compared to the value at the average pT
...
void DLM_CommonAnaFunctions::GetFractions_Xim(const TString &DataSample, const int &Variation, double *Fractions) {
    ...
    // the ratios that we have for Xis are referred to the total number of Xi particles (which already include all
    // contributions) hence Xi1530_to_Xi indeed is simply the number of Xis that stem from a Xi1530
...
void DLM_CommonAnaFunctions::GetFractions_K(const TString &DataSample, const int &Variation, double *Fractions) {
    ...
    // ratio between feed-down Phi and all feed downs, it is by default 0.06, i.e. 6% of the feed-down into Kaons is
    // from Phi
    double arrayPercLamProton = pp_f1 / (1. - pp_f0) * Modify_pp;
    Fractions[0] = pp_f0;
    Fractions[1] = (1. - pp_f0) * (arrayPercLamProton);
...
// 2 is flat feed
// 3 is missid
void DLM_CommonAnaFunctions::SetUpLambdaPars_pp(const TString &DataSample, const int &Variation_p,
                                                double *lambda_pars) {
    double Purities_p[4];
    double Fraction_p[4];
...
// the last two digits are passed to the variations of the fraction (last digit is for Lambda, the second to last for
// Xi) the first two digits are passed to the purities.
void DLM_CommonAnaFunctions::SetUpLambdaPars_pL(const TString &DataSample, const int &Variation_p,
                                                const int &Variation_L, double *lambda_pars) {
    double Purities_p[4];
    double Fraction_p[4];
    double Purities_L[5];
    double Fraction_L[5];
...
// 3
// 4 is missid
void DLM_CommonAnaFunctions::SetUpLambdaPars_pXim(const TString &DataSample, const int &Variation_p,
                                                  const int &Variation_Xim, double *lambda_pars) {
    double Purities_p[4];
    double Fraction_p[4];
    ...
    lambda_pars[4] =
        Purities_p[0] * Purities_Xim[4] + Purities_p[3] * Purities_Xim[0] + Purities_p[3] * Purities_Xim[4];
    lambda_pars[3] = 1. - lambda_pars[0] - lambda_pars[1] - lambda_pars[2] - lambda_pars[4];
    /*
        double SUM=0;
        printf("pp * fp * pxi * fxi = %.3f * %.3f * %.3f *
       %.3f\n",Purities_p[0],Fraction_p[0],Purities_Xim[0],Fraction_Xim[0]); for(unsigned uLam=0; uLam<5; uLam++){
            printf("λ(pΞ)_%u = %.1f\n",uLam,lambda_pars[uLam]*100.);
            SUM+=lambda_pars[uLam]*100.;
        }
        printf("SUM: %.1f\n------------\n",SUM);
...
// the last two digits are passed to the variations of the fraction (last digit is for Lambda, the second to last for
// Xi) the first two digits are passed to the purities.
void DLM_CommonAnaFunctions::SetUpLambdaPars_LKmin(const TString &DataSample, const int &Variation_L,
                                                   const int &Variation_K, double *lambda_pars, int SamplePurity) {
...
TH2F *DLM_CommonAnaFunctions::GetResolutionMatrix(const TString &DataSample, const TString &&System) {
    ...
    if (DataSample == "pp13TeV_MB_Run2paper") {
        ...
        FileName = "/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/CorrelationFiles_2018/ALICE_pPb_5TeV/"
                   "ResolutionMatrices/Sample3_MeV_compact.root";
    } else if (DataSample == "pp13TeV_HM_LK") {
        FileName =
            "/home/valentina/thor/cernbox/CATSFiles/SystematicsAndCalib/MomentumSmear/ALICE_pp_13TeVHM_MELKmin.root";
...
TH1F *DLM_CommonAnaFunctions::GetAliceExpCorrFun(const TString &DataSample, const TString &System,
                                                 const TString &CutVar, const int &iReb, const bool &AddSyst,
                                                 const int mTbin) {
    TString FileName;
    TString HistoName;
    ...
    if (DataSample == "pp13TeV_MB_Run2paper") {
        if (System == "pp") {
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pp_13TeV/Sample8/CFOutput_pp%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
            }
        } else if (System == "pLambda") {
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pp_13TeV/Sample8/CFOutput_pL%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pp_13TeV/Sample8/CFOutput_LL%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pp_13TeV/Sample8/CFOutput_pXi%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
        ...
        if (System == "pp") {
            if (mTbin == -1) {
                ...
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/Sample12HM/CFOutput_pp%s.root", CutVar.Data());
                if (CutVar == "_0")
                    FileName = CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/Sample10HM_ver2/CFOutput_pp.root";
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else if (mTbin >= 0 && mTbin < 7) {
                FileName = TString::Format(CatsFilesFolder[0] +
                                               "/ExpData/ALICE_pp_13TeV_HM/Sample12HM/CFOutputALL_mT_pp_HM%s.root",
                                           CutVar.Data());
                // ugly solution to make sure that the default fit combination is the correlation we have showed at the
                // preliminaries preview
                if (CutVar == "_0")
                    FileName =
                        CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/Sample10HM_ver2/CFOutputALL_mT_pp_HM.root";
            ...
            if (mTbin == -1) {
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/Sample12HM/CFOutput_pL%s.root", CutVar.Data());
                // ugly solution to make sure that the default fit combination is the correlation we have showed at the
                // preliminaries preview
                if (CutVar == "_0")
                ...
                FileName = TString::Format(CatsFilesFolder[0] +
                                               "/ExpData/ALICE_pp_13TeV_HM/Sample12HM/CFOutputALL_mT_pL_HM%s.root",
                                           CutVar.Data());
                // ugly solution to make sure that the default fit combination is the correlation we have showed at the
                // preliminaries preview
                if (CutVar == "_0")
            ...
            if (mTbin == -1) {
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/Sample12HM/CFOutput_LL%s.root", CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
            ...
            if (mTbin == -1) {
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/Sample12HM/CFOutput_pXi%s.root", CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
    ...
    // used for the pLambda paper proposal in June 2020, sideband corrected (folded)
        ...
        if (System == "pLambda") {
            if (mTbin == -1) {
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/DimiJun20/Norm240_340/DataSignal/CkSB_pL_%s.root",
                    CutVar.Data());
        ...
        if (System == "pLambda") {
            if (mTbin == -1) {
                // FileName =
                // TString::Format(CatsFilesFolder[0]+"/ExpData/ALICE_pp_13TeV_HM/DimiJun20/Norm240_340/DataSignal/Unfolded/090720/CkSB_pL_%s.root",CutVar.Data());
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/DimiJun20/Norm240_340/DataSignal/CkSB_pL_%s.root",
                    CutVar.Data());
                HistoName = TString::Format("hCkS_Norm_%.0fMeV", float(iReb + 1) * 4.);
        ...
        if (System == "pLambda") {
            if (mTbin == -1) {
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/DimiMay21/UnfoldedNorm240_340/CkSB_pL_%s.root",
                    CutVar.Data());
                HistoName = TString::Format("hCkS_Norm_%.0fMeV", float(iReb + 1) * 4.);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
        ...
        if (System == "pLambda") {
            if (mTbin == -1) {
                FileName = TString::Format(
                    CatsFilesFolder[0] + "/ExpData/ALICE_pp_13TeV_HM/DimiJun21/UnfoldedNorm240_340/CkSB_pL_%s.root",
                    CutVar.Data());
                HistoName = TString::Format("hCkS_Norm_%.0fMeV", float(iReb + 1) * 4.);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
        ...
        if (System == "pp") {
            if (mTbin == -1) {
                FileName = TString::Format(
                    "/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/CorrelationFiles_2018/"
                    "pp13TeV_HM_Baseline/MyResults_Vale/Data/CF/ME_Methods_14122019/CFOutput_pp_3.root");
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
            ...
            if (mTbin == -1) {
                FileName = TString::Format(
                    "/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/CorrelationFiles_2018/"
                    "pp13TeV_HM_Baseline/MyResults_Vale/Data/CF/ME_Methods_14122019/CFOutput_pL_3.root");
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
            ...
            if (mTbin == -1) {
                FileName = TString::Format(
                    "/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/CorrelationFiles_2018/"
                    "pp13TeV_HM_Baseline/MyResults_Vale/Data/CF/ME_Methods_14122019/CFOutput_LL_3.root");
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
            ...
            if (mTbin == -1) {
                FileName = TString::Format(
                    "/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/CorrelationFiles_2018/"
                    "pp13TeV_HM_Baseline/MyResults_Vale/Data/CF/ME_Methods_14122019/CFOutput_pXi_3.root");
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
                       System.Data());
    ...
    // the data used by Bernie for the source paper. Only pp and pLambda
        ...
        if (System == "pp") {
            if (mTbin >= 0 && mTbin < 7) {
                FileName = TString::Format(CatsFilesFolder[0] +
                                               "/ExpData/Bernie_Source/ppData/mTBin_%i/CFOutput_mT_ppVar%s_HM_%i.root",
                                           mTbin + 1, CutVar.Data(), mTbin);
            ...
            if (mTbin >= 0 && mTbin < 6) {
                FileName = TString::Format(CatsFilesFolder[0] +
                                               "/ExpData/Bernie_Source/pLData/mTBin_%i/CFOutput_mT_pLVar%s_HM_%i.root",
                                           mTbin + 1, CutVar.Data(), mTbin);
        ...
        if (System == "pp") {
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample10/CFOutput_pp%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample10/CFOutput_pL%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample10/CFOutput_LL%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample10/CFOutput_pXi%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
        ...
        if (System == "pp") {
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample11/CFOutput_pp%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample11/CFOutput_pL%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample11/CFOutput_LL%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
            ...
            if (mTbin == -1) {
                FileName = TString::Format("/home/dmihaylov/Dudek_Ubuntu/Work/Kclus/GeneralFemtoStuff/"
                                           "CorrelationFiles_2018/ALICE_pPb_5TeV/Sample11/CFOutput_pXi%s.root",
                                           CutVar.Data());
                HistoName = TString::Format("hCk_ReweightedMeV_%i", iReb);
            } else {
                printf("\033[1;31mERROR:\033[0m The mT bin #%i is not defined for %s (%s)\n", mTbin, DataSample.Data(),
        ...
        if (FileROOT) {
            ...
            // in this case the systematics are absolute!!!
                ...
                for (int iBin = 1; iBin <= histoCopy->GetNbinsX(); iBin++) {
                    ...
                    // printf("System=%s; iBin=%i; Ck=%.4f; stat=%.4f; syst=%.4f;
...
//    The "CECA" based source with SourceVar==1 is the on straight out of the box, using the params from the CECA paper
//    (full fit)
void DLM_CommonAnaFunctions::SetUpCats_Kd(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                          const int &SourceVar) {
    ...
    //(SourceVar/10)*10 is the cutoff value
        ...
        F_EposDisto_Kreso_d =
            new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/TEMP/SecondTry/ALL_D_KaonReso.root");
...
void DLM_CommonAnaFunctions::SetUpCats_pi_d(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar,
                                            const int &SourceVar) {
    ...
    //(SourceVar/10)*10 is the cutoff value
        ...
        if (SourceVar % 10 == 1)
            CleverMcLevyResoTM[8].SetUpReso(1, 0.682 * 1.1); // pions
        else if (SourceVar % 10 == 2)
            CleverMcLevyResoTM[8].SetUpReso(1, 0.682 * 0.9); // pions
        else
        ...
        F_EposDisto_d_piReso =
            new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/TEMP/SecondTry/ALL_D_piReso.root");
        ...
        for (unsigned uEntry = 0; uEntry < N_EposDisto_d_piReso; uEntry++) {
            ...
            // for primoridials (the Xis) we put 0
            Tau2 = 1.50;
            if (SourceVar / 10000 == 1)
                Tau2 *= 1.1;
            else if (SourceVar / 10000 == 2)
                Tau2 *= 0.9;
            // put in the average mass of the resonances (again from SHM or TF)
            // this is the value for pions
            fM2 = 1124;
            if ((SourceVar / 1000) % 10 == 1)
                fM2 *= 1.1;
            else if ((SourceVar / 1000) % 10 == 2)
                fM2 *= 0.9;
            // generate a random path length for the propagation of the resonances
            // nothing to change!
            RanVal2 = RanGen.Exponential(fM2 / (fP2 * Tau2));
...
DLM_CleverMcLevyReso *DLM_CommonAnaFunctions::GetCleverMcLevyReso_pp() { return &CleverMcLevyReso[0]; }

DLM_CleverMcLevyReso *DLM_CommonAnaFunctions::GetCleverMcLevyReso_pL() { return &CleverMcLevyReso[1]; }

DLM_CleverMcLevyReso *DLM_CommonAnaFunctions::GetCleverMcLevyReso_pXim() { return &CleverMcLevyReso[2]; }

DLM_CleverMcLevyReso *DLM_CommonAnaFunctions::GetCleverMcLevyReso_pOmegam() { return &CleverMcLevyReso[3]; }

DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_pp() { return &CleverMcLevyResoTM[0]; }

DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_pL() { return &CleverMcLevyResoTM[1]; }

DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_pXim() { return &CleverMcLevyResoTM[2]; }
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_pOmegam() { return &CleverMcLevyResoTM[3]; }
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_pipi() { return &CleverMcLevyResoTM[4]; }
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_ppic() { return &CleverMcLevyResoTM[6]; }
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_Kd() { return &CleverMcLevyResoTM[7]; }
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GetCleverMcLevyResoTM_pi_d() { return &CleverMcLevyResoTM[8]; }
...
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GaussCoreRsm_LPi(const int &SourceVar) {
    ...
    TFile *F_EposDisto_pReso_Kaon =
        new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/ForMaxRamona_pi_SigReso.root");
    ...
    TFile *F_EposDisto_p_KaonReso =
        new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/ForMaxRamona_piReso_Sig.root");
    ...
    TFile *F_EposDisto_pReso_KaonReso =
        new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/ForMaxRamona_piReso_SigReso.root");
    // TFile* F_EposDisto_pReso_KaonReso = new
    // TFile(TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_pReso_dReso.root",GetCernBoxDimi()));
...
DLM_CleverMcLevyResoTM *DLM_CommonAnaFunctions::GaussCoreRsm_pK(const int &SourceVar) {
    ...
    TFile *F_EposDisto_pReso_KaonReso =
        new TFile(CatsFilesFolder[0] + "/Source/EposAngularDist/ForRamona_pReso_KaonReso.root");
    // TFile* F_EposDisto_pReso_KaonReso = new
    // TFile(TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_pReso_dReso.root",GetCernBoxDimi()));
...
void DLM_CommonAnaFunctions::SetCatsFilesFolder(const TString &folder) { CatsFilesFolder[0] = folder; }

DLM_Histo<double> *ConvertThetaAngleHisto(const TString &FileName, const TString &HistoName, const double kMin,
                                          const double kMax) {
    ...
    if (InputHisto) {
        Projection = InputHisto->ProjectionX("ConvertThetaAngleHisto", InputHisto->GetYaxis()->FindBin(kMin),
    ...
    CummDistr.SetUp(1);
    // printf("NumBins=%u; %f -->
    // %f\n",NumBins,Projection->GetBinLowEdge(1),Projection->GetXaxis()->GetBinUpEdge(NumBins)); usleep(4e6);
...
void RootFile_DlmSource(const TString &RootFileName, const TString &GraphName, CATS *Kitty, const unsigned &NumBins,
                        const double &rMin, const double &rMax, const double &lambda, const bool &FourPi) {
    TFile *RootFile = new TFile(RootFileName, "update");
    if (!RootFile)
        RootFile = new TFile(RootFileName, "recreate");
    TGraph graph;
    ...
    for (unsigned uBin = 0; uBin < NumBins; uBin++) {
        ...
        if (FourPi)
            ...
            graph.SetPoint(uBin, RAD,
                           Kitty->EvaluateTheSource(10, RAD, 0) * lambda / (4. * Pi * (RAD + 1e-6) * (RAD + 1e-6)));
    }
    graph.Write("", TObject::kOverwrite);
    delete RootFile;
}
...
double Get_reff_TF1(TH1F *hsource, TF1 *&fsource, const float lambda, const float CEI) {
    ...
    fsource =
        new TF1("fit4325", "[0]*4.*TMath::Pi()*x*x*pow(4.*TMath::Pi()*[1]*[1],-1.5)*exp(-(x*x)/(4.*[1]*[1]))+1.-[0]",
...
void SetUp_RSM_Flat(DLM_CleverMcLevyResoTM &MagicSource, const double frac1, const double frac2, const double MassP1,
                    const double MassP2, const double MassR1, const double MassR2, const double TauR1,
                    const double TauR2, const double TIMEOUT, const int flag) {
...
void SetUp_RSMflat_pp(DLM_CleverMcLevyResoTM &MagicSource) {
    ...
    SetUp_RSM_Flat(MagicSource, frac1, frac2, Mass_p, Mass_p, Mass_ProtonReso, Mass_ProtonReso, Tau_Proton, Tau_Proton,
                   16, 0);
}
...
void SetUp_RSM_pp(DLM_CleverMcLevyResoTM &MagicSource, const TString InputFolder, const int flag) {
    ...
    TFile *F_EposDisto_p_pReso;
    F_EposDisto_p_pReso =
        new TFile(TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_p_pReso.root", InputFolder.Data()));
    TNtuple *T_EposDisto_p_pReso = (TNtuple *)F_EposDisto_p_pReso->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_p_pReso = T_EposDisto_p_pReso->GetEntries();
    T_EposDisto_p_pReso->SetBranchAddress("k_D", &k_D);
    T_EposDisto_p_pReso->SetBranchAddress("P1", &fP1);
    ...
    TFile *F_EposDisto_pReso_pReso;
    F_EposDisto_pReso_pReso = new TFile(
        TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_pReso_pReso.root", InputFolder.Data()));
    TNtuple *T_EposDisto_pReso_pReso = (TNtuple *)F_EposDisto_pReso_pReso->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_pReso_pReso = T_EposDisto_pReso_pReso->GetEntries();
    T_EposDisto_pReso_pReso->SetBranchAddress("k_D", &k_D);
    T_EposDisto_pReso_pReso->SetBranchAddress("P1", &fP1);
...
void SetUp_RSMflat_pL(DLM_CleverMcLevyResoTM &MagicSource) {
    ...
    SetUp_RSM_Flat(MagicSource, frac1, frac2, Mass_p, Mass_L, Mass_ProtonReso, Mass_LambdaReso, Tau_Proton, Tau_Lambda,
                   16, 0);
}
...
void SetUp_RSM_pL(DLM_CleverMcLevyResoTM &MagicSource, const TString InputFolder, const int flag) {
    ...
    TFile *F_EposDisto_p_LamReso =
        new TFile(TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_p_LamReso.root", InputFolder.Data()));
    TNtuple *T_EposDisto_p_LamReso = (TNtuple *)F_EposDisto_p_LamReso->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_p_LamReso = T_EposDisto_p_LamReso->GetEntries();
    T_EposDisto_p_LamReso->SetBranchAddress("k_D", &k_D);
    T_EposDisto_p_LamReso->SetBranchAddress("P1", &fP1);
    ...
    TFile *F_EposDisto_pReso_Lam =
        new TFile(TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_pReso_Lam.root", InputFolder.Data()));
    TNtuple *T_EposDisto_pReso_Lam = (TNtuple *)F_EposDisto_pReso_Lam->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_pReso_Lam = T_EposDisto_pReso_Lam->GetEntries();
    T_EposDisto_pReso_Lam->SetBranchAddress("k_D", &k_D);
    T_EposDisto_pReso_Lam->SetBranchAddress("P1", &fP1);
    ...
    TFile *F_EposDisto_pReso_LamReso = new TFile(
        TString::Format("%s/CatsFiles/Source/EposAngularDist/EposDisto_pReso_LamReso.root", InputFolder.Data()));
    TNtuple *T_EposDisto_pReso_LamReso = (TNtuple *)F_EposDisto_pReso_LamReso->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_pReso_LamReso = T_EposDisto_pReso_LamReso->GetEntries();
    T_EposDisto_pReso_LamReso->SetBranchAddress("k_D", &k_D);
    T_EposDisto_pReso_LamReso->SetBranchAddress("P1", &fP1);
...
void SetUp_RSMflat_pXi(DLM_CleverMcLevyResoTM &MagicSource) {
    ...
    SetUp_RSM_Flat(MagicSource, frac1, frac2, Mass_p, Mass_Xim, Mass_ProtonReso, 0, Tau_Proton, 0, 16, 0);
}
...
void SetUp_RSM_pXi(DLM_CleverMcLevyResoTM &MagicSource, const TString InputFolder, const int flag) {
    ...
    TFile *F_EposDisto_pReso_Xim =
        new TFile(InputFolder + "/CatsFiles/Source/EposAngularDist/EposDisto_pReso_Xim.root");
    TNtuple *T_EposDisto_pReso_Xim = (TNtuple *)F_EposDisto_pReso_Xim->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_pReso_Xim = T_EposDisto_pReso_Xim->GetEntries();
    T_EposDisto_pReso_Xim->SetBranchAddress("k_D", &k_D);
    T_EposDisto_pReso_Xim->SetBranchAddress("P1", &fP1);
...
void SetUp_RSMflat_pOmega(DLM_CleverMcLevyResoTM &MagicSource) {
    ...
    SetUp_RSM_Flat(MagicSource, frac1, frac2, Mass_p, MassOmega, Mass_ProtonReso, 0, Tau_Proton, 0, 16, 0);
}
...
void SetUp_RSM_pOmega(DLM_CleverMcLevyResoTM &MagicSource, const TString InputFolder, const int flag) {
    ...
    TFile *F_EposDisto_pReso_Omega =
        new TFile(InputFolder + "/CatsFiles/Source/EposAngularDist/EposDisto_pReso_Omega.root");
    TNtuple *T_EposDisto_pReso_Omega = (TNtuple *)F_EposDisto_pReso_Omega->Get("InfoTuple_ClosePairs");
    unsigned N_EposDisto_pReso_Omega = T_EposDisto_pReso_Omega->GetEntries();
    T_EposDisto_pReso_Omega->SetBranchAddress("k_D", &k_D);
    T_EposDisto_pReso_Omega->SetBranchAddress("P1", &fP1);
...
double EffRangeExp(double *x, double *par) {
    ...
    // printf("AddFactor = %.3e\n",AddFactor);
    return PreFactor *
           (AddFactor + invf_cs + 0.5 * d_cs * pow(kstar, 2.) + ere4 * pow(kstar, 4.) + ere6 * pow(kstar, 6.));
}

bool GetScattParameters(CATS &Kitty, double &ScatLen, double &EffRan, TH1F *&hFit, TF1 *&fitSP, const int Nterms,
                        const bool Fixf0, const bool Fixd0, const unsigned short usCh) {
    ...
    for (unsigned uMom = 0; uMom < Kitty.GetNumMomBins(); uMom++) {
        ...
        if (uMom) {
            if (CURRENT_POINT * LAST_POINT < 0 && fabs(CURRENT_POINT - LAST_POINT) > 1000 &&
                Kitty.GetMomentum(uMom) < 120) {
    ...
    if (Fixf0) {
        fitSP2->FixParameter(1, inv_f0);
        fitSP4->FixParameter(1, inv_f0);
        fitSP6->FixParameter(1, inv_f0);
    } else {
        fitSP2->SetParameter(1, inv_f0);
        fitSP2->SetParLimits(1, -100, 100);
        fitSP4->SetParameter(1, inv_f0);
        fitSP4->SetParLimits(1, -100, 100);
        fitSP6->SetParameter(1, inv_f0);
        fitSP6->SetParLimits(1, -100, 100);
    }
    if (Fixd0) {
        fitSP2->FixParameter(2, EffRan);
        fitSP4->FixParameter(2, EffRan);
        fitSP6->FixParameter(2, EffRan);
    } else {
        fitSP2->SetParameter(2, EffRan);
        fitSP2->SetParLimits(2, -50, 50);
        fitSP4->SetParameter(2, EffRan);
        fitSP4->SetParLimits(2, -50, 50);
        fitSP6->SetParameter(2, EffRan);
...
bool PotentialDesignerEngine(char *BaseFileName) {
    ...
    char *pot = new char[64]; // Gauss, DoubleGauss (default), Yukawa, YukawaDLM (modified to have some repulsive core
                              // to avoid singulartities)
    strcpy(pot, "DoubleGauss");
    char *pw = new char[4]; // in which partial wave should we place the potential. Possible values are s,p,d,f , s is
                            // the default
    strcpy(pw, "s");
    ...
    while (!feof(InFile)) {
        ...
        if (strcmp(ch_dummy_1, "m1") == 0) {
            if (!isNumber(ch_dummy_2)) {
                printf(
                    "\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numberic m1 value!\n",
                    TextFileName.Data());
            } else {
        ...
        if (strcmp(ch_dummy_1, "m2") == 0) {
            if (!isNumber(ch_dummy_2)) {
                printf(
                    "\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numberic m2 value!\n",
                    TextFileName.Data());
            } else {
        ...
        if(strcmp(ch_dummy_1,"f_goal")==0){
          if(!isNumber(ch_dummy_2)){
            printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numberic scattering
        length!\n",TextFileName.Data());
          }
          else{
        ...
        if(strcmp(ch_dummy_1,"f_err")==0){
          if(!isNumber(ch_dummy_2)){
            printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numberic error of the
        scattering length!\n",TextFileName.Data());
          }
          else{
        ...
        if(strcmp(ch_dummy_1,"d_goal")==0){
          if(!isNumber(ch_dummy_2)){
            printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numberic effective
        range!\n",TextFileName.Data());
          }
          else{
        ...
        */
        if (strcmp(ch_dummy_1, "kmin") == 0) {
            if (!isNumber(ch_dummy_2)) {
                printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numberic value for "
                       "kMin!\n",
                       TextFileName.Data());
            } else {
        ...
        if (strcmp(ch_dummy_1, "kbin") == 0) {
            if (!isInteger(ch_dummy_2)) {
                printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-integer value for "
                       "kBin!\n",
                       TextFileName.Data());
            } else {
        ...
        if (strcmp(ch_dummy_1, "pot") == 0) {
            if (strcmp(ch_dummy_2, "gauss") && strcmp(ch_dummy_2, "doublegauss") && strcmp(ch_dummy_2, "yukawa") &&
                strcmp(ch_dummy_2, "yukawadlm") && strcmp(ch_dummy_2, "usmanicore")) {
                printf(
                    "\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set an unknown potential %s!\n",
        ...
        if (strcmp(ch_dummy_1, "pw") == 0) {
            if (strcmp(ch_dummy_2, "s") && strcmp(ch_dummy_2, "p") && strcmp(ch_dummy_2, "d") &&
                strcmp(ch_dummy_2, "f")) {
                printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set an unknown partial wave "
                       "%s!\n",
                       TextFileName.Data(), ch_dummy_2);
            } else {
        ...
        if (strcmp(ch_dummy_1, "v_par1") == 0) {
            if (!isNumber(ch_dummy_2)) {
                printf("\033[1;33mWARNING!\033[0m Possible bad input-file (%s), trying to set a non-numerical value "
                       "for par1!\n",
                       TextFileName.Data());
            } else {
...
TF1 *fit_source_kdp(TH1F *hSrc, KdpPars &SrcPar, double &Chi2, double Chi2_Limit, double KdpFitMax, int KdpMode) {
    ...
    lowerlimit = 0;
    if (upperlimit > KdpFitMax + 2)
        upperlimit = KdpFitMax + 2;
...
DLM_Histo<KdpPars> *Convert_3Dsource_Kdp(DLM_Histo<float> &dlmMtKstarRstar, const bool CecaStyle, double Chi2_Limit,
                                         double KdpFitMax, int KdpMode) {
    TString BaseFileName =
        TString::Format("/home/dimihayl/Software/LocalFemto/Output/HighMtProblem/CECA_kstar_vs_integrated/");
    ...
    for (unsigned uMt = 0; uMt < NumMtBins; uMt++) {
        for (unsigned uKstar = 0; uKstar < NumKstarBins; uKstar++) {
            ...
            for (unsigned uRad = 0; uRad < NumRadBins; uRad++) {
                double src_val = CecaStyle ? dlmMtKstarRstar.GetBinContent(uKstar, uRad, uMt)
                                           : dlmMtKstarRstar.GetBinContent(uMt, uKstar, uRad);
                double src_err;
                if (src_val == 0) {
                    src_err = 1;
                } else {
                    src_err = CecaStyle ? dlmMtKstarRstar.GetBinError(uKstar, uRad, uMt)
                                        : dlmMtKstarRstar.GetBinError(uMt, uKstar, uRad);
                }
            ...
            hToFit->SetName(
                TString::Format("hToFit_%u_%.0f", uMt, dlmMtKstarRstar.GetBinCenter(CecaStyle ? 0 : 1, uKstar)));
            fit_ptr->SetName(
                TString::Format("fit_ptr_%u_%.0f", uMt, dlmMtKstarRstar.GetBinCenter(CecaStyle ? 0 : 1, uKstar)));
