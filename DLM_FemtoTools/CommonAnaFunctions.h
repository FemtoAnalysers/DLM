...
class DLM_CommonAnaFunctions {
  ...
  public:
    ...
    // POT:
    //   "AV18"
    void SetUpCats_pp(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                      const int &SourceVar = 0);
    void SetUpCats_ppic(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                        const int &SourceVar = 200);
    ...
    void SetUpCats_pL(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                      const int &SourceVar = 0);
    void SetUpCats_pS0(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                       const int &SourceVar = 0);
    void SetUpCats_pSp(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                       const int &SourceVar = 0);
    // POT:
    //   "pXim_Lattice" (the first version)
    //   "pXim_HALQCD1" (the second version, THE ONE TO USE)
    void SetUpCats_pXim(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                        const int &SourceVar = 0);
    void SetUpCats_pXi0(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                        const int &SourceVar = 0);
    void SetUpCats_pOmegam(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                           const int &SourceVar = 0);
    void SetUpCats_Kd(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                      const int &SourceVar = 0);
    void SetUpCats_pi_d(CATS &Kitty, const TString &POT, const TString &SOURCE, const int &PotVar = 0,
                        const int &SourceVar = 0);
    ...
    void SetUpBinning_pp(const TString &DataSample, unsigned &NumMomBins, double *&MomBins, double *&FitRegion,
                         const int &MomBinVar = 0, const int &FitRegVar = 0);
    ...
    void SetUpBinning_LKmin(const TString &DataSample, unsigned &NumMomBins, double *&MomBins, double *&FitRegion,
                            const int &MomBinVar = 0, const int &FitRegVar = 0);
    ...
    void SetUpLambdaPars_pL(const TString &DataSample, const int &Variation_p, const int &Variation_L,
                            double *lambda_pars);
    void SetUpLambdaPars_pXim(const TString &DataSample, const int &Variation_p, const int &Variation_Xim,
                              double *lambda_pars);
    void SetUpLambdaPars_LKmin(const TString &DataSample, const int &Variation_L, const int &Variation_K,
                               double *lambda_pars, int SamplePurity);
    ...
    TH1F *GetAliceExpCorrFun(const TString &DataSample, const TString &System, const TString &CutVar, const int &iReb,
                             const bool &AddSyst = false, const int mTbin = -1);
...
void RootFile_DlmSource(const TString &RootFileName, const TString &GraphName, CATS *Kitty, const unsigned &NumBins,
                        const double &rMin, const double &rMax, const double &lambda = 1, const bool &FourPi = true);
...
// in addition, we should have a BaseName.txt input file for the input arguments. The resulting pot pars will be added
// to this file The required input is the following:
...
//   pot     Gauss, DoubleGauss (default), Yukawa, YukawaDLM (modified to have some repulsive core to avoid
//   singulartities) pw      in which partial wave should we place the potential. Possible values are s,p,d,f , s is the
//   default coulomb do we include the coulomb in the evaluation (give q1*q2). By default 0 (no).
...
TF1 *fit_source_kdp(TH1F *hSrc, KdpPars &SrcPar, double &Chi2, double Chi2_Limit = 3, double KdpFitMax = 8,
                    int KdpMode = 2);
...
DLM_Histo<KdpPars> *Convert_3Dsource_Kdp(DLM_Histo<float> &dlmMtKstarRstar, const bool CecaStyle = false,
                                         double Chi2_Limit = 3, double KdpFitMax = 8, int KdpMode = 2);
