...
void Wrap_pp_Epelbaum(const char *InputFileName, const char *OutputFolder) {
    ...
    for (unsigned uDir = 0; uDir < num_dirs; uDir++) {
        ...
        for (int uMom = 0; uMom < num_mom_bins; uMom++) {
            ...
            TH1F *hist_wf_real =
                (TH1F *)(dir2->FindObjectAny(TString::Format("hist_wf_real_vs_r_p%i", TMath::Nint(mom))));
            TH1F *hist_wf_imag =
                (TH1F *)(dir2->FindObjectAny(TString::Format("hist_wf_imag_vs_r_p%i", TMath::Nint(mom))));
            if (!hist_wf_real) {
            ...
            for (int uRad = 0; uRad < num_rad_bins; uRad++) {
                ...
                complex<double>
                    wf_val; // = (hist_wf_real->GetBinContent(uRad+1)*rad, hist_wf_imag->GetBinContent(uRad+1)*rad);
                ...
                // usleep(100e3);
                if (fabs(rad - hist_wf_real->GetBinCenter(uRad + 1)) > 1e-3 ||
                    fabs(rad - hist_wf_imag->GetBinCenter(uRad + 1)) > 1e-3) {
...
void Wrap_pp_Norfolk(const char *InputFileName, const char *OutputFolder, const char *descriptor) {
    ...
    for (unsigned short usCh = 0; usCh < num_channels; usCh++) {
        for (unsigned short usPw = 0; usPw < num_pw[usCh]; usPw++) {
            if (HistoWF[usCh][usPw].IsInitialized()) {
                TString WF_file_name =
                    TString::Format("%s/wf_%s_usCh%u_usPw%u.dlm", OutputFolder, descriptor, usCh, usPw);
                TString PS_file_name =
                    TString::Format("%s/ps_%s_usCh%u_usPw%u.dlm", OutputFolder, descriptor, usCh, usPw);
                HistoWF[usCh][usPw].QuickWrite(WF_file_name.Data(), true);
                HistoPS[usCh][usPw].QuickWrite(PS_file_name.Data(), true);
            }
            // delete dlm_histo[0][usCh][usPw];
