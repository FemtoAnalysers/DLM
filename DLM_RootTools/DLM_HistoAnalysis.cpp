...
bool SameTH1structure(const TH1 *h1, const TH1 *h2) {
    ...
    for (int uBin = 0; uBin < h1->GetNbinsX(); uBin++) {
        if (h1->GetXaxis()->GetBinLowEdge(uBin + 1) != h2->GetXaxis()->GetBinLowEdge(uBin + 1))
            return false;
        if (h1->GetXaxis()->GetBinUpEdge(uBin + 1) != h2->GetXaxis()->GetBinUpEdge(uBin + 1))
            return false;
    }
    return true;
}
...
void GetCentralInterval(TF1 *f1, const double &min, const double &max, double alpha, double &from, double &to,
...
void DrawCentralInterval(TH1F *&h1, double Median, double LowReach, double UpReach, int NumNewBins, TH1F *&h1main,
                         TH1F *&h1bLow, TH1F *&h1bLow2, TH1F *&h1bUp, TH1F *&h1bUp2, TH1F *&h1median) {
    ...
    h1main = new TH1F(TString::Format("%s_vis", h1->GetName()), TString::Format("%s_vis", h1->GetName()), NumNewBins,
                      from, to);
    ...
    h1bLow = new TH1F(TString::Format("%s_vis2l", h1->GetName()), TString::Format("%s_vis2l", h1->GetName()),
                      FirstNonZeroBin - 1, from, FNZB_low);
    h1bUp = new TH1F(TString::Format("%s_vis2u", h1->GetName()), TString::Format("%s_vis2u", h1->GetName()),
                     NumNewBins - LastNonZeroBin, LNZB_up, to);
    h1bLow2 = new TH1F(TString::Format("%s_vis2l2", h1->GetName()), TString::Format("%s_vis2l2", h1->GetName()), 1,
                       FNZB_low, LowReach);
    h1bUp2 = new TH1F(TString::Format("%s_vis2u2", h1->GetName()), TString::Format("%s_vis2u2", h1->GetName()), 1,
                      UpReach, LNZB_up);

    h1median = new TH1F(TString::Format("%s_vis3", h1->GetName()), TString::Format("%s_vis3", h1->GetName()), 1,
                        Median - NewBinRange / 8., Median + NewBinRange / 8.);
...
HistoAddRatios::HistoAddRatios(const unsigned numterms, const char *OutHistoName, const unsigned numbins,
...
HistoAddRatios::~HistoAddRatios() {
    if (IgnoreUncertainty) {
        delete[] IgnoreUncertainty;
        IgnoreUncertainty = NULL;
    }
    if (MickeyMousePoisson) {
        delete[] MickeyMousePoisson;
        MickeyMousePoisson = NULL;
    }
    if (fNormalization) {
...
void HistoAddRatios::SetIgnoreUncertainty(const unsigned WhichOne, const bool yes) {
    if (WhichOne >= NumTerms) {
        printf(
            "\033[1;33mWARNING:\033[0m Bad input in HistoAddRatios::SetIgnoreUncertainty, only %u terms are allowed\n",
            NumTerms);
        return;
    }
    if (IgnoreUncertainty[WhichOne] != yes && hResult) {
        delete hResult;
        hResult = NULL;
    }
...
void HistoAddRatios::SetNormalization(const unsigned WhichOne, const float Norm) {
    ...
    if (fNormalization[WhichOne] != Norm && hResult) {
        delete hResult;
        hResult = NULL;
    }
...
void HistoAddRatios::SetConstant(const unsigned WhichOne, const float value, const float error) {
    ...
    if (Numerator[WhichOne]) {
        Numerator[WhichOne] = NULL;
    }
    if (Denominator[WhichOne]) {
        Denominator[WhichOne] = NULL;
    }
    if (Ratio[WhichOne]) {
        Ratio[WhichOne] = NULL;
    }
...
void HistoAddRatios::SetNumIter(const unsigned numiter) {
    if (numiter < 100) {
        printf(
            "\033[1;33mWARNING:\033[0m Very small number of iterations in HistoAddRatios (%u) was set, reconsider?\n",
            numiter);
    }
    if (NumIter != numiter && hResult) {
        delete hResult;
        hResult = NULL;
    }
    NumIter = numiter;
...
void HistoAddRatios::SetCompareToNullHypothesis(const bool compare, const unsigned MinEntriesForNsigma) {
    if (MinEntriesForNsigma < 10) {
        printf("\033[1;33mWARNING:\033[0m Very small number of entries required for the evaluation of nsigma in "
               "HistoAddRatios (%u) was set, reconsider?\n",
               MinEntriesForNsigma);
    }
    if (CompareToNull != compare && hResult) {
        delete hResult;
        hResult = NULL;
    }
    if (CompareToNull && EntriesForNsigma != MinEntriesForNsigma && hResult) {
        delete hResult;
        hResult = NULL;
    }
...
TH2F *HistoAddRatios::GetResult() {
    if (!hResult) {
        ...
        for (unsigned uBin = 0; uBin < NbinsX; uBin++) {
            ...
            for (unsigned uTerm = 0; uTerm < NumTerms; uTerm++) {
                if (Ratio[uTerm]) {
                    RatioProjection[uTerm] =
                        Ratio[uTerm]->ProjectionY(TString::Format("RatioProjection_%u", uTerm), uBin + 1, uBin + 1);
                } else {
            ...
            for (unsigned uIter = 0; uIter < NumIter; uIter++) {
                ...
                for (unsigned uTerm = 0; uTerm < NumTerms; uTerm++) {
                    if (hNormalization[uTerm])
                        norm = hNormalization[uTerm]->GetBinContent(uBin + 1);
                    else
                        norm = fNormalization[uTerm];
                    // division of yields
                    if (Numerator[uTerm] && Denominator[uTerm]) {
                        // no errors
                        if (IgnoreUncertainty[uTerm]) {
                            sum += norm * Numerator[uTerm]->GetBinContent(uBin + 1) /
                                   Denominator[uTerm]->GetBinContent(uBin + 1);
                        } // with errors
                        else {
                    ...
                    // the case in which we have only set the ratio
                        ...
                        // with errors
                        // By Default we assume Gaussian errors, BUT if MickeyMousePoisson is true, we do the following
                        // trick: assume that the uncertainty of the ratio is a scaled (by factor A) down Poisson (A*N),
                        // where the errors are A*sqrt(N). we can than estimate the effective yield based on the value
                        // and error from the bin, and sample the uncertainties from there, before finally rescaling by
                        // the corresponding factor A. This way, we avoid problems with negative values and should get a
                        // more accurate result in case of investigating corr. functions, where the error is dominated
                        // by the numerator (Poisson)
                        else {
                            if (!MickeyMousePoisson[uTerm]) {
                                numer = rangen->Gaus(Numerator[uTerm]->GetBinContent(uBin + 1),
                                                     Numerator[uTerm]->GetBinError(uBin + 1));
                            } else {
                ...
                if (CompareToNull) {
                    ...
                    BelowZero += sum < 0;
                    AtZero += sum == 0;
                    if ((AboveZero >= EntriesForNsigma && BelowZero >= EntriesForNsigma && uIter + 1 >= MinIter) ||
                        uIter + 1 == NumIter) {
                        if (AboveZero > BelowZero)
                            UPVAL = 2. * BelowZero + AtZero;
                        else
                            UPVAL = 2. * AboveZero + AtZero;
                        pValue[uBin] = double(UPVAL) / double(uIter + 1);
                        // printf(" %u raw pval = %.5f\n",uBin,pValue[uBin]);
                        if (pValue[uBin])
                            nSigma[uBin] = sqrt(2) * TMath::ErfcInverse(pValue[uBin]);
...
double HistoAddRatios::GetTotNsig(const bool Fisher) { return sqrt(2) * TMath::ErfcInverse(GetTotPval(Fisher)); }
...
void HistoAddRatios::SetExpectationUncertainty(const bool experr) { ExpectationUncertainty = experr; }
void HistoAddRatios::GetCentralInt(const unsigned WhichBin, const double &nsigma, double &Median, double &Low,
                                   double &Up) {
    if (WhichBin >= NbinsX) {
