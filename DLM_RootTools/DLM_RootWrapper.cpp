...
TH1F *Convert_DlmHisto_TH1F(const DLM_Histo<float> *input, const char *name) {
    ...
    for (unsigned uBinX = 0; uBinX < NumBinsX; uBinX++) {
        ...
        if (input->GetBinContent(WhichBin) != input->GetBinContent(WhichBin) || isinf(input->GetBinContent(WhichBin))) {
...
TH1F *GetSourceFromCATS(CATS &kitty, const double kstar, const double costheta, const char *name) {
    if (kitty.ComputedSource(kitty.GetMomentum(0), 0.1, 0) < 0)
        return NULL;
    DLM_Histo<float> *source_clone = kitty.CopyComputedSource();
    if (!source_clone)
        return NULL;
    if (kstar < kitty.GetMomBinLowEdge(0) || kstar > kitty.GetMomBinUpEdge(kitty.GetNumMomBins()))
        return NULL;
...
TGraph *GetSourceFromCATS_TGraph(CATS &kitty, const double kstar, const double costheta, const char *name) {
    ...
    unsigned counter = 0;
    for (unsigned uRad = 0; uRad < source_clone->GetNbins(1); uRad++) {
        if (source_clone->GetBinContent(bin_mom, uRad, bin_cth) == 0)
            continue;
        gSource->SetPoint(counter++, source_clone->GetBinCenter(1, uRad),
                          source_clone->GetBinContent(bin_mom, uRad, bin_cth));
        // printf("%u %f %f\n",uRad, source_clone->GetBinCenter(1,uRad),
        // source_clone->GetBinContent(bin_mom,uRad,bin_cth));
    }
    delete source_clone;
    return gSource;
