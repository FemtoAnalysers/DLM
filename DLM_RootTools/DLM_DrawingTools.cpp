...
DLM_DtColor::DLM_DtColor(const unsigned &NumCol) : NumColors(NumCol), PresetForCatsPaper(false) {
    ...
    *RainbowArray = TColor::GetPalette();
    ...
    for (unsigned uCol = 0; uCol < NumCol; uCol++) {
        ...
        if (Progress < Frac) {
            RgbRed = 1;
            RgbGreen = Progress * 4.5;
            RgbBlue = 0;
        } else if (Progress < 2. * Frac) {
            RgbRed = 1 - (Progress - Frac) * 4.5;
            RgbGreen = 1;
            RgbBlue = 0;
        } else if (Progress < 3. * Frac) {
            RgbRed = 0;
            RgbGreen = 1;
            RgbBlue = (Progress - 2. * Frac) * 4.5;
        } else if (Progress < 4. * Frac) {
            RgbRed = 0;
            RgbGreen = 1 - (Progress - 3. * Frac) * 4.5;
            RgbBlue = 1;
        } else {
            RgbRed = (Progress - 0.889) * 4.5;
            RgbGreen = 0;
            RgbBlue = 1;
        }
