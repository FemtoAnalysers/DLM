...
void DLM_MultiFit::ClearSpectra() {
    if (HistoToFit) {
        delete[] HistoToFit;
        delete[] FitFunction;
        for (int iSpec = 0; iSpec < max_spectra; iSpec++)
            delete[] spec_par[iSpec];
        delete[] spec_par;
        HistoToFit = NULL;
        FitFunction = NULL;
        spec_par = NULL;
...
bool DLM_MultiFit::SetEqualPar(unsigned short spec1, int par1, unsigned short spec2, int par2) {
    ...
    for (int i = 0; i < spec1; i++)
        whichpar1 += FitFunction[i]->GetNpar();
    whichpar1 += par1;

    unsigned int whichpar2 = 0;
    for (int i = 0; i < spec2; i++)
        whichpar2 += FitFunction[i]->GetNpar();
    whichpar2 += par2;
...
bool DLM_MultiFit::SetEqualPar(unsigned int whichpar1, unsigned int whichpar2) {
    ...
    if (whichpar1 >= totalpar || whichpar2 >= totalpar)
        return false;
...
ROOT::Fit::FitResult DLM_MultiFit::PerformGlobalFit(bool printinfo) {
    ...
    for (unsigned short i = 0; i < num_spectra; i++) {
        ...
        double Min, Max;
        FitFunction[i]->GetRange(Min, Max);
    ...
    if (!CorrectCovMatrix) {
        // PrintError("Please note, that the covariance matrix is wrong, hense the fit might be incorrect!",
        // "DLM_MultiFit::PerformGlobalFit");
    }
...
double DLM_MultiFit::operator()(const double *par) {
    ...
    for (unsigned int ispec = 0; ispec < num_spectra; ispec++) {
        if (!spec_par[ispec])
            spec_par[ispec] = new double[FitFunction[ispec]->GetNpar()];
        for (int ipar = 0; ipar < FitFunction[ispec]->GetNpar(); ipar++) {
