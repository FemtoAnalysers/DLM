...
DLM_SubPads::~DLM_SubPads() { Destroy(); }
...
Float_t DLM_SubPads::ConvertYPadPos(Float_t ypos) { return 1 - ypos; }

Bool_t DLM_SubPads::cd(UShort_t WhichPad) {
    if (WhichPad < NumSubPads)
        SubPad[WhichPad]->cd();
    else {
        Printf("WARNING! Bad input in DLM_SubPads::cd");
        return kFALSE;
    }
    return kTRUE;
}
...
void DLM_SubPads::SetLogx(UShort_t WhichPad, Bool_t lg) {
    if (WhichPad >= NumSubPads) {
        Printf("WARNING! Bad input in DLM_SubPads::SetLogx");
        return;
    }
...
void DLM_SubPads::SetLogy(UShort_t WhichPad, Bool_t lg) {
    if (WhichPad >= NumSubPads) {
        Printf("WARNING! Bad input in DLM_SubPads::SetLogy");
        return;
    }
...
void DLM_SubPads::SetGridx(UShort_t WhichPad, Bool_t val) {
    if (WhichPad >= NumSubPads) {
        Printf("WARNING! Bad input in DLM_SubPads::SetGridx");
        return;
    }
...
void DLM_SubPads::SetGridy(UShort_t WhichPad, Bool_t val) {
    if (WhichPad >= NumSubPads) {
        Printf("WARNING! Bad input in DLM_SubPads::SetGridy");
        return;
    }
...
void DLM_SubPads::SetGrid(UShort_t WhichPad, Bool_t val) {
    if (WhichPad >= NumSubPads) {
        Printf("WARNING! Bad input in DLM_SubPads::SetGrid");
        return;
    }
...
Bool_t DLM_SubPads::AddSubPad(Float_t Left, Float_t Right, Float_t Bottom, Float_t Top) {
    if (Left < 0 || Left > 1) {
        Printf("WARNING! Bad input in DLM_SubPads::AddSubPad");
        return kFALSE;
    }
    if (Right < 0 || Right > 1) {
        Printf("WARNING! Bad input in DLM_SubPads::AddSubPad");
    ...
    for (UShort_t iPad = 0; iPad < NumSubPads; iPad++) {
        ...
        if (Left < SubPadPostion[iPad][kMarRight] && Right > SubPadPostion[iPad][kMarLeft] &&
            Bottom < SubPadPostion[iPad][kMarTop] && Top > SubPadPostion[iPad][kMarBottom])
            return kFALSE;
    }

    if (NumSubPads >= MaxSubPads)
        AllocMoreMemForSubPad();
    SubPad[NumSubPads] = new TPad(TString::Format("SubPad%i", NumSubPads), TString::Format("SubPad%i", NumSubPads),
...
Bool_t DLM_SubPads::SetMargin(UShort_t PadNr, Float_t Left, Float_t Right, Float_t Bottom, Float_t Top,
    ...
    if (PadNr >= NumSubPads) {
        Printf("ERROR in DLM_SubPads::SetMargin! Accessing non-existing TPad!");
        return kFALSE;
    }
    if (MainCoordinates) {
    ...
    if (Left < 0 || Left > 1) {
        Printf("WARNING! Bad input in DLM_SubPads::SetMargin");
        return kFALSE;
    }
    if (Right < 0 || Right > 1) {
        Printf("WARNING! Bad input in DLM_SubPads::SetMargin");
...
Float_t DLM_SubPads::GetScaleWidth(const UShort_t &PadNr) { return PadNr < NumSubPads ? ScaleFactor[PadNr][0] : 0; }
Float_t DLM_SubPads::GetScaleHeight(const UShort_t &PadNr) { return PadNr < NumSubPads ? ScaleFactor[PadNr][1] : 0; }

TCanvas *DLM_SubPads::GetCanvas() { return MainCanvas; }
