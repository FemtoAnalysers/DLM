...
DLM_CRAB_PM::~DLM_CRAB_PM() { MemCleanup(); }
...
void DLM_CRAB_PM::MemCleanup() {
    ...
    if (IntegratedSource) {
        for (unsigned ui = 0; ui < NumSpecies; ui++) {
            delete IntegratedSource[ui];
            IntegratedSource[ui] = NULL;
        }
        delete[] IntegratedSource;
        IntegratedSource = NULL;
    }
    /*
    if(rx){
...
void DLM_CRAB_PM::SetNumEvents(unsigned num) { NumEvents = num; }

void DLM_CRAB_PM::SetNumPerEvent(unsigned num) { NumPerEvent = num; }

void DLM_CRAB_PM::SetOutputFile(string OutputName) { OutputFileName = OutputName; }
...
void DLM_CRAB_PM::SetParticleMass(unsigned np, double mass) {
    if (np >= NumSpecies) {
        printf("ERROR! There are only %u particle species!\n", NumSpecies);
        return;
    }
...
void DLM_CRAB_PM::SetTemperature(double temp) { Temperature = temp; }
...
void DLM_CRAB_PM::RunPhasemaker(unsigned Seed) {
    ...
    for (unsigned ui = 0; ui < NumSpecies; ui++) {
        IntegratedSource[ui] =
            new TH1F(TString::Format("IntegratedSource%u", ui), TString::Format("IntegratedSource%u", ui),
    ...
    for (unsigned uEvents = 0; uEvents < NumEvents; uEvents++) {
        if (fptr)
            fprintf(fptr, "%10u%12u%10.3lf%10.3lf\n", uEvents + 1, NumPerEvent, 0.0, 0.0);
        for (i = 0; i < NumPerEvent; i++) {
            ...
            */
            ...
            if (fptr)
                fprintf(fptr, "%10u%12i%14.6e%14.6e%14.6e%14.6e%14.6e%14.6e%14.6e%14.6e%14.6e\n", i + 1,
                        ParticleSpecies[id], p[kx] / 1000.0, p[ky] / 1000.0, p[kz] / 1000.0, p[energy] / 1000.0,
...
double DLM_CRAB_PM::GetRandSpace(TRandom3& rangen, unsigned Species, int Coord){
    ...
    ExtrapolatedValue =
((SecondPointY-FirstPointY)*RandPositionX+SecondPointX*FirstPointY-FirstPointX*SecondPointY)/(SecondPointX-FirstPointX);
