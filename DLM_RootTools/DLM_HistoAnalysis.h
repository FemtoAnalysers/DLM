...
double FunctionVar(TF1 *f1, double min, double max, double *mean = NULL, double *mean2 = NULL,
...
// compute the PDF (i.e. errors) numeriacally, given: sum_i Norm_i * Num_i / Denom_i, where Num and Denom are
// histograms, which represent COUNTS! I.e. the uncertainties are Poisson.
class HistoAddRatios {
  public:
    HistoAddRatios(const unsigned numterms, const char *OutHistoName, const unsigned numbins, const double ylow,
                   const double yup);
    ~HistoAddRatios();
