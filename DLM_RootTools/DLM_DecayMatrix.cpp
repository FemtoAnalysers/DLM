...
DLM_DecayMatrix::~DLM_DecayMatrix() {
    if (Mass_Daughter1)
        delete[] Mass_Daughter1;
    if (Mass_Daughter2)
        delete[] Mass_Daughter2;
}
...
void DLM_DecayMatrix::SetFileName(const char *name) { strcpy(FileName, name); }
void DLM_DecayMatrix::SetHistoName(const char *name) { strcpy(HistoName, name); }
...
void DLM_DecayMatrix::SetMotherMass1(const double &mass) { Mass1 = mass; }
void DLM_DecayMatrix::SetNumDaughters2(const unsigned &numdaughters) {
    if (NumDaughters2 == numdaughters)
        return;
    if (Mass_Daughter2)
        delete[] Mass_Daughter2;
...
void DLM_DecayMatrix::SetMotherMass2(const double &mass) { Mass2 = mass; }
void DLM_DecayMatrix::SetMeanMomentum(const double &mean) { MomMean = mean; }
void DLM_DecayMatrix::SetMomentumSpread(const double &spread) { MomSpread = spread; }
...
void DLM_DecayMatrix::Run(const int &SEED, const unsigned &NumIter) {
    ...
    for (unsigned uIter = 0; uIter < NumIter; uIter++) {
        tvec_m1.SetXYZM(rangen.Gaus(MomMean, MomSpread), rangen.Gaus(MomMean, MomSpread),
                        rangen.Gaus(MomMean, MomSpread), Mass1);
        tvec_m2.SetXYZM(rangen.Gaus(MomMean, MomSpread), rangen.Gaus(MomMean, MomSpread),
                        rangen.Gaus(MomMean, MomSpread), Mass2);
